...
public:
   ...
   */
  std::vector<ZenithInstrumentPreset>
  getPresetsForInstrument(const std::string &instrumentId) const {
    std::vector<ZenithInstrumentPreset> presets;
    ...
    auto scanDir = [&](const juce::File &dir) {
      ...
      auto xmlFiles =
          dir.findChildFiles(juce::File::findFiles, true, "*.zpreset");
      ...
      auto jsonFiles =
          dir.findChildFiles(juce::File::findFiles, true, "*.json");

      DBG("  Found " + juce::String(jsonFiles.size()) + " .json files");

      for (const auto &file : jsonFiles) {
        auto loadedPresets =
            ZenithInstrumentPreset::loadPresetsFromJsonFile(file);
        for (const auto &preset : loadedPresets) {
          DBG("    Loaded: " + juce::String(preset.name) +
              " (ID: " + juce::String(preset.instrumentId) + ")");

          if (preset.instrumentId == instrumentId) {
            presets.push_back(preset);
          } else {
            DBG("      Skipped: Instrument ID mismatch (Expected: " +
    ...
    // Path:
    // C:/zenith/daw/zenith-core/Content/Instruments/Presets/<InstrumentID>
    juce::File repoDir("C:/zenith/daw/zenith-core/Content/Instruments/Presets");
    if (repoDir.exists()) {
      scanDir(repoDir.getChildFile(instrumentId));
    }

    // 3. DEV FALLBACK: Load from Content/Presets (Bank files)
    juce::File contentPresetsDir("C:/zenith/daw/zenith-core/Content/Presets");
    if (contentPresetsDir.exists()) {
      if (instrumentId == "zenith_poly_synth") {
        scanDir(contentPresetsDir.getChildFile("PolySynth"));
      } else if (instrumentId == "zenith_sampler") {
        scanDir(contentPresetsDir.getChildFile("Sampler"));
      }
    }
   ...
   */
  std::vector<ZenithInstrumentPreset>
  findPresetsByTag(const std::string &instrumentId,
   ...
   */
  std::vector<ZenithInstrumentPreset>
  findPresetsByCategory(const std::string &instrumentId,
...
private:
   ...
   */
  juce::File getInstrumentPresetsDir(const std::string &instrumentId,
                                     bool factory) const {
