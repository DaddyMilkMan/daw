...
void CommandAPI::initializeCommandMap() {
  ...
  commandMap["set_synth_parameter"] = CommandID::SetSynthParameter;
  ...
  commandMap["set_synth_filter_type"] = CommandID::SetSynthFilterType;
  ...
  commandMap["apply_synth_preset"] = CommandID::ApplySynthPreset;
  commandMap["randomize_synth_patch"] = CommandID::RandomizeSynthPatch;
  commandMap["analyze_synth_patch"] = CommandID::AnalyzeSynthPatch;
  commandMap["set_synth_unison_voices"] = CommandID::SetSynthUnisonVoices;
  commandMap["set_synth_unison_detune"] = CommandID::SetSynthUnisonDetune;
  commandMap["set_synth_unison_spread"] = CommandID::SetSynthUnisonSpread;
  commandMap["set_synth_unison_pan_random"] = CommandID::SetSynthUnisonPanRandom;
  commandMap["set_synth_arp_enable"] = CommandID::SetSynthArpEnable;
  commandMap["set_synth_arp_mode"] = CommandID::SetSynthArpMode;
  commandMap["set_synth_arp_rate"] = CommandID::SetSynthArpRate;
  commandMap["set_synth_arp_gate"] = CommandID::SetSynthArpGate;
  commandMap["set_synth_arp_swing"] = CommandID::SetSynthArpSwing;
  commandMap["set_synth_arp_hold"] = CommandID::SetSynthArpHold;
  ...
  // Synth Control Handlers
  registerCommand("set_synth_parameter",
                  [this](const juce::var &p) { return setSynthParameter(p); });
  registerCommand("set_synth_oscillator_wave",
                  [this](const juce::var &p) { return setSynthOscillatorWave(p); });
  registerCommand("set_synth_oscillator_detune",
                  [this](const juce::var &p) { return setSynthOscillatorDetune(p); });
  registerCommand("set_synth_oscillator_mix",
                  [this](const juce::var &p) { return setSynthOscillatorMix(p); });
  registerCommand("set_synth_filter_type",
                  [this](const juce::var &p) { return setSynthFilterType(p); });
  registerCommand("set_synth_filter_cutoff",
                  [this](const juce::var &p) { return setSynthFilterCutoff(p); });
  registerCommand("set_synth_filter_resonance",
                  [this](const juce::var &p) { return setSynthFilterResonance(p); });
  registerCommand("set_synth_filter_drive",
                  [this](const juce::var &p) { return setSynthFilterDrive(p); });
  registerCommand("set_synth_amp_envelope",
                  [this](const juce::var &p) { return setSynthAmpEnvelope(p); });
  registerCommand("set_synth_filter_envelope",
                  [this](const juce::var &p) { return setSynthFilterEnvelope(p); });
  registerCommand("set_synth_lfo_rate",
                  [this](const juce::var &p) { return setSynthLFORate(p); });
  registerCommand("set_synth_lfo_amount",
                  [this](const juce::var &p) { return setSynthLFOAmount(p); });
  registerCommand("set_synth_distortion",
                  [this](const juce::var &p) { return setSynthDistortion(p); });
  registerCommand("set_synth_chorus",
                  [this](const juce::var &p) { return setSynthChorus(p); });
  registerCommand("set_synth_reverb",
                  [this](const juce::var &p) { return setSynthReverb(p); });
  registerCommand("set_synth_delay",
                  [this](const juce::var &p) { return setSynthDelay(p); });
  registerCommand("apply_synth_preset",
                  [this](const juce::var &p) { return applySynthPreset(p); });
  registerCommand("randomize_synth_patch",
                  [this](const juce::var &p) { return randomizeSynthPatch(p); });
  registerCommand("analyze_synth_patch",
                  [this](const juce::var &p) { return analyzeSynthPatch(p); });
  registerCommand("set_synth_unison_voices",
                  [this](const juce::var &p) { return setSynthUnisonVoices(p); });
  registerCommand("set_synth_unison_detune",
                  [this](const juce::var &p) { return setSynthUnisonDetune(p); });
  registerCommand("set_synth_unison_spread",
                  [this](const juce::var &p) { return setSynthUnisonSpread(p); });
  registerCommand("set_synth_unison_pan_random",
                  [this](const juce::var &p) { return setSynthUnisonPanRandom(p); });
  registerCommand("set_synth_arp_enable",
                  [this](const juce::var &p) { return setSynthArpEnable(p); });
  registerCommand("set_synth_arp_mode",
                  [this](const juce::var &p) { return setSynthArpMode(p); });
  registerCommand("set_synth_arp_rate",
                  [this](const juce::var &p) { return setSynthArpRate(p); });
  registerCommand("set_synth_arp_gate",
                  [this](const juce::var &p) { return setSynthArpGate(p); });
  registerCommand("set_synth_arp_swing",
                  [this](const juce::var &p) { return setSynthArpSwing(p); });
  registerCommand("set_synth_arp_hold",
                  [this](const juce::var &p) { return setSynthArpHold(p); });
}
