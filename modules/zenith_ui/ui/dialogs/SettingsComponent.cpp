...
AISettingsTab::AISettingsTab() {
    ...
    validateButton_ = std::make_unique<SkiaButton>("Verify & Save");
    ...
    // Model Mode Section
    modelModeSelector_ = std::make_unique<SkiaComboBox>("AI Model Mode");
    modelModeSelector_->addItem("Obedient - Precise, instruction-following", 1);
    modelModeSelector_->addItem("Teacher - Educational, explains reasoning", 2);
    modelModeSelector_->addItem("Creative - Imaginative, suggests alternatives", 3);
    modelModeSelector_->setSelectedId((int)Settings::getInstance().getAIModelMode() + 1, false);
    modelModeSelector_->onChange = [this]() {
        Settings::getInstance().setAIModelMode(
            static_cast<Settings::AIModelMode>(modelModeSelector_->getSelectedId() - 1));
    };
    addAndMakeVisible(modelModeSelector_.get());

    // Feature Toggles
    reasoningDisplayToggle_ = std::make_unique<SkiaButton>("Show AI Reasoning");
    reasoningDisplayToggle_->setStyle(SkiaButton::Style::Secondary);
    reasoningDisplayToggle_->setToggleable(true);
    reasoningDisplayToggle_->setToggleState(Settings::getInstance().getWingmanReasoningDisplay());
    reasoningDisplayToggle_->onClick = [this]() {
        Settings::getInstance().setWingmanReasoningDisplay(reasoningDisplayToggle_->getToggleState());
    };
    addAndMakeVisible(reasoningDisplayToggle_.get());

    webSearchToggle_ = std::make_unique<SkiaButton>("Enable Web Search");
    webSearchToggle_->setStyle(SkiaButton::Style::Secondary);
    webSearchToggle_->setToggleable(true);
    webSearchToggle_->setToggleState(Settings::getInstance().getWingmanWebSearch());
    webSearchToggle_->onClick = [this]() {
        Settings::getInstance().setWingmanWebSearch(webSearchToggle_->getToggleState());
    };
    addAndMakeVisible(webSearchToggle_.get());

    streamingToggle_ = std::make_unique<SkiaButton>("Stream Responses");
    streamingToggle_->setStyle(SkiaButton::Style::Secondary);
    streamingToggle_->setToggleable(true);
    streamingToggle_->setToggleState(Settings::getInstance().getWingmanStreaming());
    streamingToggle_->onClick = [this]() {
        Settings::getInstance().setWingmanStreaming(streamingToggle_->getToggleState());
    };
    addAndMakeVisible(streamingToggle_.get());

    dawContextToggle_ = std::make_unique<SkiaButton>("Include DAW Context");
    dawContextToggle_->setStyle(SkiaButton::Style::Secondary);
    dawContextToggle_->setToggleable(true);
    dawContextToggle_->setToggleState(Settings::getInstance().getWingmanDAWContext());
    dawContextToggle_->onClick = [this]() {
        Settings::getInstance().setWingmanDAWContext(dawContextToggle_->getToggleState());
    };
    addAndMakeVisible(dawContextToggle_.get());

    // Context Management
    historyLengthSlider_ = std::make_unique<SkiaSlider>("Conversation History", design::colors::CYAN);
    historyLengthSlider_->setRange(1, 50, (float)Settings::getInstance().getWingmanMaxHistory());
    historyLengthSlider_->setValue((float)Settings::getInstance().getWingmanMaxHistory());
    historyLengthSlider_->onValueChange = [](float v) {
        Settings::getInstance().setWingmanMaxHistory((int)v);
    };
    addAndMakeVisible(historyLengthSlider_.get());
}

void AISettingsTab::resized() {
    int y = 80;
    int leftCol = 20;
    int rightCol = getWidth() / 2 + 10;
    int colWidth = getWidth() / 2 - 40;

    // API Key Section (full width)
    if (apiKeyEditor_) apiKeyEditor_->setBounds(leftCol, y, 350, 30);
    if (validateButton_) validateButton_->setBounds(380, y, 120, 30);
    y += 35;
    if (statusLabel_) statusLabel_->setBounds(leftCol, y, 400, 20);
    y += 50;

    // Model Mode Section (full width)
    if (modelModeSelector_) modelModeSelector_->setBounds(leftCol, y, 380, 32);
    y += 60;

    // Feature Toggles - Two column layout
    if (reasoningDisplayToggle_) reasoningDisplayToggle_->setBounds(leftCol, y, 200, 30);
    if (webSearchToggle_) webSearchToggle_->setBounds(rightCol, y, 200, 30);
    y += 45;

    if (streamingToggle_) streamingToggle_->setBounds(leftCol, y, 200, 30);
    if (dawContextToggle_) dawContextToggle_->setBounds(rightCol, y, 200, 30);
    y += 60;

    // Context Management
    if (historyLengthSlider_) historyLengthSlider_->setBounds(leftCol, y, 300, 30);
}
...
void AISettingsTab::drawSkia(SkCanvas* canvas) {
    ...
    canvas->drawString("API Configuration", 20, 70, sectionFont, textPaint);

    // Section: Model Mode
    textPaint.setColor(design::unified::accent_primary());
    canvas->drawString("Model Behavior", 20, 175, sectionFont, textPaint);

    // Section: Features
    textPaint.setColor(design::unified::accent_primary());
    canvas->drawString("AI Features", 20, 255, sectionFont, textPaint);

    // Section: Context
    textPaint.setColor(design::unified::accent_primary());
    canvas->drawString("Context Management", 20, 360, sectionFont, textPaint);

    // Help text
    textPaint.setColor(design::unified::text_secondary());
    canvas->drawString("Get an API key at console.x.ai", 20, 130, labelFont, textPaint);
    canvas->drawString("Messages to include in AI context", 20, 405, labelFont, textPaint);
}
    ...
    testClient_->sendChat(
        ...
        [this, key](juce::String response) {
            juce::MessageManager::callAsync([this, key]() {
                ...
                validateButton_->setText("Verify & Save");
