/*
  ==============================================================================
    ...
    Consolidated platform-specific logic for all targets.
...
namespace {
    ...
    bool libsecretAvailable() { return true; }
#endif
...
//==============================================================================
// Public API - Platform Implementations
//==============================================================================
...
bool SecureKeyStore::storeKey(const juce::String &keyName,
    ...
    if (libsecretAvailable())
    {
        ...
        if (error) {
            ZENITH_LOG_ERROR("libsecret store error: " + juce::String(error->message));
            g_error_free(error);
        } else if (result) {
            return true;
        }
...
bool SecureKeyStore::retrieveKey(const juce::String &keyName,
    ...
    if (libsecretAvailable())
    {
        ...
        if (error) {
            ZENITH_LOG_ERROR("libsecret lookup error: " + juce::String(error->message));
            g_error_free(error);
        } else if (password) {
...
bool SecureKeyStore::deleteKey(const juce::String &keyName) {
    ...
    if (libsecretAvailable())
    {
        ...
        if (error) {
            ZENITH_LOG_ERROR("libsecret delete error: " + juce::String(error->message));
            g_error_free(error);
        } else if (result) {
            return true;
        }
