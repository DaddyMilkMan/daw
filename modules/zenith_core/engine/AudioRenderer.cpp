...
void AudioRenderer::renderAudioGraph(
  ...
  context.numActiveAuxPtrs = 0;
  for (auto &buffer : context.auxBusBuffers) {
      buffer.clear();
      if (context.numActiveAuxPtrs < AudioRenderContext::kMaxTrackBuses) {
          context.auxBufferPtrsArray[context.numActiveAuxPtrs++] = &buffer;
      }
  }
    ...
    if (rn.type == RoutingGraph::RenderNode::Type::Track) {
      ...
      if (track->isFrozen()) {
        ...
        if (freezeBuffer != nullptr && freezeBuffer->getNumSamples() > 0) {
          ...
          if (auto* processor = track->getProcessor()) {
              ...
              // Create a span from the fixed array
              std::span<juce::AudioBuffer<float>* const> auxSpan(context.auxBufferPtrsArray.data(), context.numActiveAuxPtrs);
              processor->processBlock(trackInfo, scratchMidi_, auxSpan, nullptr);
          }
      ...
      if (track != nullptr) {
          ...
          track->getNextAudioBlock(trackInfo, playheadPosition, trackMidiInput,
                                    auxSpan, tempoMap, sidechainBuffer);
      }
