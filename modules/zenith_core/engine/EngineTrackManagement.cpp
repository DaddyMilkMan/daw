...
void Engine::removeTrack(int index) {
  ...
  // Keep a shared pointer to the track to defer its destruction
  auto trackToDestroy = tracks_[index];

  // Remove from vector and unregister listener
  ...
  // Update snapshot for audio thread FIRST so it stops accessing the track
  updateTrackSnapshot();

  // Defer resource release and destruction to GC
  RealTimeGarbageCollector::getInstance().deferDelete([trackToDestroy]() {
      trackToDestroy->releaseResources();
  });
