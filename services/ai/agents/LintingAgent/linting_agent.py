...
class LintingAgent:
    ...
    def scan_files(self, file_patterns: List[str]) -> LintReport:
        ...
        # Enforce juce::Graphics allowlist when scanning the full tree
        self._check_juce_graphics_allowlist(report)
    ...
    def _collect_juce_graphics_usages(self) -> List[str]:
        """Collect all juce::Graphics usages under apps/desktop/Source."""
        usages = []
        source_root = self.project_root / "apps" / "desktop" / "Source"
        ...
        for file_path in source_root.rglob("*"):
            ...
            if file_path.suffix not in {".cpp", ".h", ".hpp", ".cc", ".cxx", ".mm"}:
                continue

            try:
                with open(file_path, "r", encoding="utf-8", errors="ignore") as f:
                    for line_num, line in enumerate(f, start=1):
                        if "juce::Graphics" in line:
                            relative_path = file_path.relative_to(self.project_root)
                            usages.append(f"{relative_path}:{line_num}:{line.rstrip()}")
            except Exception:
                continue

        return usages

    def _check_juce_graphics_allowlist(self, report: LintReport) -> None:
        """Fail lint if juce::Graphics usages diverge from allowlist."""
        allowlist_path = self.project_root / "tools" / "lint" / "juce_graphics_allowlist.txt"

        if not allowlist_path.exists():
            report.findings.append(LintFinding(
                category=LintCategory.STRUCTURE,
                severity=LintSeverity.ERROR,
                description="juce::Graphics allowlist missing (tools/lint/juce_graphics_allowlist.txt)",
                file_path=allowlist_path,
                line_number=1,
                suggestion="Generate allowlist with: rg -n \"juce::Graphics\" apps/desktop/Source > tools/lint/juce_graphics_allowlist.txt"
            ))
            return

        current = self._collect_juce_graphics_usages()
        allowed = allowlist_path.read_text(encoding="utf-8", errors="ignore").splitlines()

        if current != allowed:
            added = [line for line in current if line not in allowed]
            removed = [line for line in allowed if line not in current]
            details = []
            if added:
                details.append(f\"{len(added)} added\")
            if removed:
                details.append(f\"{len(removed)} removed\")
            detail_text = \", \".join(details) if details else \"changes detected\"

            report.findings.append(LintFinding(
                category=LintCategory.STRUCTURE,
                severity=LintSeverity.ERROR,
                description=f\"juce::Graphics usage changed ({detail_text})\",\n                file_path=allowlist_path,
                line_number=1,
                suggestion=\"Update allowlist with: rg -n \\\"juce::Graphics\\\" apps/desktop/Source > tools/lint/juce_graphics_allowlist.txt\"
            ))
    ...
    def scan_project(self, exclude_dirs: Optional[List[str]] = None) -> LintReport:
        ...
        # Enforce juce::Graphics allowlist for UI migration
        self._check_juce_graphics_allowlist(report)
