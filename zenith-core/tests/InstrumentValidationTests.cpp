...
bool validateInstrumentMetadata(const juce::String &instrumentId,
  ...
  for (const auto &param : metadata->parameters) {
    ...
    if (param.defaultValue < param.minValue ||
  ...
  for (const auto &[macroId, value] : preset.macros) {
    if (value < 0.0f || value > 1.0f) {
      reportFailure(preset.instrumentId, preset.name, "Macro Range",
                    "Macro '" + macroId + "' value " + std::to_string(value) +
                        " out of range [0, 1]");
      valid = false;
    }
  }
...
bool testInstrumentPresets(const juce::String &instrumentId,
  ...
  auto presets =
      presetManager.getPresetsForInstrument(instrumentId.toStdString());

  if (presets.empty()) {
    std::cout << "[INFO] " << instrumentId << " has no factory presets"
              << std::endl;

    // Test with default state anyway
    auto instrument =
        InstrumentRegistry::getInstance().createInstrument(instrumentId);
  ...
  for (const auto &preset : presets) {
    ...
    auto instrument =
        InstrumentRegistry::getInstance().createInstrument(instrumentId);
    if (instrument == nullptr) {
      reportFailure(instrumentId.toStdString(), preset.name,
                    "Instrument Creation",
...
void performRTSafetyAnalysis() {
  ...
  std::cout << "  - zenith-core/Source/instruments/ZenithPolySynth.cpp:52-58"
            << std::endl;
  std::cout << "  - zenith-core/Source/instruments/ZenithPolySynth.cpp:90-131 "
               "(renderNextBlock)"
            << std::endl;
  std::cout
      << "  - zenith-core/Source/instruments/ZenithSampler.cpp (processBlock)"
      << std::endl;
  std::cout << "  - zenith-core/src/Engine.cpp:590-712 (renderBlock)"
            << std::endl;
  ...
  std::cout << "  ✓ Check: No std::vector::push_back (may allocate)"
            << std::endl;
  std::cout << "  ✓ Check: No std::mutex locks (use lock-free structures)"
            << std::endl;
...
int main() {
  std::cout << "=== Zenith DAW Instrument Stack Validation Tests ==="
            << std::endl;
  std::cout << std::endl;
  ...
  // Register built-in instruments (correct function name with namespace)
  ...
  for (const auto &instrumentId : instrumentIds) {
    std::cout << "=== Testing Instrument: " << instrumentId
              << " ===" << std::endl;
    ...
    if (!metadataValid) {
      std::cout << "[SKIP] Skipping preset tests due to invalid metadata"
                << std::endl;
      continue;
    }

    // Test 2-4: Validate and test presets
    bool presetsValid = testInstrumentPresets(instrumentId, metadataPtr);
  ...
  if (g_failures.empty()) {
    ...
    std::cout << "✓ " << instrumentIds.size() << " instruments validated"
              << std::endl;
  } else {
    std::cout << "✗ " << g_failures.size()
              << " validation failures detected:" << std::endl;
    std::cout << std::endl;
    ...
    for (const auto &[type, failures] : failuresByType) {
      std::cout << type << " failures (" << failures.size()
                << "):" << std::endl;
      for (const auto &failure : failures) {
        std::cout << "  - " << failure.instrumentId << " / "
                  << failure.presetName << ": " << failure.description
                  << std::endl;
      }
      std::cout << std::endl;
    }
