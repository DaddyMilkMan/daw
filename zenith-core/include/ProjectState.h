...
public:
    ...
    static const juce::Identifier PROP_START_SAMPLES;
    static const juce::Identifier PROP_LENGTH_SAMPLES;
     ...
     * @param trackId Track ID
     * @param clipId Clip ID (pre-generated by engine)
     * @param type Clip type ("audio" or "midi")
     * @param startBeats Start position in beats
     * @param lengthBeats Length in beats
     * @param startSamples Start position in samples (for sample-accurate sync)
     * @param lengthSamples Length in samples (for sample-accurate sync)
     * @return true if clip was added successfully
    ...
    /**
     * @brief Find clip and its parent track by ID
     * @param clipId Clip ID
     * @return Pair of (track ValueTree, clip ValueTree) - both invalid if not found
     * @note Message thread only
     */
    std::pair<juce::ValueTree, juce::ValueTree> findClipAndTrack(const juce::String& clipId);

    /**
     * @brief Update clip position (from arranger)
     * @param clipId Clip ID
     * @param startBeats New start position in beats
     * @param lengthBeats New length in beats
     * @return true if clip was found and updated
     * @note Message thread only
     */
    bool updateClipPosition(const juce::String& clipId, double startBeats, double lengthBeats);
    ...
    /**
     * @brief Debug helper to dump clip structure to console
     * @note Message thread only
     */
