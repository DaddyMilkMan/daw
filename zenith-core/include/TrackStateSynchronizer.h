 ...
 * Usage:
 * ```cpp
 * auto synchronizer = std::make_unique<TrackStateSynchronizer>(projectState, engine);
 * synchronizer->initialize();  // Start listening
 * ```
 *
 * The synchronizer will:
 * 1. Listen to TRACKS node for track add/remove
 * 2. Listen to each TRACK node for property changes (volume/pan/mute/solo/armed)
...
private:
     ...
     */
    void syncTrackProperty(const juce::ValueTree& track, const juce::Identifier& property);
