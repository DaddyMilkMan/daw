/**
 * @file ZenithSlider.cpp
 * @brief Implementation of beautiful custom slider component
 */
...
void ZenithSlider::mouseDrag(const juce::MouseEvent& event)
{
    ...
    float sensitivity = event.mods.isShiftDown() ? 0.002f : 0.005f;  // Fine control with Shift
...
void ZenithSlider::drawVerticalSlider(juce::Graphics& g, const juce::Rectangle<float>& bounds)
{
    ...
    auto trackBounds = bounds.withTrimmedBottom(20.0f).reduced(bounds.getWidth() * 0.3f, 4.0f);

    // Draw track
    drawTrack(g, trackBounds);

    // Calculate thumb position (top = 1.0, bottom = 0.0)
    float thumbY = trackBounds.getY() + (trackBounds.getHeight() * (1.0f - value_));
    float thumbSize = trackBounds.getWidth() * 1.5f;
    auto thumbBounds = juce::Rectangle<float>(thumbSize, thumbSize).withCentre(
        juce::Point<float>(trackBounds.getCentreX(), thumbY));
    ...
    if (isHovered_ || isDragging_)
    {
        ...
        auto valueTextBounds = thumbBounds.translated(thumbBounds.getWidth() + 4.0f, 0.0f);
...
void ZenithSlider::drawHorizontalSlider(juce::Graphics& g, const juce::Rectangle<float>& bounds)
{
    ...
    auto thumbBounds = juce::Rectangle<float>(thumbSize, thumbSize).withCentre(
        juce::Point<float>(thumbX, trackBounds.getCentreY()));
...
void ZenithSlider::drawTrack(juce::Graphics& g, const juce::Rectangle<float>& trackBounds)
{
    ...
    float cornerRadius = std::min(trackBounds.getWidth(), trackBounds.getHeight()) * 0.5f;
    ...
    if (orientation_ == Vertical)
    {
        ...
        backgroundGradient = juce::ColourGradient(
            ...
            juce::Colour(0xff1a1a1a), center.x, trackBounds.getBottom(),
            false);
    }
    else
    {
        ...
        backgroundGradient = juce::ColourGradient(
            ...
            juce::Colour(0xff1a1a1a), trackBounds.getRight(), center.y,
            false);
    }
    ...
    if (orientation_ == Vertical)
    {
        valueGradient = juce::ColourGradient(
            juce::Colour(0xff5ab4ff), filledBounds.getCentreX(), filledBounds.getY(),
            juce::Colour(0xff4a9eff), filledBounds.getCentreX(), filledBounds.getBottom(),
            false);
    }
    else
    {
        valueGradient = juce::ColourGradient(
            juce::Colour(0xff4a9eff), filledBounds.getX(), filledBounds.getCentreY(),
            juce::Colour(0xff5ab4ff), filledBounds.getRight(), filledBounds.getCentreY(),
            false);
    }
...
void ZenithSlider::drawThumb(juce::Graphics& g, const juce::Rectangle<float>& thumbBounds)
{
    ...
    juce::ColourGradient thumbGradient(
        ...
        juce::Colour(0xff2a2a2a), center.x, center.y + radius,
        false);
    ...
    g.fillEllipse(center.x - dotRadius, center.y - dotRadius, dotRadius * 2.0f, dotRadius * 2.0f);
