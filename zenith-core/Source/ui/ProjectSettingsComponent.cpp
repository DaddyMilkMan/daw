    ...
    : projectState_(state)
{
    ...
    timeSignatureNumDownButton_.onClick = [this]() { adjustTimeSignatureNum(-1); };
    ...
    timeSignatureDenDownButton_.onClick = [this]() { adjustTimeSignatureDen(-1); };
    ...
    sampleRateValue_.setColour(juce::Label::textColourId, juce::Colour(0xffcccccc));
    addAndMakeVisible(sampleRateValue_);

    // Action buttons
    applyButton_.setButtonText("Apply");
    applyButton_.setColour(juce::TextButton::buttonColourId, juce::Colour(0xff0A84FF));
    ...
    cancelButton_.setColour(juce::TextButton::buttonColourId, juce::Colour(0xff3A3A3C));
    ...
    tapTempoButton_.setColour(juce::TextButton::buttonColourId, juce::Colour(0xff34C759));
    tapTempoButton_.setColour(juce::TextButton::textColourOffId, juce::Colours::white);
}
...
void ProjectSettingsComponent::paint(juce::Graphics& g)
{
    ...
    juce::DropShadow shadow(juce::Colours::black.withAlpha(0.25f), 6, juce::Point<int>(0, 1));
    shadow.drawForPath(g, shadowPath);

    // Title with shadow
    g.setColour(juce::Colour(0x00000000).withAlpha(0.3f));
    g.setFont(juce::FontOptions(18.0f, juce::Font::bold));
    g.drawText("Project Settings", bounds.removeFromTop(50).translated(1, 1).reduced(PADDING, 0),
               juce::Justification::centredLeft, false);

    g.setColour(juce::Colours::white.withAlpha(0.95f));
    g.setFont(juce::FontOptions(18.0f, juce::Font::bold));
    g.drawText("Project Settings", bounds.toFloat().translated(0, -50).reduced(PADDING, 0),
               juce::Justification::centredLeft, false);
    ...
    // Project Name
    paintSettingRow(g, "Project Name", contentBounds.withHeight(ROW_HEIGHT).withY(contentBounds.getY() + rowIndex * (ROW_HEIGHT + SPACING)));
    // Draw validation icon
    if (projectNameValid_ && !projectNameEditor_.isEmpty())
    {
        g.setColour(juce::Colour(0xff34C759));
        g.drawText("✓", contentBounds.getRight() - 30, contentBounds.getY() + rowIndex * (ROW_HEIGHT + SPACING),
                   24, ROW_HEIGHT, juce::Justification::centred, false);
    }
    rowIndex++;

    // Tempo
    paintSettingRow(g, "Tempo (BPM)", contentBounds.withHeight(ROW_HEIGHT).withY(contentBounds.getY() + rowIndex * (ROW_HEIGHT + SPACING)));
    // Draw validation icon
    if (tempoValid_)
    {
        g.setColour(juce::Colour(0xff34C759));
    ...
    // Time Signature
    paintSettingRow(g, "Time Signature", contentBounds.withHeight(ROW_HEIGHT).withY(contentBounds.getY() + rowIndex * (ROW_HEIGHT + SPACING)));
    // Draw validation icon
    ...
    // Sample Rate
    paintSettingRow(g, "Sample Rate", contentBounds.withHeight(ROW_HEIGHT).withY(contentBounds.getY() + rowIndex * (ROW_HEIGHT + SPACING)));

    // Draw focus glows
    if (nameEditorFocusAnim_ > 0.01f)
    {
        auto editorBounds = projectNameEditor_.getBounds().toFloat().expanded(2.0f);
        g.setColour(juce::Colour(0xff0A84FF).withAlpha(nameEditorFocusAnim_ * 0.3f));
    ...
    if (tempoEditorFocusAnim_ > 0.01f)
    {
        ...
        g.setColour(juce::Colour(0xff0A84FF).withAlpha(tempoEditorFocusAnim_ * 0.3f));
        g.drawRoundedRectangle(editorBounds, 6.0f, 2.0f);
    }
}
...
void ProjectSettingsComponent::resized()
{
    ...
    // Time Signature row with stepper buttons
    {
        ...
        int slashWidth = 20;
        int halfWidth = (row.getWidth() - slashWidth - (buttonSize * 4) - (buttonSpacing * 6)) / 2;
...
void ProjectSettingsComponent::paintSettingRow(juce::Graphics& g, const juce::String& label,
                                               const juce::Rectangle<int>& bounds)
{
    // Label text with subtle color
    g.setColour(juce::Colours::white.withAlpha(0.7f));
    g.setFont(juce::FontOptions(13.0f, juce::Font::plain));
    g.drawText(label, bounds.withWidth(LABEL_WIDTH), juce::Justification::centredLeft, false);
...
//==============================================================================
void ProjectSettingsComponent::setupTextEditor(juce::TextEditor& editor, int maxLength)
{
    editor.setFont(juce::FontOptions(14.0f));
    editor.setColour(juce::TextEditor::backgroundColourId, juce::Colour(0xff2C2C2E));
    editor.setColour(juce::TextEditor::textColourId, juce::Colours::white.withAlpha(0.95f));
    editor.setColour(juce::TextEditor::outlineColourId, juce::Colour(0xff3A3A3C));
    editor.setColour(juce::TextEditor::focusedOutlineColourId, juce::Colour(0xff0A84FF));
    editor.setColour(juce::CaretComponent::caretColourId, juce::Colour(0xff0A84FF));
...
void ProjectSettingsComponent::textEditorTextChanged(juce::TextEditor& editor)
{
    ...
    } else if (&editor == &timeSignatureNumEditor_ || &editor == &timeSignatureDenEditor_) {
...
void ProjectSettingsComponent::textEditorReturnKeyPressed(juce::TextEditor& editor)
{
...
void ProjectSettingsComponent::textEditorEscapeKeyPressed(juce::TextEditor& editor)
{
    // Revert on escape
    if (&editor == &projectNameEditor_) {
        projectNameEditor_.setText(projectState_.getProjectName(), false);
        ...
        timeSignatureNumEditor_.setText(juce::String(projectState_.getTimeSignatureNumerator()), false);
    } else if (&editor == &timeSignatureDenEditor_) {
        timeSignatureDenEditor_.setText(juce::String(projectState_.getTimeSignatureDenominator()), false);
    }
}
...
void ProjectSettingsComponent::timerCallback()
{
    ...
    tempoEditorFocusAnim_ += (tempoTarget - tempoEditorFocusAnim_) * animationSpeed;

    float numTarget = timeSignatureNumEditor_.hasKeyboardFocus(true) ? 1.0f : 0.0f;
    timeSignatureNumFocusAnim_ += (numTarget - timeSignatureNumFocusAnim_) * animationSpeed;

    float denTarget = timeSignatureDenEditor_.hasKeyboardFocus(true) ? 1.0f : 0.0f;
    timeSignatureDenFocusAnim_ += (denTarget - timeSignatureDenFocusAnim_) * animationSpeed;
...
void ProjectSettingsComponent::adjustTimeSignatureDen(int delta)
{
    ...
    int newPower = juce::jlimit(0, 5, currentPower + delta); // 2^0 to 2^5 (1 to 32)
...
void ProjectSettingsComponent::onTapTempo()
{
    ...
    if (tapTimes_.size() >= 2)
    {
        ...
        double averageInterval = static_cast<double>(totalInterval) / (tapTimes_.size() - 1);
