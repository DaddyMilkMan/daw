 ...
 * @brief GPU-accelerated button using PURE Skia rendering (no intermediate renderer)
 *
 * Features:
 ...
 */
class SkiaButtonNative : public juce::Component, public SkiaComponent, private juce::Timer
{
public:
    enum class Style
    {
    ...
    SkiaButtonNative(const juce::String& buttonText = {}, Style style = Style::Primary)
        ...
        , pressVelocity_(0.0f)
    {
        setOpaque(false);  // Parent handles rendering
        startTimer(16);    // 60 FPS animation
    }

    ~SkiaButtonNative() override
    {
        stopTimer();
    }
    ...
    //==========================================================================
    // juce::Component override - DISABLED
    //==========================================================================
    ...
    void paint(juce::Graphics& g) override
    {
        ...
        g.drawText(buttonText_, getLocalBounds(), juce::Justification::centred);
#else
        juce::ignoreUnused(g);
#endif
    }
    ...
    void paintToSkia(SkCanvas* canvas, SkRect bounds) override
    {
        ...
        switch (style_)
        {
            case Style::Primary:   baseColor = 0xFF0A84FF; break;  // Blue
            case Style::Secondary: baseColor = 0xFF636366; break;  // Gray
            case Style::Success:   baseColor = 0xFF34C759; break;  // Green
            case Style::Danger:    baseColor = 0xFFFF3B30; break;  // Red
            case Style::Warning:   baseColor = 0xFFFF9500; break;  // Orange
            default:               baseColor = 0xFF0A84FF; break;
        }
        ...
        if (pressProgress_ < 0.5f)
        {
            ...
            SkRRect shadowRRect = SkRRect::MakeRectXY(scaledBounds.makeOffset(0, 2), cornerRadius, cornerRadius);
        ...
        SkColor gradColors[2] = {
            SkColorSetARGB(SkColorGetA(baseColor), r, g, b),
            baseColor
        };
        ...
        SkRRect buttonRRect = SkRRect::MakeRectXY(scaledBounds, cornerRadius, cornerRadius);
        ...
        if (hoverProgress_ > 0.01f)
        {
            ...
            glowPaint.setColor(SkColorSetARGB(
                (uint8_t)(hoverProgress_ * 40),  // Max 40 alpha
                255, 255, 255
            ));
        ...
        if (!buttonText_.isEmpty())
        {
            ...
            font.measureText(textStr, buttonText_.getNumBytesAsUTF8(), SkTextEncoding::kUTF8, &textBounds);
...
private:
    ...
    void timerCallback() override
    {
        ...
        {
            float force = -stiffness * (pressProgress_ - pressTarget) - damping * pressVelocity_;
