...
SkiaButtonComponent_NEW::SkiaButtonComponent_NEW(const juce::String& buttonText, Style style)
...
SkiaButtonComponent_NEW::~SkiaButtonComponent_NEW()
{
    stopTimer();
}
...
void SkiaButtonComponent_NEW::paintSkia(SkCanvas* canvas)
{
#ifdef ZENITH_USE_SKIA
    ...
    SkRRect roundRect = SkRRect::MakeRectXY(buttonRect, CORNER_RADIUS, CORNER_RADIUS);
    ...
    if (pressProgress_ < 0.5f)
    {
        ...
        shadowPaint.setMaskFilter(SkMaskFilter::MakeBlur(
            SkBlurStyle::kNormal_SkBlurStyle, 6.0f));
    ...
    SkRRect highlightRRect = SkRRect::MakeRectXY(
        highlightRect, CORNER_RADIUS, CORNER_RADIUS / 2.0f);
...
void SkiaButtonComponent_NEW::paintFallback(juce::Graphics& g)
{
    ...
    if (isHovered_)
    {
        ...
        g.drawRoundedRectangle(bounds.toFloat(), CORNER_RADIUS, 2.0f);
    }

    // Text
    g.setColour(juce::Colours::white);
    g.setFont(14.0f);
    g.drawText(buttonText_, bounds, juce::Justification::centred);
}
...
juce::Colour SkiaButtonComponent_NEW::getStyleColour() const
{
    switch (style_)
    {
        case Style::Primary:   return juce::Colour(0xff0A84FF);
        case Style::Secondary: return juce::Colour(0xff3A3A3C);
        case Style::Success:   return juce::Colour(0xff34C759);
        case Style::Danger:    return juce::Colour(0xffFF3B30);
        case Style::Warning:   return juce::Colour(0xffFF9500);
...
void SkiaButtonComponent_NEW::timerCallback()
{
    updateAnimations();
}
