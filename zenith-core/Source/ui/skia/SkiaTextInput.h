 ...
 */
class SkiaTextInput : public juce::Component, public SkiaComponent, private juce::Timer
{
public:
    ...
    bool keyPressed(const juce::KeyPress& key) override
    {
        ...
        {
            text_ = text_.substring(0, caretPosition_ - 1) + text_.substring(caretPosition_);
            caretPosition_--;
            if (onTextChange) onTextChange(text_);
            repaint();
            return true;
        }
        else if (key == juce::KeyPress::deleteKey && caretPosition_ < text_.length())
        {
            text_ = text_.substring(0, caretPosition_) + text_.substring(caretPosition_ + 1);
            if (onTextChange) onTextChange(text_);
            repaint();
            return true;
        }
        ...
        else if (key == juce::KeyPress::rightKey && caretPosition_ < text_.length())
        {
            caretPosition_++;
            repaint();
            return true;
        ...
        {
            text_ = text_.substring(0, caretPosition_) + juce::String::charToString(key.getTextCharacter()) + text_.substring(caretPosition_);
            caretPosition_++;
            if (onTextChange) onTextChange(text_);
            repaint();
            return true;
    ...
    void paintToSkia(SkCanvas* canvas, SkRect bounds) override
    {
        ...
        textOpts.color = text_.isEmpty() ? colors.textSecondary : colors.textPrimary;

        float textX = bounds.x() + 8.0f;
        float textY = bounds.centerY() + 4.0f;

        textRenderer.drawText(canvas, displayText, textX, textY, TextStyle::Regular, textOpts);

        // Draw caret if focused
        if (isFocused_ && caretVisible_ && !isReadOnly_)
        {
            SkRect caretBounds = textRenderer.measureText(text_.substring(0, caretPosition_), TextStyle::Regular);
            float caretX = textX + caretBounds.width();

            SkPaint caretPaint;
            caretPaint.setColor(colors.primary);
            caretPaint.setStrokeWidth(2.0f);
            canvas->drawLine(caretX, bounds.y() + 6, caretX, bounds.bottom() - 6, caretPaint);
        }
    }
