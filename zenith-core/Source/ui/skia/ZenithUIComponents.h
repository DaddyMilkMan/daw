...
public:
  explicit ZenithVisualizer(ZenithPolySynthProcessor& p) : processor_(p) {
    startTimerHz(60);
    // Initialize lock-free atomic buffer (each sample is atomic for thread-safe access)
    displayBuffer_.resize(4096);
    for (auto& sample : displayBuffer_) {
  ...
  void drawSkia(SkCanvas * const canvas) override {
    ...
    for (int x = 0; x < numSamplesToDraw; ++x) {
      // Thread-safe read of each sample
      float sample = displayBuffer_[(readPtr + x) % bufferSize].load(std::memory_order_acquire);
  ...
  void timerCallback() override {
    ...
    if (numRead > 0) {
        ...
        for (int i = 0; i < numRead; ++i) {
            displayBuffer_[currentWritePtr].store(tempBuffer_[i], std::memory_order_release);
...
private:
  ...
  std::vector<std::atomic<float>> displayBuffer_;  // Lock-free atomic ring buffer
