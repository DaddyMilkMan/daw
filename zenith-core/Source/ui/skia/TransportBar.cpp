...
void TransportBar::drawSkia(SkCanvas* canvas) {
    ...
    canvas->drawString(tempoStr.toStdString().c_str(), 250, 35, font, paint);
    
    // Draw Project Name
    font.setSize(14.0f);
    paint.setColor(SkColorSetARGB(150, 255, 255, 255));
    canvas->drawString(projectName_.toStdString().c_str(), 350, 35, font, paint);
...
void TransportBar::drawMeter(SkCanvas* canvas, const juce::Rectangle<int>& bounds,
    ...
    font.setSize(12.0f);
    paint.setColor(SK_ColorWHITE);
    canvas->drawString(label, bounds.getX() + 5, bounds.getY() - 5, font, paint);
}
