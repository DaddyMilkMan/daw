 ...
 */
class SkiaListBox : public juce::Component, public SkiaComponent, private juce::Timer
{
public:
    /**
     ...
     */
    using ItemPaintCallback = std::function<void(SkCanvas* canvas, SkRect bounds, int rowNumber, bool isSelected, bool isHovered)>;
    ...
    void paintToSkia(SkCanvas* canvas, SkRect bounds) override
    {
        ...
        int lastVisibleRow = static_cast<int>((scrollOffset_ + bounds.height()) / rowHeight_) + 1;
        ...
        for (int row = firstVisibleRow; row < lastVisibleRow; ++row)
        {
            ...
            SkRect itemBounds = SkRect::MakeXYWH(bounds.x(), y, bounds.width(), rowHeight_);
            ...
            else
            {
                ...
                textRenderer.drawText(canvas, text, textX, textY, TextStyle::Regular, opts);
            }
            ...
            canvas->drawLine(itemBounds.x(), itemBounds.bottom(),
                           itemBounds.right(), itemBounds.bottom(), dividerPaint);
        }
        ...
        if (numRows_ * rowHeight_ > bounds.height())
        {
            ...
            float scrollbarY = bounds.y() + (scrollOffset_ / contentHeight) * bounds.height();

            SkRect scrollbarBounds = SkRect::MakeXYWH(
...
private:
    void timerCallback() override
    {
        ...
        {
            float force = -stiffness * (selectionAlpha_ - selectionTarget) - damping * selectionVelocity_;
