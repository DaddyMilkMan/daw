...
SkiaMainWindowIntegration::SkiaMainWindowIntegration()
  ...
  // DISABLED: We use software Skia renderer in MainComponent instead
  // The OpenGL path was conflicting with the JUCE-based paint() override
  /*
  ...
  */
  ...
  DBG("SkiaMainWindowIntegration: Initialized (OpenGL disabled)");
}
...
void SkiaMainWindowIntegration::renderOpenGL() {
  ...
  renderer_->render([this, scale](SkCanvas *canvas) {
    ...
    SkRect inspectorRect =
        SkRect::MakeXYWH(w - rightW, topBarH, rightW, h - topBarH);
    SkRect timelineRect =
        SkRect::MakeXYWH(sideW, topBarH, w - sideW - rightW, h - topBarH);
...
void SkiaMainWindowIntegration::drawTransportBar(SkCanvas *canvas,
                                                 const SkRect &bounds,
                                                 float scale) {
  SkPaint paint;
  ...
  canvas->drawString("ZENITH DAW", bounds.left() + 20, bounds.centerY() + 6,
                     font, paint);

  // Play Button (Center)
  SkRect playBtn =
      SkRect::MakeXYWH(bounds.centerX() - 40, bounds.centerY() - 15, 80, 30);
  SkRRect rrect;
  ...
  font.setSize(12.0f);
  canvas->drawString(isPlaying_ ? "PLAYING" : "STOPPED", playBtn.left() + 15,
                     playBtn.bottom() - 10, font, paint);

  // BPM Counter
  paint.setColor(SK_COLOR_TEXT_DIM);
  canvas->drawString("128.00 BPM", bounds.right() - 150, bounds.centerY() + 5,
                     font, paint);
}

void SkiaMainWindowIntegration::drawBrowserPanel(SkCanvas *canvas,
                                                 const SkRect &bounds,
                                                 float scale) {
  SkPaint paint;
  ...
  paint.setStrokeWidth(1.0f);
  canvas->drawLine(bounds.right(), bounds.top(), bounds.right(),
                   bounds.bottom(), paint);
  ...
  const char *items[] = {" Audio Files", " Instruments", " MIDI Effects",
                         " Audio Effects", " Presets"};

  for (int i = 0; i < 5; ++i) {
    paint.setColor(i == 1 ? SK_COLOR_ACCENT
                          : SK_COLOR_TEXT_MAIN); // Highlight 'Instruments'
    if (i == 1) {
      // Draw selection highlight
      SkPaint bgPaint;
      bgPaint.setColor(SkColorSetARGB(30, 0, 160, 255));
      canvas->drawRect(
          SkRect::MakeXYWH(bounds.left(), itemY - 15, bounds.width() - 1, 30),
          bgPaint);
    }
...
void SkiaMainWindowIntegration::drawTimelineArea(SkCanvas *canvas,
                                                 const SkRect &bounds,
                                                 float scale) {
  ...
  for (float y = bounds.top(); y < bounds.bottom(); y += trackHeight) {
    ...
    canvas->drawRect(SkRect::MakeXYWH(bounds.left(), y, 150, trackHeight),
                     headerPaint);
    ...
    canvas->drawString("Track Name", bounds.left() + 10,
                       y + trackHeight / 2 + 4, font, textPaint);
  }

  // 2. Draw a Sample Clip
  SkRect clipRect =
      SkRect::MakeXYWH(bounds.left() + 250, bounds.top() + 10, 200, 60);
  SkRRect clipRRect;
...
void SkiaMainWindowIntegration::drawPlayhead(SkCanvas *canvas,
                                             const SkRect &bounds,
                                             float scale) {
...
void SkiaMainWindowIntegration::drawInspectorPanel(SkCanvas *canvas,
                                                   const SkRect &bounds,
                                                   float scale) {
  SkPaint paint;
  ...
  paint.setColor(SK_COLOR_TEXT_MAIN);
  canvas->drawString("PROPERTIES", bounds.left() + 20, bounds.top() + 30, font,
                     paint);
