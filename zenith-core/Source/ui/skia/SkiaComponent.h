...
public:
    ...
    bool canRedo() const { return !history_.empty() && currentIndex_ < history_.size() - 1; }
    T undo() {
        if (canUndo()) {
            currentIndex_--;
...
class SkiaComponent : public juce::Component, public virtual juce::Timer, public juce::KeyListener {
public:
