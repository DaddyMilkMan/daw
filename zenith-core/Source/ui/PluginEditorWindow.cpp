...
PluginEditorWindow::PluginEditorWindow(
    juce::AudioPluginInstance *pluginInstance, bool useGenericEditor)
    : DocumentWindow(pluginInstance != nullptr ? pluginInstance->getName()
                                               : "Plugin",
  ...
  if (editor != nullptr) {
    ...
    DBG("PluginEditorWindow: ERROR - Failed to create editor for " +
        plugin->getName());
  }
}
...
PluginEditorWindowManager::~PluginEditorWindowManager() { closeAllEditors(); }

PluginEditorWindow *
PluginEditorWindowManager::openEditor(juce::AudioPluginInstance *plugin,
                                      bool useGenericEditor) {
  if (plugin == nullptr)
    return nullptr;
  ...
  // Set close callback to remove from manager (which deletes it)
  windowPtr->setOnCloseCallback([this, plugin] { closeEditor(plugin); });
...
bool PluginEditorWindowManager::hasOpenEditor(
...
PluginEditorWindow *PluginEditorWindowManager::getEditorWindow(
    juce::AudioPluginInstance *plugin) const {
  if (plugin == nullptr)
    return nullptr;
