...
FileMenuComponent::FileMenuComponent(ProjectState &state, Engine &eng)
  ...
  fileButton.setColour(
      juce::TextButton::buttonColourId,
      juce::Colour(ZenithLookAndFeel::Colors::accentAlt).withAlpha(0.8f));
  fileButton.setColour(juce::TextButton::buttonOnColourId,
                       juce::Colour(ZenithLookAndFeel::Colors::accentAlt));
  fileButton.setColour(juce::TextButton::textColourOnId,
                       juce::Colour(ZenithLookAndFeel::Colors::textOnAccent));
  fileButton.setColour(juce::TextButton::textColourOffId,
                       juce::Colour(ZenithLookAndFeel::Colors::textOnAccent));
...
void FileMenuComponent::paint(juce::Graphics &g) {
  ...
  // Background
  g.fillAll(juce::Colour(ZenithLookAndFeel::Colors::backgroundPanel));
  ...
  if (menuOpen_ && menuAlpha_ > 0.01f) {
    ...
    g.setColour(juce::Colour(ZenithLookAndFeel::Colors::backgroundDark)
                    .withAlpha(menuAlpha_));
    g.fillRoundedRectangle(menuBounds.toFloat().expanded(1.0f),
                           ZenithLookAndFeel::Radius::s);

    // Menu border
    g.setColour(
        juce::Colour(ZenithLookAndFeel::Colors::border).withAlpha(menuAlpha_));
    g.drawRoundedRectangle(menuBounds.toFloat().expanded(1.0f),
                           ZenithLookAndFeel::Radius::s, 1.0f);

    // Menu items
    int y = menuBounds.getY();

    // New Project
    newProjectBounds_ = juce::Rectangle<int>(
        menuBounds.getX(), y, menuBounds.getWidth(), MENU_ITEM_HEIGHT);
    paintMenuItem(g, "New Project", "Ctrl+N", 0, hoveredMenuItemIndex_ == 0);
    y += MENU_ITEM_HEIGHT;

    // Open Project
    openProjectBounds_ = juce::Rectangle<int>(
        menuBounds.getX(), y, menuBounds.getWidth(), MENU_ITEM_HEIGHT);
    ...
    // Separator
    g.setColour(
        juce::Colour(ZenithLookAndFeel::Colors::divider).withAlpha(menuAlpha_));
    g.drawHorizontalLine(y + SEPARATOR_HEIGHT / 2,
                         (float)menuBounds.getX() + 8.0f,
                         (float)menuBounds.getRight() - 8.0f);
    y += SEPARATOR_HEIGHT;

    // Save Project
    saveProjectBounds_ = juce::Rectangle<int>(
        menuBounds.getX(), y, menuBounds.getWidth(), MENU_ITEM_HEIGHT);
    paintMenuItem(g, "Save Project", "Ctrl+S", 2, hoveredMenuItemIndex_ == 2);
    y += MENU_ITEM_HEIGHT;

    // Save As
    saveAsBounds_ = juce::Rectangle<int>(
        menuBounds.getX(), y, menuBounds.getWidth(), MENU_ITEM_HEIGHT);
    ...
    if (!recentFiles_.isEmpty()) {
      g.setColour(juce::Colour(ZenithLookAndFeel::Colors::textSecondary)
                      .withAlpha(menuAlpha_ * 0.7f));
      g.setFont(juce::FontOptions(10.0f, juce::Font::bold));
      g.drawText("Recent Files",
                 juce::Rectangle<int>(menuBounds.getX() + 12, y,
      ...
      for (int i = 0; i < juce::jmin(recentFiles_.size(), 5); ++i) {
        ...
        if (isHovered) {
          g.setColour(juce::Colour(ZenithLookAndFeel::Colors::accentAlt)
                          .withAlpha(0.15f * menuAlpha_));
          g.fillRoundedRectangle(recentFileBounds_[i].reduced(4, 2).toFloat(),
                                 ZenithLookAndFeel::Radius::s);
        }
        ...
        juce::String filename =
            juce::File(recentFiles_[i]).getFileNameWithoutExtension();
...
void FileMenuComponent::paintMenuItem(juce::Graphics &g,
                                      const juce::String &label,
                                      const juce::String &shortcut, int index,
                                      bool isHovered) {
  juce::Rectangle<int> bounds;

  if (index == 0)
    bounds = newProjectBounds_;
  else if (index == 1)
    bounds = openProjectBounds_;
  else if (index == 2)
    bounds = saveProjectBounds_;
  else if (index == 3)
    bounds = saveAsBounds_;

  if (!bounds.isEmpty()) {
    // Background highlight
    if (isHovered) {
      g.setColour(juce::Colour(ZenithLookAndFeel::Colors::accentAlt)
                      .withAlpha(0.2f * menuAlpha_));
      g.fillRoundedRectangle(bounds.reduced(4, 2).toFloat(),
                             ZenithLookAndFeel::Radius::s);

      // Subtle border
      g.setColour(juce::Colour(ZenithLookAndFeel::Colors::accentAlt)
                      .withAlpha(0.4f * menuAlpha_));
      g.drawRoundedRectangle(bounds.reduced(4, 2).toFloat(),
                             ZenithLookAndFeel::Radius::s, 1.0f);
    }
    ...
    g.drawText(label,
               bounds.removeFromLeft(bounds.getWidth() - 80).reduced(12, 0),
               juce::Justification::centredLeft, false);
