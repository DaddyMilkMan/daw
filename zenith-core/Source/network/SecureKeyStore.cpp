...
bool SecureKeyStore::storeKeyWindows(const juce::String& keyName, const juce::String& keyValue)
{
    // Convert to UTF-8
    ...
    if (!result)
    {
        DBG("SecureKeyStore: Failed to encrypt key: " + keyName);
        return false;
    }
...
bool SecureKeyStore::retrieveKeyWindows(const juce::String& keyName, juce::String& outKey)
{
    ...
    if (!result)
    {
        DBG("SecureKeyStore: Failed to decrypt key: " + keyName);
        return false;
    }
    
    // Convert decrypted data to string
    outKey = juce::String::fromUTF8((const char*)dataOut.pbData, (int)dataOut.cbData);
...
bool SecureKeyStore::storeKeyMac(const juce::String& keyName, const juce::String& keyValue)
{
    ...
    {
        DBG("SecureKeyStore: Failed to store key in Keychain: " + juce::String(status));
        return false;
    }
...
bool SecureKeyStore::retrieveKeyMac(const juce::String& keyName, juce::String& outKey)
{
    auto serviceName = getServiceName();
    ...
    // Convert to string
    outKey = juce::String::fromUTF8((const char*)passwordData, (int)passwordLength);
...
bool SecureKeyStore::storeKeyLinux(const juce::String& keyName, const juce::String& keyValue)
{
    // Use JUCE's Blowfish encryption with machine-specific key
    auto machineID = juce::SystemStats::getComputerName() + juce::SystemStats::getLogonName();
    ...
    blowfish.encrypt((const uint32*)utf8Data.getAddress(), 
                     (uint32*)encrypted.getData(), 
                     (int)utf8Data.sizeInBytes());
...
bool SecureKeyStore::retrieveKeyLinux(const juce::String& keyName, juce::String& outKey)
{
    auto configDir = juce::File::getSpecialLocation(juce::File::userApplicationDataDirectory)
        .getChildFile(".zenith-daw")
        .getChildFile("keys");
    ...
    blowfish.decrypt((const uint32*)encrypted.getData(),
                     (uint32*)decrypted.getData(),
                     (int)encrypted.getSize());
    
    outKey = juce::String::fromUTF8((const char*)decrypted.getData(), (int)decrypted.getSize());
