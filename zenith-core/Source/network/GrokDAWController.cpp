...
public:
    ...
    */
    ...
    {
        juce::Array<GrokFunction> functions;
        ...
        functions.add(createFunctionDef(
            ...
            R"({
                ...
                "required": ["sourceClipId", "targetName"]
            })"
        ));
    ...
    */
        ...
        std::function<void(juce::String status)> onProgress)
    {
        ...
        juce::Thread::launch([this, call, onComplete, onError, onProgress]()
        {
            ...
            // TODO: Make Engine::exportProjectToWav thread-safe in future
            bool exportSuccess = false;
            
            juce::MessageManager::getInstance()->callFunctionOnMessageThread(
                [this, exportParams, &exportSuccess](void*) {
                    auto* cmdObj = new juce::DynamicObject();
                    cmdObj->setProperty("command", "export_audio");
                    cmdObj->setProperty("params", juce::var(exportParams));
    ...
    std::function<void(juce::String error)> onError)
{
    ...
    pImpl->grokClient.sendChat(
        ...
        [this, instrumentId, description, genre, onComplete, onError](juce::String response)
        {
            ...
            // Validate
            if (!PresetGenerator::validatePresetParameters(instrumentId, preset.parameters))
            {
                onError("Generated parameters failed validation");
                return;
            }
            
            // Success!
            onComplete(preset.name, preset.parameters);
        },
        [](GrokFunctionCall) {
