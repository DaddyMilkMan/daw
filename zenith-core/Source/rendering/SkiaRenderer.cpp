...
SkiaRenderer::SkiaRenderer(juce::Component& component,
                           Backend backend,
                           bool enableVSync)
...
void SkiaRenderer::render(std::function<void(SkCanvas*)> drawCallback)
{
    ...
    if (platformHandle_ && backend_ == Backend::Direct3D)
    {
        ...
        UINT presentFlags = (vsyncEnabled_ || targetFPS_ <= 60) ? 0 : DXGI_PRESENT_ALLOW_TEARING;
...
bool SkiaRenderer::createD3DContext()
{
    ...
    for (UINT i = 0; i < 2; i++)
    {
        hr = d3dCtx->swapChain->GetBuffer(i, IID_PPV_ARGS(&d3dCtx->renderTargets[i]));
        if (FAILED(hr))
        {
    ...
    hr = d3dCtx->device->CreateCommandAllocator(
        ...
        IID_PPV_ARGS(&d3dCtx->commandAllocator)
    );
    ...
    hr = d3dCtx->device->CreateCommandList(
        0,
        D3D12_COMMAND_LIST_TYPE_DIRECT,
        d3dCtx->commandAllocator.Get(),
        nullptr,
        IID_PPV_ARGS(&d3dCtx->commandList)
    );
    ...
    DBG("  Device: Yes");
    DBG("  Swap Chain: Yes (" << swapChainDesc.Width << "x" << swapChainDesc.Height << ")");
