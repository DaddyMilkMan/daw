...
bool SkiaContextManager::initialize(RenderMode preferredMode)
{
    ...
    DBG("SkiaContextManager: Initializing with preferred mode: " << (int)preferredMode);
    ...
    if (success)
    {
        ...
        DBG("SkiaContextManager: Initialized successfully with mode: " << (int)currentMode_);
    }
    else
    {
...
#else
    DBG("WARNING: OpenGL backend not available (SK_GL not defined or Skia disabled)");
    return false;
#endif
}
...
#else
    DBG("WARNING: Direct3D backend not available (SK_DIRECT3D not defined or not Windows)");
    return false;
#endif
}
...
#else
    DBG("WARNING: Metal backend not available (SK_METAL not defined or not macOS)");
    return false;
#endif
}
...
#else
    DBG("WARNING: Vulkan backend not available (SK_VULKAN not defined or not Linux)");
    return false;
#endif
}
...
sk_sp<SkSurface> SkiaContextManager::getOrCreateSurface(juce::Component& component)
{
#ifdef ZENITH_USE_SKIA
    std::lock_guard<std::mutex> lock(surfacesMutex_);
    ...
    else if (grContext_)
    {
        ...
        surface = SkSurfaces::RenderTarget(grContext_.get(),
                                          skgpu::Budgeted::kNo,
                                          info);
    }
    ...
    DBG("Created SkSurface for component: " << width << "x" << height
...
void SkiaContextManager::renderToComponent(juce::Component& component,
...
juce::Image SkiaContextManager::renderToImage(int width, int height,
                                             std::function<void(SkCanvas*)> drawCallback)
{
#ifdef ZENITH_USE_SKIA
    if (!initialized_)
...
void SkiaContextManager::componentResized(juce::Component& component)
{
    ...
    // Simply remove the surface - it will be recreated with new size on next render
    auto it = surfaces_.find(&component);
    if (it != surfaces_.end())
    {
        surfaces_.erase(it);
