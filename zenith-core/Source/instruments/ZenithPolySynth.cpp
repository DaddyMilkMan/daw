...
void ZenithEffects::process(float &left, float &right) {
  ...
  // 3. Reverb (Enhanced)
  if (reverbAmount_ > 0.01f) {
      if (!reverbInit_) initReverb();
      
      // Pre-delay (optional, but good for clarity)
      
      // Mono mix for reverb input
      float input = (left + right) * 0.5f * 0.015f; 
      ...
      for (int i = 0; i < 4; ++i) {
          float combOut = combs_[i].process(input);
          // Simple stereo spread: odd combs to left, even to right
          if (i % 2 == 0) outL += combOut;
          else            outR += combOut;
      }
      
      // Process allpasses in series for diffusion
      for (auto& allpass : allpasses_) {
          outL = allpass.process(outL);
          outR = allpass.process(outR); // Independent processing would be better for stereo, but sharing state for now
      }
      
      // Mix wet/dry
      left += outL * reverbAmount_;
      right += outR * reverbAmount_;
  }
}
...
void ZenithPolySynthProcessor::updateVoiceParameters() {
  ...
  for (int i = 0; i < getNumVoices(); ++i) {
    if (auto *voice = dynamic_cast<ZenithPolySynthVoice *>(getVoice(i))) {
      ...
      voice->setReverb(getVal("reverb_amount"));
      
      // Update modulation matrix
      for (int slot = 0; slot < 64; ++slot) {
          if (slot < 8) { // Voice only supports 8 slots currently
              const auto& mod = globalModMatrix_[slot];
              voice->setModulationSlot(slot, mod.source, mod.destination, mod.amount);
          }
      }
    }
  }
...
ZenithPolySynthProcessor::createParameterLayout() {
  ...
  layout.add(std::make_unique<juce::AudioParameterFloat>(NoiseLevel, "Noise Level",
                                                         0.0f, 1.0f, 0.0f));
  layout.add(std::make_unique<juce::AudioParameterFloat>(SubOscLevel, "Sub Osc Level",
                                                         0.0f, 1.0f, 0.0f));
  ...
  layout.add(std::make_unique<juce::AudioParameterFloat>(FilterDrive, "Drive",
                                                         1.0f, 5.0f, 1.0f));
  layout.add(std::make_unique<juce::AudioParameterFloat>(FilterEnvAmount, "Env Amount",
                                                         0.0f, 1.0f, 0.5f));

  // Envelopes - Use skewed ranges for natural feel
  auto timeRange = juce::NormalisableRange<float>(0.001f, 5.0f, 0.001f, 0.3f); // 0.3 skew factor

  layout.add(std::make_unique<juce::AudioParameterFloat>(
      AmpAttack, "Amp Attack", timeRange, 0.01f));
  layout.add(std::make_unique<juce::AudioParameterFloat>(AmpDecay, "Amp Decay",
                                                         timeRange, 0.1f));
  layout.add(std::make_unique<juce::AudioParameterFloat>(
      AmpSustain, "Amp Sustain", 0.0f, 1.0f, 0.7f));
  layout.add(std::make_unique<juce::AudioParameterFloat>(
      AmpRelease, "Amp Release", timeRange, 0.3f));

  layout.add(std::make_unique<juce::AudioParameterFloat>(
      ModAttack, "Mod Attack", timeRange, 0.01f));
  layout.add(std::make_unique<juce::AudioParameterFloat>(ModDecay, "Mod Decay",
                                                         timeRange, 0.1f));
  layout.add(std::make_unique<juce::AudioParameterFloat>(
      ModSustain, "Mod Sustain", 0.0f, 1.0f, 0.0f));
  layout.add(std::make_unique<juce::AudioParameterFloat>(
      ModRelease, "Mod Release", timeRange, 0.1f));
  ...
  layout.add(std::make_unique<juce::AudioParameterFloat>(
      "reverb_amount", "Reverb", 0.0f, 1.0f, 0.0f));
...
float ZenithPolySynthProcessor::getModulationMatrix(ModulationSource src, ModulationDestination dst) const {
    // Find slot with matching src/dst
...
int ZenithPolySynthProcessor::readFromVisualizer(float* buffer, int numSamples) {
    auto& fifo = visualizerFifo_;
    auto& bufferData = visualizerBuffer_;
    
    int start1, size1, start2, size2;
    fifo.prepareToRead(numSamples, start1, size1, start2, size2);
    
    if (size1 > 0) memcpy(buffer, bufferData.data() + start1, size1 * sizeof(float));
    if (size2 > 0) memcpy(buffer + size1, bufferData.data() + start2, size2 * sizeof(float));
...
void ZenithPolySynthProcessor::pushToVisualizer(const float* buffer, int numSamples) {
    ...
    if (size1 > 0) memcpy(bufferData.data() + start1, buffer, size1 * sizeof(float));
    if (size2 > 0) memcpy(bufferData.data() + start2, buffer + size1, size2 * sizeof(float));
