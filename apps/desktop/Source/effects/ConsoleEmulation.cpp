...
void ConsoleEmulation::prepare(juce::dsp::ProcessSpec &spec) {
  ...
  // Pre-calculate coefficients for all possible modes
  vintageCoeffs =
...
void ConsoleEmulation::updateFilterCoefficients() {
  ...
  // Swap to pre-calculated coefficients
  if (mode == Mode::Vintage) {
    if (vintageCoeffs)
      lowPass.coefficients = vintageCoeffs;
  } else {
    if (modernCoeffs)
      lowPass.coefficients = modernCoeffs;
  }
}
...
void ConsoleEmulation::process(juce::AudioBuffer<float> &buffer) {
  ...
  for (int ch = 0; ch < numChannels; ++ch) {
    ...
    for (int i = 0; i < numSamples; ++i) {
      ...
      // Applying saturation based on selected mode and drive intensity
