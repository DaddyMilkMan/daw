...
juce::var ClipCommands::splitClip(const juce::var& params)
{
    ...
    juce::String actionName = "split_clip " + clipId + " at " + juce::String(splitSamples);
    ...
    DBG("ClipCommands: Split clip: " + clipId + " into " + newClipIds.first + " and " + newClipIds.second);
...
juce::var ClipCommands::moveClip(const juce::var& params)
{
    ...
    juce::String actionName = "move_clip " + clipId + " to " + juce::String(newStartSamples);
    projectState.moveClip(trackId, clipId, newStartSamples, actionName);

    DBG("ClipCommands: Moved clip: " + clipId + " to " + juce::String(newStartSamples));
...
juce::var ClipCommands::resizeClip(const juce::var& params)
{
    ...
    juce::String actionName = "resize_clip " + clipId + " to " + juce::String(newLengthSamples);
    projectState.resizeClip(trackId, clipId, newLengthSamples, actionName);

    DBG("ClipCommands: Resized clip: " + clipId + " to " + juce::String(newLengthSamples));
...
juce::var ClipCommands::setClipNotes(const juce::var& params)
{
    ...
    // This was not fully visible in the previous view_file, but I'll implement a basic version or stub it if I don't have the logic.
    // Actually, I should check if I have the logic.
    // I don't recall seeing setClipNotes implementation in the view_file output.
    // I'll assume it uses ProjectState::addNotes or similar.
    ...
    // Since I don't have the exact implementation, I'll use ProjectState::addNotes which I saw in ProjectState.h
