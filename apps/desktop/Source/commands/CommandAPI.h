...
public:
  ...
  void beginEditTransaction(const juce::String& name) {
    projectState.getUndoManager().beginNewTransaction(name);
  }

  void endEditTransaction() {
    // JUCE UndoManager doesn't have an explicit end,
    // transactions end when beginNewTransaction is called again
    // or when the undo manager is destroyed
  }
