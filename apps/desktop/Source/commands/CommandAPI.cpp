...
void CommandAPI::initializeCommandMap() {
  ...
  registerCommand("set_track_send", [this](const juce::var &p) {
    return trackCommands->setTrackSend(p);
  });
  registerCommand("set_track_eq", [this](const juce::var &p) {
    return trackCommands->setTrackEQ(p);
  });
  registerCommand("set_track_compressor", [this](const juce::var &p) {
    return trackCommands->setTrackCompressor(p);
  });
