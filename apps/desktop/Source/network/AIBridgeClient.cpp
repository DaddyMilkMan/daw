...
//==============================================================================
AIBridgeClient::AIBridgeClient() : Thread("AIBridgeClient") {
...
juce::String AIBridgeClient::getServerUrl() const { return serverUrl; }
...
void AIBridgeClient::sendRequest(const juce::String &naturalLanguage,
  ...
  // Build request JSON - wrap DynamicObject in var immediately for proper
  // memory management Build request JSON - use DynamicObject::Ptr for safe
  // ref-counting
  juce::DynamicObject::Ptr requestObj = new juce::DynamicObject();
  juce::var requestVar(requestObj.get());
...
bool AIBridgeClient::isConnected() const { return connected.load(); }
...
void AIBridgeClient::handleResponse(const juce::String &requestId,
                                    const juce::String &responseJson) {
  // Parse JSON response
  ...
  if (parseResult.failed()) {
    ...
    errorResponse.status = "error";
    errorResponse.errorMessage =
        "Failed to parse JSON response: " + parseResult.getErrorMessage();
  ...
  DBG("AIBridgeClient: Response queued for " + requestId +
      " (status: " + response.status + ")");
}

AIBridgeClient::Response
AIBridgeClient::parseResponse(const juce::var &responseVar) {
  Response response;
