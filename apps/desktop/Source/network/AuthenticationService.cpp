...
juce::String AuthenticationService::calculateStretchedKey(const juce::String& password, const juce::String& username) {
    // Client-Side Key Stretching (Improved)
    // Offloads work from server (~100ms per login) to client
    // 100,000 rounds of SHA-256 with proper binary intermediate values

    // Create a proper salt combining username with a fixed pepper
    // This provides per-user salting while keeping it simple
    juce::String salt = username + "_zenith_pepper_v1";
    juce::String initialInput = password + salt;

    // Convert initial input to binary for first hash
    // Use binary hashing in the loop to avoid 100,000 string allocations
    std::vector<uint8_t> currentHash(32);  // SHA-256 produces 32 bytes
    std::vector<uint8_t> inputBuffer;

    // Initial hash
    juce::SHA256 initialHasher(initialInput.toRawUTF8(), initialInput.length());
    juce::String initialHex = initialHasher.toHexString();
    {
        const char* hexStr = initialHex.toRawUTF8();
        for (int j = 0; j < 32; ++j) {
            uint8_t byte = 0;
            for (int k = 0; k < 2; ++k) {
                char c = hexStr[j * 2 + k];
    ...
    // Perform key stretching with binary intermediate values (MUCH faster)
    constexpr int ROUNDS = 100000;
    for (int i = 0; i < ROUNDS; ++i) {
        // Hash the previous hash result (binary)
        juce::SHA256 hasher(reinterpret_cast<const char*>(currentHash.data()), 32);

        // Get binary result
        auto result = hasher.toHexString();

        // Convert hex string back to binary bytes
        // JUCE's toHexString() gives us 64 hex chars (32 bytes * 2)
        const char* hexStr = result.toRawUTF8();
        for (int j = 0; j < 32; ++j) {
    ...
    // Final conversion to hex string for transmission
    juce::String finalHash;
    for (int i = 0; i < 32; ++i) {
        finalHash += juce::String::toHexString(currentHash[i]).paddedLeft('0', 2);
    }
