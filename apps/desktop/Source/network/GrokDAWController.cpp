...
namespace {
juce::String buildChatStylePrompt(Settings::ChatStyle style) {
  switch (style) {
  case Settings::ChatStyle::Teacher:
    return "Response style: Teacher. Explain reasoning clearly, break complex "
           "tasks into steps, and include concise practical guidance.";
  case Settings::ChatStyle::Creative:
    return "Response style: Creative. Offer imaginative but useful ideas, "
           "present alternatives, and keep recommendations musically grounded.";
  case Settings::ChatStyle::Customized: {
    auto custom = Settings::getInstance().getCustomPersonaPrompt().trim();
    if (custom.isNotEmpty()) {
      return "Response style: Customized. Follow this persona exactly: " + custom;
    }
    return "Response style: Customized. No custom persona text provided yet. "
           "Default to concise, direct assistance.";
  }
  case Settings::ChatStyle::Obedient:
  default:
    return "Response style: Obedient. Follow the user's instructions exactly, "
           "prioritize direct execution, and keep responses concise.";
  }
}

juce::String buildReasoningPreviewPrompt(Settings::ReasoningPreview preview) {
  switch (preview) {
  case Settings::ReasoningPreview::Hidden:
    return "Reasoning visibility: Hidden. Do not include chain-of-thought or internal reasoning in the final response.";
  case Settings::ReasoningPreview::Detailed:
    return "Reasoning visibility: Detailed. Include a concise but explicit rationale before the final answer.";
  case Settings::ReasoningPreview::Compact:
  default:
    return "Reasoning visibility: Compact. Include a brief rationale only when it materially helps the user.";
  }
}

juce::String buildApprovalPrompt(Settings::WingmanApprovalMode mode) {
  switch (mode) {
  case Settings::WingmanApprovalMode::AutoApproveSafeChanges:
    return "Execution policy: YOLO. For routine low-risk DAW actions, execute directly. Ask only before destructive or ambiguous actions.";
  case Settings::WingmanApprovalMode::AskBeforeApplying:
  default:
    return "Execution policy: Ask. Request confirmation before applying DAW/project-changing actions.";
  }
}

juce::String buildResponseLengthPrompt(Settings::WingmanResponseLength mode) {
  switch (mode) {
  case Settings::WingmanResponseLength::Short:
    return "Response length: Short. Keep responses concise and tightly scoped.";
  case Settings::WingmanResponseLength::Long:
    return "Response length: Long. Provide fuller detail, options, and rationale when useful.";
  case Settings::WingmanResponseLength::Balanced:
  default:
    return "Response length: Balanced. Prefer concise responses with moderate supporting detail.";
  }
}
} // namespace
...
void GrokDAWController::executeCommand(
  ...
  systemPrompt += "\n\n" + buildChatStylePrompt(settings.getChatStyle()) +
                  "\n" + buildReasoningPreviewPrompt(settings.getReasoningPreview()) +
                  "\n" + buildApprovalPrompt(settings.getApprovalMode()) +
                  "\n" + buildResponseLengthPrompt(settings.getResponseLength());
