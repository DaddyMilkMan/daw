...
void AdvancedAudioAnalyzer::calculateSpectralFeatures() {
    ...
    float flux = 0.0f;
    if (lastMagnitudeBuffer.size() == magnitudeBuffer.size()) {
        for (size_t i = 0; i < magnitudeBuffer.size(); ++i) {
            float diff = magnitudeBuffer[i] - lastMagnitudeBuffer[i];
            // Hanning-style rectification (only positive changes) or standard?
            // Onset detection usually uses positive flux
            if (diff > 0.0f) flux += diff;
        }
    }
    spectrumAnalysis.spectralFlux = flux;
    
    // Store current magnitudes for next frame
    lastMagnitudeBuffer = magnitudeBuffer;
