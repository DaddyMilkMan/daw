/*
  ==============================================================================
    ...
    Implementation of transport controls for Zenith DAW's premium matte-black UI.
...
TransportBar::TransportBar() {
  ...
  svgicons::preload({svgicons::IconId::Play, svgicons::IconId::Stop,
                     svgicons::IconId::Record, svgicons::IconId::Settings,
                     svgicons::IconId::Wing, svgicons::IconId::ViewToggle,
                     svgicons::IconId::Folder});
...
void TransportBar::onAnimationTick(float deltaMs) {
  ...
  if (playState_.isAnimating() || stopState_.isAnimating() ||
      recordState_.isAnimating() || browserState_.isAnimating() ||
      viewToggleState_.isAnimating() ||
...
void TransportBar::resized() {
  ...
  // 3. Right Section: Tools (includes Wingman to map to right-docked panel)
  ...
  // Wingman AI (near settings on right cluster)
  wingmanButtonBounds_ = rightSection.removeFromRight(40).withSizeKeepingCentre(40, 40);
  rightSection.removeFromRight(14);  // Spacing
  ...
  // 4. Left Section: Browser + View toggle
  auto leftSection = area.removeFromLeft(200).reduced(16, 8);
  browserButtonBounds_ = leftSection.removeFromLeft(40).withSizeKeepingCentre(40, 40);
  leftSection.removeFromLeft(14);  // Spacing
...
void TransportBar::drawSkia(SkCanvas *canvas) {
  ...
  // 2. LCD Display (Center)
  {
      ...
      juce::String bpmStr =
          (activeEditTarget_ == EditTarget::BPM) ? editBuffer_
                                                 : juce::String(tempo_, 1);
      ...
      {
          std::string numStr =
              (activeEditTarget_ == EditTarget::TimeSigNum)
                  ? editBuffer_.toStdString()
                  : std::to_string(timeSigNum_);
      ...
      {
          std::string denStr =
              (activeEditTarget_ == EditTarget::TimeSigDen)
                  ? editBuffer_.toStdString()
                  : std::to_string(timeSigDen_);
  ...
  // 3. Transport Buttons
  // Play - Triangle
  drawTransportButton(canvas, playButtonBounds_, svgicons::IconId::Play, isPlaying_,
  ...
  drawTransportButton(canvas, stopButtonBounds_, svgicons::IconId::Stop, false,
                      design::colors::TEXT_SECONDARY, stopState_);
                      
  // Record - Circle
  drawTransportButton(canvas, recordButtonBounds_, svgicons::IconId::Record,
                      isRecording_ || recordState_.isHovered,
                      design::colors::RED, recordState_);


  drawTransportButton(canvas, settingsButtonBounds_, svgicons::IconId::Settings, false,
                      design::colors::TEXT_SECONDARY, settingsState_);


  // 5. Wingman AI - Angel wing icon
  // This toggles the Wingman AI panel for creative suggestions
  drawTransportButton(canvas, wingmanButtonBounds_, svgicons::IconId::Wing,
                      false, design::colors::NEON_PURPLE, wingmanState_,
                      false, true);

  // 6. Browser (Left)
  drawTransportButton(canvas, browserButtonBounds_, svgicons::IconId::Folder,
                      false, design::colors::TEXT_SECONDARY, browserState_);

  // 7. View Toggle (Left)
  drawTransportButton(canvas, viewToggleButtonBounds_, svgicons::IconId::ViewToggle,
...
void TransportBar::drawTransportButton(SkCanvas *canvas,
  ...
  style.filled = forceFilled ? true : (allowFill && isActive);
  ...
  if (iconId == svgicons::IconId::Wing) {
      iconSize = rect.width() * 0.72f;
  }
...
void TransportBar::mouseDown(const juce::MouseEvent &e) {
  ...
  if (timeSigTemplateBounds_.contains(e.getPosition())) {
      ...
      timeSigTemplatesMenu_ = std::make_unique<SkiaPopupMenu>();
      timeSigTemplatesMenu_->setBackdropBlurEnabled(false);

      auto addTemplate = [this](int n, int d, const juce::String& label, int id) {
        const bool ticked = (timeSigNum_ == n && timeSigDen_ == d);
        timeSigTemplatesMenu_->addItem(id, label, true, ticked, [this, n, d]() {
          setTimeSignature(n, d);
          if (onTimeSignatureChanged) onTimeSignatureChanged(n, d);
        });
      };

      addTemplate(4, 4, "4/4 (Common)", 1);
      addTemplate(3, 4, "3/4 (Waltz)", 2);
      addTemplate(6, 8, "6/8 (Compound)", 3);
      addTemplate(2, 4, "2/4 (March)", 4);
      addTemplate(5, 4, "5/4 (Take Five)", 5);
      addTemplate(7, 8, "7/8", 6);
      addTemplate(12, 8, "12/8 (Shuffle)", 7);
      addTemplate(9, 8, "9/8", 8);
      timeSigTemplatesMenu_->onDismiss = [this]() { timeSigTemplatesMenu_.reset(); };
      timeSigTemplatesMenu_->showAt(this, timeSigTemplateBounds_.getX(), timeSigTemplateBounds_.getBottom());
      return;
  }
  ...
  browserState_.isPressed = browserButtonBounds_.contains(e.getPosition()) && !isRightClick;
  ...
  } else if (browserButtonBounds_.contains(e.getPosition()) && !isRightClick) {
    if (onBrowserClicked) onBrowserClicked();
    requestRepaint();
...
void TransportBar::mouseDoubleClick(const juce::MouseEvent &e) {
    if (bpmHitBounds_.contains(e.getPosition())) {
        activeEditTarget_ = EditTarget::BPM;
        editBuffer_ = juce::String(tempo_, 1);
        grabKeyboardFocus();
        requestRepaint();
    } else if (timeSigHitBounds_.contains(e.getPosition())) {
        activeEditTarget_ = EditTarget::TimeSigNum;
        editBuffer_ = juce::String(timeSigNum_);
        grabKeyboardFocus();
        requestRepaint();
    }
}
...
bool TransportBar::keyPressed(const juce::KeyPress &key) {
  if (activeEditTarget_ == EditTarget::None) return false;

  if (key == juce::KeyPress::escapeKey) {
    activeEditTarget_ = EditTarget::None;
    editBuffer_.clear();
    requestRepaint();
    return true;
  }
  ...
  if (key == juce::KeyPress::returnKey) {
    const int value = editBuffer_.getIntValue();
    if (activeEditTarget_ == EditTarget::BPM) {
      const double newTempo = juce::jlimit(1.0, 2000.0, (double)value);
      setTempo(newTempo);
      if (onTempoChanged) onTempoChanged(newTempo);
      activeEditTarget_ = EditTarget::None;
    } else if (activeEditTarget_ == EditTarget::TimeSigNum) {
      const int newNum = juce::jlimit(1, 64, value);
      setTimeSignature(newNum, timeSigDen_);
      if (onTimeSignatureChanged) onTimeSignatureChanged(newNum, timeSigDen_);
      activeEditTarget_ = EditTarget::TimeSigDen;
      editBuffer_ = juce::String(timeSigDen_);
    } else if (activeEditTarget_ == EditTarget::TimeSigDen) {
      const int newDen = juce::jlimit(1, 64, value);
      setTimeSignature(timeSigNum_, newDen);
  ...
  if (key == juce::KeyPress::backspaceKey) {
    if (editBuffer_.isNotEmpty()) {
      editBuffer_ = editBuffer_.dropLastCharacters(1);
      requestRepaint();
    }
    return true;
  }
  ...
  if (c >= '0' && c <= '9') {
    editBuffer_ += juce::String::charToString(c);
    requestRepaint();
    return true;
  }
...
void TransportBar::mouseExit(const juce::MouseEvent &e) {
  ...
  bool wasHovered = playState_.isHovered || stopState_.isHovered || recordState_.isHovered ||
                    browserState_.isHovered || viewToggleState_.isHovered ||
                    wingmanState_.isHovered || settingsState_.isHovered;
                    
  playState_.isHovered = false;
  stopState_.isHovered = false;
  recordState_.isHovered = false;
  browserState_.isHovered = false;
...
void TransportBar::mouseMove(const juce::MouseEvent &e) {
  ...
  update(browserState_, browserButtonBounds_);
  ...
  if (globalHelpCallback) {
    ...
    else if (browserState_.isHovered)
        globalHelpCallback("Browser", "Open the library browser for instruments, sounds, effects, MIDI, presets, and projects.");
  ...
  if (bpmHitBounds_.contains(pos)) {
      ...
      setMouseCursor(juce::MouseCursor::PointingHandCursor);
             ...
             browserButtonBounds_.contains(pos) ||
...
bool TransportBar::hitTest(int x, int y) {
    ...
    if (browserButtonBounds_.contains(x, y)) return true;
