...
void MixerChannelComponent::InsertSlotIndicator::mouseDown(const juce::MouseEvent& e) {
  auto showPluginSelectionMenu = [this](bool replaceExisting) {
    auto menu = ContextMenuManager::createMenu();
    menu->addSectionHeader(replaceExisting ? "Replace With" : "Add Plugin");
    ...
    if (descriptions.isEmpty()) {
      menu->addItem(9000, "No plugins found", false, false, nullptr);
    ...
    for (int i = 0; i < maxItems; ++i) {
      ...
      menu->addItem(1000 + i, label, true, false, [this, track, desc, replaceExisting]() {
        if (track == nullptr) {
          return;
        }
    ...
    if (descriptions.size() > maxItems) {
      menu->addSeparator();
      menu->addItem(9100, "More plugins available in Browser panel", false, false, nullptr);
    }
