 ...
 *
 * Features:
 * - Vertical fader with precision grip handle
 ...
 * rendering with Zenith's premium matte-black design system and restrained accent lighting.
 */
...
private:
  ...
  public:
    struct Ballistics {
        ...
        void update(float input, float dtMs) {
            // Physics-based smoothing (300ms integration approx)
            const float attack = 1.0f - std::exp(-dtMs / 10.0f); // Fast attack
            const float release = 1.0f - std::exp(-dtMs / 300.0f); // Slow release (VU-style)
            
            if (input > current)
                current += (input - current) * attack;
            else
                current += (input - current) * release;
                
            // Peak hold logic
            if (current > peak) {
                peak = current;
                peakHoldMs = 1000.0f; // 1 second hold
            } else if (peakHoldMs > 0) {
                peakHoldMs -= dtMs;
            } else {
                peak *= std::pow(0.95f, dtMs / 16.0f); // Decay
            }
        }
    };
  ...
  private:
    ...
    Ballistics ballisticsL_;
    Ballistics ballisticsR_;
