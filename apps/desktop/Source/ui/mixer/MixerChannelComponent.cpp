...
void MixerChannelComponent::LevelMeter::onAnimationTick(float deltaMs) {
  ...
  if (stereo_) {
    ballisticsL_.update(targetLevelL_.load(), deltaMs);
    ballisticsR_.update(targetLevelR_.load(), deltaMs);
    repaint();
  } else {
    // For mono, we map main targetLevel_ to Left channel ballistics for simplicity
    ballisticsL_.update(targetLevel_.load(), deltaMs);
    repaint();
  }
}
...
void MixerChannelComponent::LevelMeter::drawSkia(SkCanvas *canvas) {
  ...
  if (stereo_) {
    ...
    drawMeterBar(canvas, leftBounds, ballisticsL_.current, ballisticsL_.peak);
    drawMeterBar(canvas, rightBounds, ballisticsR_.current, ballisticsR_.peak);
  } else {
    drawMeterBar(canvas, skBounds, ballisticsL_.current, ballisticsL_.peak);
  }
}
...
void MixerChannelComponent::LevelMeter::mouseDown(const juce::MouseEvent& e) {
  if (e.mods.isRightButtonDown()) {
    ...
    menu->addItem(1, "Reset Peak", true, false, [this]() {
        ballisticsL_.peak = 0.0f;
        ballisticsR_.peak = 0.0f;
        repaint();
    });
    ContextMenuManager::getInstance().showMenuAt(std::move(menu), this, e.x, e.y);
  }
...
void MixerChannelComponent::InsertSlotIndicator::mouseDown(const juce::MouseEvent& e) {
  auto showPluginSelectionMenu = [this](bool replaceExisting) {
    auto pluginMenu = ContextMenuManager::createMenu();
    pluginMenu->addSectionHeader(replaceExisting ? "Replace With" : "Add Plugin");

    auto* track = owner_.getTrack();
    auto& host = owner_.getEngine().getPluginHost();
    auto descriptions = host.getPluginDescriptions();

    if (descriptions.isEmpty()) {
      pluginMenu->addItem(9000, "No plugins found", false, false, nullptr);
      ContextMenuManager::getInstance().showMenuAtMouse(std::move(pluginMenu));
      return;
    }

    const int maxItems = juce::jmin(40, descriptions.size());
    for (int i = 0; i < maxItems; ++i) {
      const auto desc = descriptions[i];
      const juce::String label = desc.name + "  -  " + desc.manufacturerName;
      pluginMenu->addItem(1000 + i, label, true, false, [this, track, desc, replaceExisting]() {
        if (track == nullptr) {
          return;
        }

        if (auto plugin = owner_.getEngine().getPluginHost().createPlugin(desc)) {
          if (replaceExisting) {
    ...
    if (descriptions.size() > maxItems) {
      pluginMenu->addSeparator();
      pluginMenu->addItem(9100, "More plugins available in Browser panel", false, false, nullptr);
    }
  ...
  if (e.mods.isRightButtonDown()) {
    ...
    if (isOccupied_) {
      ...
      menu->addItem(3, "Replace Plugin...", true, false, [this, &showPluginSelectionMenu]() {
          showPluginSelectionMenu(true);
      });
      ...
      menu->addItem(1, "Add Plugin...", true, false, [this, &showPluginSelectionMenu]() {
          showPluginSelectionMenu(false);
      });
    }
