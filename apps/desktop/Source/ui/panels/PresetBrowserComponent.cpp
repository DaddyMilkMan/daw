...
void PresetBrowserComponent::saveCurrentPreset() {
  if (captureCallback) {
    ...
    auto* dialog = new zenith::SkiaAlertWindow(
        "Save Preset",
        "Enter a name for your preset:",
        zenith::SkiaAlertWindow::IconType::QuestionIcon);

    dialog->addTextEditor("presetName", preset.name, "Preset Name:");
    dialog->addButton("Save", zenith::SkiaAlertWindow::Result::Button1, zenith::SkiaButton::Style::Primary);
    dialog->addButton("Cancel", zenith::SkiaAlertWindow::Result::Cancelled, zenith::SkiaButton::Style::Secondary);
    dialog->showAsync([this, dialog, preset](zenith::SkiaAlertWindow::Result result) mutable {
      if (result == zenith::SkiaAlertWindow::Result::Button1) {
        auto updatedPreset = preset;
        updatedPreset.name = dialog->getTextEditorContents("presetName");
