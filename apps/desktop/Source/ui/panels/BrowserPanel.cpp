...
void BrowserPanel::loadWaveform(const juce::File &file) {
  ...
  for (int i = 0; i < numPoints; ++i) {
    ...
    buffer.clear();
    reader->read(&buffer, 0, static_cast<int>(samplesPerPoint), startSample,
                 true, false);
...
void BrowserPanel::drawSkia(SkCanvas *canvas) {
  ...
  // Filter bar
  filterBarBounds_ =
      juce::Rectangle<int>(0, y, bounds.getWidth(), filterBarHeight_);
  y += filterBarHeight_;

  // Progress bar (only visible when scanning)
  progressBarBounds_ =
      juce::Rectangle<int>(0, y, bounds.getWidth(), progressBarHeight_);
  y += progressBarHeight_;

  // Preview area (bottom)
  previewAreaBounds_ =
      juce::Rectangle<int>(0, bounds.getHeight() - previewHeight_,
...
void BrowserPanel::drawHeader(SkCanvas *canvas) {
  ...
  canvas->drawLine(0, headerHeight_ - 0.5f, getWidth(), headerHeight_ - 0.5f,
                   borderPaint);
  ...
  if (showBack) {
    ...
    SkFont titleFont = design::getSkFont(15.0f, design::FontWeight::Medium);

    SkPaint textPaint;
    textPaint.setColor(SK_ColorWHITE);
    textPaint.setAntiAlias(true);
    canvas->drawString(currentRoot_->name.toStdString().c_str(), 44, 28,
                       titleFont, textPaint);
  } else {
    ...
    searchBoxBounds_ =
        juce::Rectangle<int>(12, 9, getWidth() - 60, searchBoxHeight_ - 2);

    // Search box gradient background
    SkRect searchRect = SkRect::MakeXYWH(
        searchBoxBounds_.getX(), searchBoxBounds_.getY(),
        searchBoxBounds_.getWidth(), searchBoxBounds_.getHeight());

    SkPoint searchGradPoints[2] = {{0, searchRect.fTop},
                                   {0, searchRect.fBottom}};
    SkColor searchGradColors[2] = {SkColorSetRGB(42, 42, 48),
                                   SkColorSetRGB(35, 35, 40)};
    auto searchGradient = SkGradientShader::MakeLinear(
        searchGradPoints, searchGradColors, nullptr, 2, SkTileMode::kClamp);
    ...
    canvas->drawRoundRect(searchRect.makeInset(0.5f, 0.5f), 7.5f, 7.5f,
                          innerShadow);
    ...
    SkPaint textPaint;
    textPaint.setColor(searchText_.isEmpty() ? SkColorSetARGB(90, 255, 255, 255)
                                             : SkColorSetRGB(230, 230, 240));
    textPaint.setAntiAlias(true);

    juce::String displayText = searchText_.isEmpty()
                                   ? "Search samples, plugins, presets..."
                                   : searchText_;
    canvas->drawString(
        displayText.toStdString().c_str(), searchBoxBounds_.getX() + 32,
        searchBoxBounds_.getCentreY() + 4, searchFont, textPaint);

    // Add Folder button - premium style
    addFolderButtonBounds_ =
        juce::Rectangle<int>(getWidth() - 40, 9, 32, searchBoxHeight_ - 2);

    // Button gradient
    SkRect addBtnRect = SkRect::MakeXYWH(
        addFolderButtonBounds_.getX(), addFolderButtonBounds_.getY(),
    ...
    SkColor btnGradColors[2] = {SkColorSetRGB(55, 75, 55),
                                SkColorSetRGB(40, 60, 40)};
    auto btnGradient = SkGradientShader::MakeLinear(
        btnGradPoints, btnGradColors, nullptr, 2, SkTileMode::kClamp);
...
void BrowserPanel::drawProgressBar(SkCanvas *canvas) {
  ...
  auto gradient = SkGradientShader::MakeLinear(
      gradientPoints, gradientColors, positions, 3, SkTileMode::kClamp);
...
void BrowserPanel::drawItemList(SkCanvas *canvas) {
  ...
  canvas->clipRect(SkRect::MakeXYWH(
      listAreaBounds_.getX(), listAreaBounds_.getY(),
  ...
  int visibleEnd =
      juce::jmin(visibleStart + maxVisible + 1, (int)displayItems_.size());
  ...
  if (displayItems_.size() > maxVisible) {
    ...
    float scrollbarY = listAreaBounds_.getY() +
                       ((float)scrollOffset_ / (float)displayItems_.size()) *
                           listAreaBounds_.getHeight();

    SkPaint scrollPaint;
    scrollPaint.setColor(SkColorSetARGB(60, 255, 255, 255));
    canvas->drawRoundRect(
        SkRect::MakeXYWH(getWidth() - 6, scrollbarY, 4, scrollbarHeight), 2, 2,
        scrollPaint);
  }
...
void BrowserPanel::drawBrowserItem(SkCanvas *canvas, int index,
  ...
  if (!item->metadata.tags.empty()) {
    ...
    SkPaint chevronPaint;
    chevronPaint.setColor(index == hoverIndex_
  ...
  // Duration pill (if no tags, or alongside?)
    ...
    canvas->drawRoundRect(
        SkRect::MakeXYWH(w - 55, bounds.getCentreY() - 9, 45, 18), 9, 9,
        pillPaint);
    ...
    canvas->drawString(durStr.toStdString().c_str(), w - 48,
                       bounds.getCentreY() + 3, metaFont, metaPaint);
  }
}
...
void BrowserPanel::drawPreviewArea(SkCanvas *canvas) {
  ...
  canvas->drawRect(SkRect::MakeXYWH(previewAreaBounds_.getX(),
                                    ...
                                    previewAreaBounds_.getHeight()),
                   bgPaint);

  // Top border
  SkPaint borderPaint;
  borderPaint.setColor(SkColorSetRGB(50, 50, 55));
  canvas->drawLine(0, previewAreaBounds_.getY(), getWidth(),
                   previewAreaBounds_.getY(), borderPaint);
  ...
  // Play/Pause button - uses vector icons
  playButtonBounds_ = juce::Rectangle<int>(x, y, buttonSize, buttonSize);
  drawIconButton(canvas, playButtonBounds_,
                 previewEngine_.isPlaying() ? icons::Pause() : icons::Play(),
                 previewEngine_.isPlaying());
  x += buttonSize + 4;
  ...
  drawIconButton(canvas, loopButtonBounds_, icons::Loop(),
                 previewEngine_.isLooping());
  x += buttonSize + 8;

  // Auto-play toggle (still uses text since "AUTO" is self-explanatory)
  autoPlayButtonBounds_ =
      juce::Rectangle<int>(x, y, buttonSize + 20, buttonSize);
  drawButton(canvas, autoPlayButtonBounds_, "AUTO",
             previewEngine_.isAutoPlayEnabled(), false);

  // Waveform area
  waveformBounds_ = juce::Rectangle<int>(
      padding, y + buttonSize + 4, previewAreaBounds_.getWidth() - padding * 2,
      previewAreaBounds_.getHeight() - buttonSize - padding * 2 - 4);

  if (waveformBounds_.getHeight() > 10) {
    drawWaveform(canvas, SkRect::MakeXYWH(waveformBounds_.getX(),
                                          waveformBounds_.getY(),
                                          waveformBounds_.getWidth(),
                                          waveformBounds_.getHeight()));
  }

  // File name
  if (previewEngine_.isLoaded()) {
    SkFont nameFont = design::getSkFont(11.0f, design::FontWeight::Medium);
    SkPaint namePaint;
    namePaint.setColor(SK_ColorWHITE);
    namePaint.setAntiAlias(true);

    juce::String fileName =
        previewEngine_.getCurrentFile().getFileNameWithoutExtension();
    if (fileName.length() > 25)
      fileName = fileName.substring(0, 23) + "...";

    canvas->drawString(fileName.toStdString().c_str(),
                       autoPlayButtonBounds_.getRight() + 10, y + 18, nameFont,
                       namePaint);
  }
}
...
void BrowserPanel::drawWaveform(SkCanvas *canvas, const SkRect &bounds) {
  ...
  SkColor bgGradColors[2] = {SkColorSetRGB(15, 15, 20),
                             SkColorSetRGB(25, 25, 30)};
  ...
  if (waveformData_.empty()) {
    ...
    textPaint.setAntiAlias(true);
    canvas->drawString("Select audio to preview", bounds.centerX() - 65,
                       bounds.centerY() + 4, font, textPaint);
    ...
    for (int i = -3; i <= 3; ++i) {
      ...
      canvas->drawRoundRect(SkRect::MakeXYWH(bounds.centerX() + i * 8 - 2,
                                             bounds.centerY() - barHeight / 2,
                                             4, barHeight),
                            2, 2, iconPaint);
    }
    return;
  ...
  canvas->drawLine(bounds.fLeft + 4, centerY, bounds.fRight - 4, centerY,
                   centerLinePaint);

  // Playback position indicator (clean, no glow)
  if (previewEngine_.isLoaded()) {
    float posX =
        bounds.x() + previewEngine_.getPlaybackPosition() * bounds.width();
...
void BrowserPanel::drawIcon(SkCanvas *canvas, BrowserItemType type, float x,
                            float y, float size) {
  SkPaint iconPaint;
...
void BrowserPanel::drawButton(SkCanvas *canvas,
                              const juce::Rectangle<int> &bounds,
                              const juce::String &icon, bool active,
                              bool hovered) {
  SkPaint bgPaint;
  if (active)
  ...
  SkPaint textPaint;
  textPaint.setColor(active ? SK_ColorWHITE
                            : SkColorSetARGB(180, 255, 255, 255));
  textPaint.setAntiAlias(true);

  // Center text
  SkRect textBounds;
  font.measureText(icon.toStdString().c_str(), icon.length(),
...
void BrowserPanel::drawIconButton(SkCanvas *canvas,
                                  const juce::Rectangle<int> &bounds,
  ...
  // Calculate icon size (about 50% of button size for these smaller buttons)
  float iconSize = std::min(bounds.getWidth(), bounds.getHeight()) * 0.5f;

  // Set up icon style
  icons::IconStyle style;
  style.color = active ? SK_ColorWHITE : SkColorSetARGB(200, 255, 255, 255);
  style.filled = active;
  style.strokeWidth = 1.8f; // Slightly thinner for small buttons

  if (active) {
    style.glowRadius = 3.0f;
    style.glowColor = SkColorSetRGB(0, 200, 255);
  }

  // Draw icon centered
  SkRect skBounds = SkRect::MakeXYWH(bounds.getX(), bounds.getY(),
                                     bounds.getWidth(), bounds.getHeight());
  icons::drawIconCentered(canvas, iconPath, skBounds, iconSize, style);
}
...
void BrowserPanel::mouseDown(const juce::MouseEvent &e) {
  ...
  // Check back button
  if (backButtonBounds_.contains(e.getPosition()) &&
      !backButtonBounds_.isEmpty()) {
    navigateUp();
    return;
  }
  ...
  if (addFolderButtonBounds_.contains(e.getPosition()) &&
      !addFolderButtonBounds_.isEmpty()) {
    showAddFolderDialog();
    return;
  }
...
void BrowserPanel::mouseWheelMove(const juce::MouseEvent &e,
  ...
  int maxScroll = std::max(0, (int)displayItems_.size() -
                                  (listAreaBounds_.getHeight() / itemHeight_));
...
void BrowserPanel::showAddFolderDialog() {
  ...
  fileChooser_ = std::make_unique<juce::FileChooser>(
      ...
      juce::File::getSpecialLocation(juce::File::userMusicDirectory), "", true);
...
void BrowserPanel::drawFilterBar(SkCanvas *canvas) {
  ...
  filterAllBounds_ = juce::Rectangle<int>(
      x, static_cast<int>(y) + 4, tabWidth - 4, static_cast<int>(h) - 8);
  x += tabWidth;
  filterAudioBounds_ = juce::Rectangle<int>(
      x, static_cast<int>(y) + 4, tabWidth - 4, static_cast<int>(h) - 8);
  x += tabWidth;
  filterMidiBounds_ = juce::Rectangle<int>(
      x, static_cast<int>(y) + 4, tabWidth - 4, static_cast<int>(h) - 8);
  x += tabWidth;
  filterPluginBounds_ = juce::Rectangle<int>(
      x, static_cast<int>(y) + 4, tabWidth - 4, static_cast<int>(h) - 8);
...
void BrowserPanel::drawFilterTab(SkCanvas *canvas,
  ...
  if (active) {
    ...
    glowLine.setMaskFilter(SkMaskFilter::MakeBlur(SkBlurStyle::kNormal, 3.0f));
  ...
  // Center text
  SkRect textBounds;
  font.measureText(label.toStdString().c_str(), label.length(),
                   SkTextEncoding::kUTF8, &textBounds);
  float textX = bounds.getCentreX() - textBounds.width() / 2;
  float textY = bounds.getCentreY() + 4;

  canvas->drawString(label.toStdString().c_str(), textX, textY, font,
                     textPaint);
}
      ...
      [this, item](int result) {
        if (result == 1) {
          ...
          auto *alertWindow = new juce::AlertWindow(
              "Add Tag",
              "Enter a tag for this item:", juce::MessageBoxIconType::NoIcon);
          ...
          alertWindow->enterModalState(
              true, juce::ModalCallbackFunction::create(
                        [this, item, alertWindow](int result) {
                          if (result == 1) {
                            juce::String tag =
                                alertWindow->getTextEditorContents("tag");
                            if (tag.isNotEmpty()) {
