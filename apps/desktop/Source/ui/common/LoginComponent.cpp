...
LoginComponent::LoginComponent(LoginSuccessCallback onSuccess)
    ...
    auto setupField = [](SkiaTextEditor& field, const juce::String& placeholder, bool isPassword = false) {
        field.setMultiLine(false);
        field.setTextToShowWhenEmpty(placeholder, design::withAlpha(colors::TEXT_SECONDARY, 0.6f));
        field.setBackgroundColour(SkColorSetRGB(30, 30, 40));
        field.setTextColour(colors::TEXT_PRIMARY);
        field.setFont(design::getSkFont(16.0f, design::FontWeight::Regular));
        field.setPasswordMode(isPassword);
    };
    ...
    usernameField_.onReturnKey = [this]() { 
        if (mode_ == Mode::SignUp) emailField_.grabKeyboardFocus();
        else passwordField_.grabKeyboardFocus();
    };
