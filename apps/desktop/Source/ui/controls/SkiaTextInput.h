...
public:
  ...
  void setDrawChrome(bool shouldDrawChrome) { drawChrome_ = shouldDrawChrome; markDirty(); }
  void setPlaceholderColor(SkColor color) { placeholderColor_ = color; hasPlaceholderColorOverride_ = true; markDirty(); }
...
private:
  ...
  bool drawChrome_ = true;
  bool hasPlaceholderColorOverride_ = false;
  SkColor placeholderColor_ = SK_ColorTRANSPARENT;
  ...
  float getContentPadding() const {
    if (!drawChrome_) {
      return pillShape_ ? std::max(6.0f, getHeight() * 0.22f) : 8.0f;
    }
    return getPadding();
  }
