...
void SkiaTextEditor::setPasswordMode(bool enabled) {
...
void SkiaTextEditor::drawSkia(SkCanvas *canvas) {
  ...
  juce::String textToDraw = text_.isEmpty() ? placeholderText_ : text_;
  if (passwordMode_ && !text_.isEmpty()) {
    textToDraw = juce::String::repeatedString("\xE2\x80\xA2", text_.length());
  }
  ...
  if (isFocused() && caretVisible_ && !readOnly_) {
    ...
    if (!text_.isEmpty()) {
      juce::String measureText = text_.substring(0, caretPosition_);
      if (passwordMode_) {
        measureText = juce::String::repeatedString("\xE2\x80\xA2",
                                                   measureText.length());
      }
      caretX += font_.measureText(measureText.toRawUTF8(),
                                  measureText.length(),
