...
void SkiaTextInput::drawSkia(SkCanvas* canvas) {
  ...
  if (text_.isEmpty() && !isFocused_) {
    // Placeholder
    textPaint.setColor(hasPlaceholderColorOverride_ ? placeholderColor_ : design::colors::TEXT_TERTIARY);
  ...
  //==========================================================================
  if (drawChrome_ && scrollOffset_ > 0) {
    // Left fade
    SkPaint fadePaint;
  ...
  if (drawChrome_ && totalTextWidth_ > textAreaWidth_ + scrollOffset_) {
    // Right fade
    SkPaint fadePaint;
...
void SkiaTextInput::resized() {
  textAreaWidth_ = getWidth() - getContentPadding() * 2;
  ensureCursorVisible();
  markDirty();
}
...
int SkiaTextInput::getCharIndexAt(float localX) const {
  ...
  float adjustedX = localX + scrollOffset_ - getContentPadding();
