...
SkiaButton::SkiaButton(const juce::String &text) : text_(text) {
  ...
  juce::Component::setSize(
      static_cast<int>(design::dimensions::BUTTON_HEIGHT * 3),
...
void SkiaButton::setSize(Size size) {
  if (size_ != size) {
    ...
    switch (size) {
    case Size::Small:
      height = design::dimensions::BUTTON_HEIGHT_SM;
      break;
    case Size::Medium:
      height = design::dimensions::BUTTON_HEIGHT;
      break;
    case Size::Large:
      height = design::dimensions::BUTTON_HEIGHT_LG;
      break;
    }
...
void SkiaButton::setAudioReactive(bool reactive) { audioReactive_ = reactive; }
...
SkColor SkiaButton::getGlowColor() const {
  ...
  case Style::Success:
    return design::colors::NEON_GREEN;
  case Style::Ghost:
    return design::colors::CYAN;
  }
...
void SkiaButton::invalidateColors() { colorsDirty_ = true; }
...
void SkiaButton::updateTextBlob() {
  ...
  switch (size_) {
  case Size::Small:
    fontSize = design::typography::FONT_SM;
    break;
  case Size::Medium:
    fontSize = design::typography::FONT_MD;
    break;
  case Size::Large:
    fontSize = design::typography::FONT_LG;
    break;
  }

  SkFont font =
      zenith::design::getSkFont(fontSize, zenith::design::FontWeight::Medium);

  textBlob_ = SkTextBlob::MakeFromText(
      text_.toStdString().c_str(), text_.length(), font, SkTextEncoding::kUTF8);
...
void SkiaButton::calculateLayout() {
  ...
  if (icon_ && iconPosition_ == IconPosition::Left) {
    iconRect_ =
        SkRect::MakeXYWH(startX, centerY - iconSize / 2, iconSize, iconSize);
