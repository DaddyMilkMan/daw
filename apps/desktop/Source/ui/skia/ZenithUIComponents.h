...
protected:
  ...
  std::atomic<float> cachedValue_{
      0.0f};           // Thread-safe cached value (fixes race condition)
...
public:
  ...
  void drawSkia(SkCanvas *canvas) override {
    ...
    // Get target bounds relative to parent (assuming overlay is sibling or
    // covers parent) We need target's bounds relative to THIS component. If
    // both are children of ZenithPolySynthUI, and overlay is full size at
    // (0,0):
    ...
    paint.setMaskFilter(SkMaskFilter::MakeBlur((SkBlurStyle)0, 8.0f));
  ...
  // If overlay is visible, we might want to block interaction with other
  // controls? "Learning Mode" usually implies inspection. But if we want to
  // click through, we need `setInterceptsMouseClicks(false, false)`. But then
  // we can't hover? Wait, if overlay is on top, it will steal hover. So overlay
  // should be `hitTest` false? If `hitTest` is false, `mouseEnter` on
  // `ZenithControl` (underneath) will still fire. Yes. So
  // `ZenithTooltipOverlay` should be transparent to mouse events.
...
public:
  ...
  void setRange(double newMin, double newMax, double /*interval*/ = 0.0) {
    range_ = juce::NormalisableRange<float>(static_cast<float>(newMin),
                                            static_cast<float>(newMax));
  }
  ...
  void setLabel(const juce::String &newLabel) { name_ = newLabel; }
  ...
  void drawSkia(SkCanvas *canvas) override {
    ...
    if (parameter_)
      normValue = parameter_->getValue();
    else
      normValue = juce::jmap(value_, range_.start, range_.end, 0.0f, 1.0f);

    float sweepAngle = normValue * 270.0f;

    // Create gradient for the arc
    SkPoint pts[2] = {{bounds.getX(), bounds.getY()},
                      { bounds.getRight(),
                        bounds.getBottom() }};
    SkColor colors[2] = {SkColorSetRGB(0, 200, 255),
                         accentColor_}; // Cyan to Accent
    auto shader = SkGradientShader::MakeLinear(pts, colors, nullptr, 2,
                                               SkTileMode::kClamp);
    ...
    if (isHovered_ || normValue > 0.0f) {
      paint.setMaskFilter(
          SkMaskFilter::MakeBlur((SkBlurStyle)0, isHovered_ ? 6.0f : 3.0f));
    ...
    SkRect capRect = SkRect::MakeXYWH(cx - capRadius, cy - capRadius,
    ...
    shadowPaint.setMaskFilter(SkMaskFilter::MakeBlur((SkBlurStyle)0, 4.0f));
    ...
    paint.setStrokeWidth(2.0f);
    paint.setStrokeCap(SkPaint::kRound_Cap);
    canvas->drawLine(cx, cy - capRadius * 0.5f, cx, cy - capRadius + 4.0f,
                     paint);
    canvas->restore();
...
public:
  ...
  void drawSkia(SkCanvas *canvas) override {
    ...
    SkRect trackRect =
        SkRect::MakeXYWH(cx - trackWidth / 2, h * 0.1f, trackWidth, h * 0.8f);
    ...
    float handleW = 24.0f;
    float handleH = 12.0f;
    SkRect handleRect = SkRect::MakeXYWH(cx - handleW / 2, yPos - handleH / 2,
                                         handleW, handleH);
    ...
    shadowPaint.setAntiAlias(true);
    shadowPaint.setColor(SkColorSetARGB(100, 0, 0, 0));
    shadowPaint.setMaskFilter(SkMaskFilter::MakeBlur((SkBlurStyle)0, 3.0f));
    ...
    // canvas->drawRect(SkRect::MakeLTRB(cx - 1, yPos, cx + 1, h * 0.9f),
    // paint);
...
public:
  ...
  void drawSkia(SkCanvas *canvas) override {
    ...
    case Primary:
      baseColor = SkColorSetRGB(0, 100, 200);
      glowColor = SkColorSetRGB(0, 200, 255);
      break;
    case Danger:
      baseColor = SkColorSetRGB(150, 20, 20);
      glowColor = SkColorSetRGB(255, 50, 50);
      break;
    case Success:
      baseColor = SkColorSetRGB(20, 150, 50);
      glowColor = SkColorSetRGB(50, 255, 100);
      break;
    case Warning:
      baseColor = SkColorSetRGB(150, 120, 0);
      glowColor = SkColorSetRGB(255, 200, 0);
      break;
    case Secondary:
    default:
      break;
    ...
    SkColor bgColors[2] = {SkColorSetA(baseColor, isHovered_ ? 200 : 150),
    ...
    if (isHovered_) {
      ...
      glowPaint.setMaskFilter(SkMaskFilter::MakeBlur((SkBlurStyle)0, 6.0f));
  ...
  void mouseUp(const juce::MouseEvent &) override {
    ...
    repaint();
    if (onClick)
      onClick(); // Use public onClick
    else if (onClick_)
      onClick_(); // Fallback to private onClick_
  }
...
public:
    ...
    for (int r = 0; r < numRows; ++r) {
      for (int c = 0; c < numCols; ++c) {
        ...
        if (std::abs(val) > 0.01f) {
          SkColor color =
              val > 0 ? SkColorSetRGB(0, 255, 100) : SkColorSetRGB(255, 50, 50);
          ...
          // Glow
          paint.setMaskFilter(SkMaskFilter::MakeBlur((SkBlurStyle)0, 5.0f));
          canvas->drawCircle(cx, cy, radius * amount + 2.0f, paint);
          paint.setMaskFilter(nullptr);
        }
        ...
        if (hoverRow_ == r && hoverCol_ == c) {
          ...
          paint.setStyle(SkPaint::kStroke_Style);
          paint.setStrokeWidth(1.0f);
          canvas->drawRect(SkRect::MakeXYWH(x, y, cellWidth, cellHeight),
                           paint);
          paint.setStyle(SkPaint::kFill_Style);
        }
      }
  ...
  void mouseDrag(const juce::MouseEvent &e) override {
    if (isDragging_ && hoverRow_ >= 0 && hoverCol_ >= 0) {
      float diff =
          (lastMouseY_ - (float)e.position.y) * 0.01f; // Drag up to increase
  ...
  void mouseUp(const juce::MouseEvent &) override { isDragging_ = false; }
...
public:
  ...
  void drawSkia(SkCanvas *const canvas) override {
    ...
    int readPtr =
        (currentWritePtr - numSamplesToDraw + bufferSize) % bufferSize;
    ...
    if (displayMode_ == Stereo || displayMode_ == Mono) {
      ...
      for (int x = 0; x < numSamplesToDraw; ++x) {
        ...
        float y =
            (displayMode_ == Mono)
                ? cy - (sampleL + displayBufferR_[(readPtr + x) % bufferSize]) *
                           0.5f * (h * 0.4f)
                : cy - sampleL * (h * 0.4f);
        if (x == 0)
          pathL.moveTo((float)x, y);
        else
          pathL.lineTo((float)x, y);
      }
      ...
      if (displayMode_ == Stereo) {
        ...
        for (int x = 0; x < numSamplesToDraw; ++x) {
          ...
          if (x == 0)
            pathR.moveTo((float)x, y);
          else
            pathR.lineTo((float)x, y);
        }
  ...
  void timerCallback() override {
    ...
    // First try to read stereo interleaved data
    int numRead = 0;

    // Read left channel
    numRead = processor_.readFromVisualizer(tempBufferL_.data(),
                                            (int)tempBufferL_.size());

    // Try to read right channel if processor supports it
    // For now, synthesize stereo by slightly phase-shifting the mono signal
    // This gives visual differentiation until processor provides true stereo
    if (numRead > 0) {
      int bufferSize = (int)displayBufferL_.size();
      int currentWritePtr = writePtr_.load(std::memory_order_acquire);
      ...
      for (int i = 0; i < numRead; ++i) {
        ...
        // Synthesize stereo by using a phase-offset sample for R channel
        // This creates visual stereo separation for the oscilloscope
        int rOffset = (i + 8) % numRead; // 8 sample offset
        displayBufferR_[currentWritePtr] = tempBufferL_[rOffset];
...
public:
  ZenithPresetBar() {
    prevBtn_ = std::make_unique<ZenithButton>("<", [this]() {
      if (onPrev_)
        onPrev_();
    });
    nextBtn_ = std::make_unique<ZenithButton>(">", [this]() {
      if (onNext_)
        onNext_();
    });
  ...
  void setCallbacks(std::function<void()> onPrev, std::function<void()> onNext,
  ...
  void drawSkia(SkCanvas *canvas) override {
    ...
    paint.setStyle(SkPaint::kFill_Style);
    paint.setColor(SkColorSetRGB(0, 255, 255));
    paint.setMaskFilter(SkMaskFilter::MakeBlur((SkBlurStyle)0, 10.0f));
    canvas->drawSimpleText(str.c_str(), str.length(), SkTextEncoding::kUTF8,
