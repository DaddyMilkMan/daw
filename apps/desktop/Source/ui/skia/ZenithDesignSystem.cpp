...
void ThemeManager::loadTheme(const juce::String& name) {
    ...
    if (auto* obj = json.getDynamicObject()) {
        ...
        if (obj->hasProperty("CYAN")) colors::CYAN = (uint32_t)static_cast<int64_t>(obj->getProperty("CYAN"));
        if (obj->hasProperty("MAGENTA")) colors::MAGENTA = (uint32_t)static_cast<int64_t>(obj->getProperty("MAGENTA"));
        if (obj->hasProperty("NEON_GREEN")) colors::NEON_GREEN = (uint32_t)static_cast<int64_t>(obj->getProperty("NEON_GREEN"));
        if (obj->hasProperty("BG_DARKEST")) colors::BG_DARKEST = (uint32_t)static_cast<int64_t>(obj->getProperty("BG_DARKEST"));
        if (obj->hasProperty("BG_DARKER")) colors::BG_DARKER = (uint32_t)static_cast<int64_t>(obj->getProperty("BG_DARKER"));
        if (obj->hasProperty("BG_DARK")) colors::BG_DARK = (uint32_t)static_cast<int64_t>(obj->getProperty("BG_DARK"));
        
        // Trigger repaint globally (would need a listener, but for now relies on repaint calls)
    }
}
...
void LayoutManager::setPanelState(const juce::String& panelId, const PanelState& state) {
    panels_[panelId] = state;
}

LayoutManager::PanelState LayoutManager::getPanelState(const juce::String& panelId) const {
            ...
            for (auto& v : *panelArray) {
                if (auto* p = v.getDynamicObject()) {
                    ...
                    state.relativeBounds = juce::Rectangle<float>(
                        (float)p->getProperty("x"),
                        (float)p->getProperty("y"),
                        (float)p->getProperty("w"),
                        (float)p->getProperty("h")
                    );
