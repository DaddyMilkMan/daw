...
void TransportBar::drawSkia(SkCanvas* canvas) {
    ...
    canvas->drawLine(0.0f, skBounds.height(), skBounds.width(), skBounds.height(), borderPaint_);
    
    // 3. Draw buttons (Delegated helper - still allocates, needs future fix but acceptable for now)
    ...
    canvas->drawString(tempoStr.toStdString().c_str(), 260.0f, 38.0f, font_, textPaint);
    
    // Project Name (Subtle)
    textPaint.setColor(SkColorSetARGB(150, 255, 255, 255));
    canvas->drawString(projectName_.toStdString().c_str(), 380.0f, 37.0f, smallFont_, textPaint);
