...
void AudioSettingsTab::showDeviceSelector() {
    SkiaAlertWindow::showMessageBoxAsync(
        ...
        "The legacy JUCE audio-device popup is disabled.\n"
        "Use the in-panel backend/buffer controls and system audio settings.",
        "OK");
}
...
PluginSettingsTab::PluginSettingsTab(PluginHost& host) : host_(host) {
    ...
    clearBlacklistButton_->onClick = [this]() {
        ...
        SkiaAlertWindow::showMessageBoxAsync(
            SkiaAlertWindow::IconType::InfoIcon,
            "Blacklist Cleared",
            "The VST3 plugin crash blacklist has been cleared. Problematic plugins will be re-scanned on the next attempt.",
            "OK");
    };
...
PluginSettingsTab::~PluginSettingsTab() = default;

void PluginSettingsTab::resized() {
    scanButton_->setBounds(getWidth() - 140, getHeight() - 50, 120, 36);
    clearBlacklistButton_->setBounds(getWidth() - 280, getHeight() - 50, 130, 36);
    
    // Calculate max scroll offset based on contents
    float y = 110.0f; // Start of paths
    y += searchPaths_.size() * 38.0f; // Paths height
    y += 60.0f; // Space for Blacklist header
    y += currentBlacklist_.size() * 50.0f; // Blacklist height
    
    float visibleHeight = getHeight() - 70.0f; // Area above buttons
    maxScrollOffset_ = std::max(0.0f, y - visibleHeight + 20.0f);
    scrollOffset_ = juce::jlimit(0.0f, maxScrollOffset_, scrollOffset_);
}

void PluginSettingsTab::mouseWheelMove(const juce::MouseEvent& e, const juce::MouseWheelDetails& wheel) {
...
void PluginSettingsTab::drawSkia(SkCanvas* canvas) {
    ...
    SkFont headerFont = design::getSkFont(24.0f, design::FontWeight::SemiBold);
    SkFont sectionFont = design::getSkFont(16.0f, design::FontWeight::Medium);
    SkFont itemFont = design::getSkFont(13.0f, design::FontWeight::Regular);
    SkFont monoFont = design::getMonoFont(12.0f, design::FontWeight::Regular);

    // Fixed header area
    canvas->drawSimpleText("Plugin Management", 17, SkTextEncoding::kUTF8, 20.0f, 40.0f, headerFont, textPrimary);
    
    canvas->save();
    // Clip the scrollable area above the bottom buttons
    canvas->clipRect(SkRect::MakeXYWH(20.0f, 60.0f, getWidth() - 40.0f, getHeight() - 130.0f));
    canvas->translate(0.0f, -scrollOffset_);

    float y = 90.0f;
    canvas->drawSimpleText("VST3 Search Paths", 17, SkTextEncoding::kUTF8, 20.0f, y, sectionFont, textPrimary);
    y += 20.0f;

    SkPaint rowFill;
    rowFill.setAntiAlias(true);
    rowFill.setColor(SkColorSetRGB(22, 29, 45));
    ...
    for (const auto& path : searchPaths_) {
        SkRect rowRect = SkRect::MakeXYWH(20.0f, y, getWidth() - 40.0f, 32.0f);
        canvas->drawRoundRect(rowRect, 8.0f, 8.0f, rowFill);
        canvas->drawRoundRect(rowRect, 8.0f, 8.0f, rowStroke);
        
        canvas->drawSimpleText(path.toRawUTF8(), path.getNumBytesAsUTF8(), SkTextEncoding::kUTF8, 32.0f, y + 21.0f, monoFont, textSecondary);
        y += 38.0f;
    }
    
    if (searchPaths_.isEmpty()) {
        const char* msg = "No search paths configured.";
        canvas->drawSimpleText(msg, strlen(msg), SkTextEncoding::kUTF8, 20.0f, y + 20.0f, itemFont, textSecondary);
        y += 40.0f;
    }

    y += 20.0f;
    const char* blMsg = "Crash Recovery Blacklist";
    canvas->drawSimpleText(blMsg, strlen(blMsg), SkTextEncoding::kUTF8, 20.0f, y, sectionFont, textPrimary);
    y += 20.0f;

    if (currentBlacklist_.isEmpty()) {
        const char* msg = "No plugins are currently blacklisted.";
        canvas->drawSimpleText(msg, strlen(msg), SkTextEncoding::kUTF8, 20.0f, y + 20.0f, itemFont, textSecondary);
    } else {
        SkPaint errorText;
        errorText.setColor(design::colors::RED);
        errorText.setAntiAlias(true);

        for (const auto& entry : currentBlacklist_) {
            SkRect rowRect = SkRect::MakeXYWH(20.0f, y, getWidth() - 40.0f, 44.0f);
            canvas->drawRoundRect(rowRect, 8.0f, 8.0f, rowFill);
            canvas->drawRoundRect(rowRect, 8.0f, 8.0f, rowStroke);
            
            juce::File f(entry.path);
            juce::String name = f.getFileName();
            canvas->drawSimpleText(name.toRawUTF8(), name.getNumBytesAsUTF8(), SkTextEncoding::kUTF8, 32.0f, y + 18.0f, itemFont, textPrimary);
            
            juce::String details = entry.reason + "  •  " + entry.timestamp.formatted("%Y-%m-%d %H:%M:%S");
            canvas->drawSimpleText(details.toRawUTF8(), details.getNumBytesAsUTF8(), SkTextEncoding::kUTF8, 32.0f, y + 34.0f, monoFont, errorText);
...
void PluginSettingsTab::updateList() {
    searchPaths_ = host_.getSearchPaths();
    PluginBlacklistManager tempManager;
    currentBlacklist_ = tempManager.getEntries();
    resized();
    repaint();
}
...
AISettingsTab::AISettingsTab() {
    chatStyleLabel_ = std::make_unique<SkiaLabel>("Wingman Response Style");
    addAndMakeVisible(chatStyleLabel_.get());

    chatStyleSelector_ = std::make_unique<SkiaComboBox>("Response Style");
    chatStyleSelector_->addItem("Obedient (Default)", 1);
    chatStyleSelector_->addItem("Teacher", 2);
    chatStyleSelector_->addItem("Creative", 3);
    chatStyleSelector_->addItem("Customized", 4);
    chatStyleSelector_->setSelectedId((int)Settings::getInstance().getChatStyle() + 1, false);
    chatStyleSelector_->onChange = [this]() {
        Settings::getInstance().setChatStyle((Settings::ChatStyle)(chatStyleSelector_->getSelectedId() - 1));
    };
    addAndMakeVisible(chatStyleSelector_.get());

    statusLabel_ = std::make_unique<SkiaLabel>("API access is managed by Zenith. End users do not configure keys.");
    addAndMakeVisible(statusLabel_.get());
}

void AISettingsTab::resized() {
    const int contentWidth = juce::jmax(220, getWidth() - 40);
    const int selectorWidth = juce::jmin(340, contentWidth);

    chatStyleLabel_->setBounds(20, 80, contentWidth, 20);
    chatStyleSelector_->setBounds(20, 105, selectorWidth, 30);
    statusLabel_->setBounds(20, 145, contentWidth, 20);
}
...
void AISettingsTab::drawSkia(SkCanvas* canvas) {
    ...
    canvas->drawString("Wingman Style Configuration", 20, 70, labelFont, textPaint);
}
...
void SettingsComponent::showAITab() { setActiveTab(7); }

void SettingsComponent::showTab(int index) { setActiveTab(index); }
