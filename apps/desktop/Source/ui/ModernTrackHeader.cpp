      ...
      trackColor_(ZenithTheme::Colors::getTrackColor(trackIndex))
{
    ...
    // Setup name label
    nameLabel_.setText("Track " + juce::String(trackIndex + 1), juce::dontSendNotification);
    ...
    nameLabel_.setColour(juce::Label::backgroundWhenEditingColourId, Colors::bg_03);
    ...
    nameLabel_.setColour(juce::TextEditor::highlightColourId, Colors::accent_subtle);
    nameLabel_.setColour(juce::TextEditor::outlineColourId, Colors::border_default);
    nameLabel_.setColour(juce::TextEditor::focusedOutlineColourId, Colors::accent_primary);
    ...
    muteButton_.onClick = [this]() {
        ...
        if (onMuteToggled) onMuteToggled(isMuted_);
    };
    ...
    soloButton_.onClick = [this]() {
        ...
        if (onSoloToggled) onSoloToggled(isSoloed_);
    };
    ...
    armButton_.onClick = [this]() {
        ...
        if (onArmToggled) onArmToggled(isArmed_);
    };
...
void ModernTrackHeader::paint(juce::Graphics& g) {
    ...
    g.fillRect(bounds.withTop(bounds.getBottom() - 1.0f).withLeft(0.0f).withRight(bounds.getRight()));
...
void ModernTrackHeader::TrackButton::setButtonType(Type type) {
    type_ = type;
}

void ModernTrackHeader::TrackButton::paintButton(juce::Graphics& g, bool isHighlighted, bool isDown) {
    using namespace ZenithTheme;
