/*
  ==============================================================================
    ...
    Usage:
      ...
      NeonGlow::drawTextGlow(canvas, "Title", x, y, font, design::colors::MAGENTA);
      NeonGlow::drawActiveRing(canvas, center, radius, design::colors::CYAN);
...
public:
     ...
     */
    static void drawGlowOutline(SkCanvas* canvas, const SkRect& bounds, SkColor color,
                                Intensity intensity = Intensity::Medium,
     ...
     * @param animProgress Animation progress from 0.0 to 1.0 (use sin wave for pulse)
     */
    static void drawPulsingGlow(SkCanvas* canvas, const SkRect& bounds, SkColor color,
    ...
    static void drawVUMeterGlow(SkCanvas* canvas, const SkRect& bounds, 
        ...
        SkColor peakColor;
        if (value > 0.95f) {
            peakColor = colors::RED;      // Clipping
        } else if (value > 0.8f) {
            peakColor = colors::AMBER;    // Hot
        } else if (value > 0.5f) {
            peakColor = colors::NEON_GREEN; // Good
        } else {
            peakColor = colors::CYAN;     // Low
        }
        ...
        if (globalGlow > 0.01f) {
            ...
            if (vertical) {
                ...
                peakRect = SkRect::MakeXYWH(bounds.left(), peakY - 4.0f,
                                            bounds.width(), 8.0f);
            } else {
                float peakX = bounds.left() + (bounds.width() * value);
                peakRect = SkRect::MakeXYWH(peakX - 4.0f, bounds.top(),
                                            8.0f, bounds.height());
            }
     ...
     */
    static void drawPlayheadGlow(SkCanvas* canvas, float x, float top, float bottom,
