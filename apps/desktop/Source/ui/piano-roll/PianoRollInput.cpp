  ...
  if (e.mods.isRightButtonDown()) {
    ...
    if (noteUnderMouse) {
      ...
      menu->addItem(4, "Humanize...", true, false, [this]() { 
          auto* alert = new SkiaAlertWindow("Humanize", "Adjust randomization parameters:", SkiaAlertWindow::IconType::QuestionIcon);
          alert->addTextEditor("velocity", "10", "Velocity Range (+/-):");
          alert->addTextEditor("timing", "0.05", "Timing Range (beats):");
          alert->addButton("Apply", SkiaAlertWindow::Result::Button1, SkiaButton::Style::Primary);
          ...
          alert->showAsync([this, alert](SkiaAlertWindow::Result result) {
              if (result == SkiaAlertWindow::Result::Button1) {
                  const double velRange = alert->getTextEditorContents("velocity").getDoubleValue();
                  const double timeRange = alert->getTextEditorContents("timing").getDoubleValue();
