...
ArrangerTrackComponent::ArrangerTrackComponent(ProjectState &ps, TrackType type)
  ...
  vBlankAttachment_ = std::make_unique<juce::VBlankAttachment>(this, [this] {
    ...
    float diff = target - selectionGlow_;
    if (std::abs(diff) > 0.001f) {
      selectionGlow_ += diff * 0.2f;
      markDirty();
    }
  });
}
...
void ArrangerTrackComponent::drawSkia(SkCanvas *canvas) {
  ...
  SkRect rect = SkRect::MakeWH(static_cast<float>(bounds.getWidth()),
...
void ArrangerTrackComponent::drawTrackHeader(SkCanvas *canvas,
  ...
  if (selectionGlow_ > 0.01f) {
    SkPaint glowPaint;
    glowPaint.setColor(
        SkColorSetARGB(static_cast<int>(40 * selectionGlow_), 0, 255, 255));
...
void ArrangerTrackComponent::drawControls(SkCanvas *canvas, float startX,
  ...
  auto drawBtn = [&](int index, const char *label, bool active,
    ...
    bool isSectionHovered = (hoveredButtonIndex_ == index);
    ...
    shadowPaint.setMaskFilter(
        SkMaskFilter::MakeBlur(SkBlurStyle::kNormal, 2.0f));
    ...
    if (active) {
      ...
      glowPaint.setMaskFilter(
          SkMaskFilter::MakeBlur(SkBlurStyle::kNormal, 6.0f));
      ...
      SkPaint btnPaint;
      btnPaint.setAntiAlias(true);
      btnPaint.setColor(isSectionHovered ? SkColorSetARGB(30, 255, 255, 255)
...
void ArrangerTrackComponent::drawSections(SkCanvas *canvas,
  ...
  for (size_t idx = 0; idx < sections_.size(); ++idx) {
    ...
    if (w > 30) {
      ...
      textPaint.setAntiAlias(true);
      juce::String labelText = section.name;
      canvas->drawSimpleText(labelText.toRawUTF8(), labelText.length(),
...
void ArrangerTrackComponent::syncWithState() {
  if (trackId_.isEmpty())
    return;

  auto trackNode = projectState.getTrack(trackId_);
  if (!trackNode.isValid())
    return;

  setMuted(trackNode.getProperty(ProjectState::PROP_MUTE));
  setSoloed(trackNode.getProperty(ProjectState::PROP_SOLO));
  setRecordArmed(trackNode.getProperty(ProjectState::PROP_ARMED));
  setTrackName(trackNode.getProperty(ProjectState::PROP_NAME).toString());
}

void ArrangerTrackComponent::mouseDown(const juce::MouseEvent &e) {
  if (type_ == TrackType::Section) {
    double clickBeats =
        (static_cast<double>(e.position.x) / pixelsPerBeat_) + viewStartBeats_;
    ...
    if (e.position.x < HEADER_WIDTH && hoveredButtonIndex_ >= 0) {
      ...
      if (hoveredButtonIndex_ == 0) {
        ...
        if (trackId_.isNotEmpty()) {
          ...
          if (track.isValid()) {
            track.setProperty(ProjectState::PROP_MUTE, isMuted_,
        ...
        if (trackId_.isNotEmpty()) {
          auto track = projectState.getTrack(trackId_);
          if (track.isValid()) {
            track.setProperty(ProjectState::PROP_SOLO, isSoloed_,
                              &projectState.getUndoManager());
          }
        }
        ...
        if (trackId_.isNotEmpty()) {
          auto track = projectState.getTrack(trackId_);
          if (track.isValid()) {
            track.setProperty(ProjectState::PROP_ARMED, isRecordArmed_,
                              &projectState.getUndoManager());
          }
        }
      }
...
void ArrangerTrackComponent::mouseUp(const juce::MouseEvent &e) {
  if (type_ == TrackType::Section && draggingSectionIndex_ >= 0) {
    ...
    moveSection(draggingSectionIndex_,
                static_cast<double>(juce::roundDoubleToInt(finalStart)));
  }
  draggingSectionIndex_ = -1;
}
...
const ArrangementSection *ArrangerTrackComponent::getDraggingSection() const {
  if (draggingSectionIndex_ >= 0 &&
      draggingSectionIndex_ < static_cast<int>(sections_.size())) {
