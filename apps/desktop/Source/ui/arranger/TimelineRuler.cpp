...
void TimelineRuler::drawSkia(SkCanvas* canvas)
{
    ...
    for (int beat = startBeat; beat <= endBeat; ++beat)
    {
        ...
        if (beat % 4 == 0)
        {
            ...
            canvas->drawSimpleText(text.toRawUTF8(), text.length(), SkTextEncoding::kUTF8, x + 8, bounds.getHeight() / 2 + typo.small.size / 2, font, textPaint);
        }
        else
        {
    ...
    if (isHovered && hoverAnimation > 0.5f)
    {
        ...
        canvas->drawSimpleText(timeText.toRawUTF8(), timeText.length(), SkTextEncoding::kUTF8, textX, textY, tooltipFont, tooltipTextPaint);
    }
}
#else
