...
void MarkerLaneComponent::showRenameDialog(const juce::String& markerId)
{
    ...
    auto dialog = std::make_unique<SkiaAlertWindow>(
        "Rename Marker",
    ...
    // Find root parent to add to
    auto* root = getTopLevelComponent();
    if (root != nullptr) {
        int w = 400;
        int h = 200;
        dialog->setBounds((root->getWidth() - w) / 2, (root->getHeight() - h) / 2, w, h);
        
        // Transfer ownership to the root component or manage via member
        // In this implementation, we use showAsync which handles its own lifecycle if we wrap it properly.
        // But SkiaAlertWindow needs to be a child to be visible.
        
        auto* rawDialog = dialog.get();
        root->addAndMakeVisible(dialog.release());
        
        rawDialog->showAsync([this, markerId, rawDialog](SkiaAlertWindow::Result result) {
            if (result == SkiaAlertWindow::Result::Button1) {
                juce::String newName = rawDialog->getTextEditorContents("name");
                if (newName.isNotEmpty()) {
                    projectState.renameMarker(markerId, newName, "Rename Marker");
                }
            }
            // Auto-cleanup: remove from parent and delete
            if (auto* p = rawDialog->getParentComponent())
                p->removeChildComponent(rawDialog);
            
            // Note: Since we released the unique_ptr to the parent, 
            // the parent will delete it if it's a standard JUCE component management,
            // or we delete it here since we are the 'owner' of the raw pointer now.
            delete rawDialog;
        });
    }
}
