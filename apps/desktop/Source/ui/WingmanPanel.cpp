      ...
      aiBridgeClient(client)
{
    ...
    conversationDisplay->setColour(juce::TextEditor::backgroundColourId, ZenithLookAndFeel::Colors::panel);
    conversationDisplay->setColour(juce::TextEditor::textColourId, ZenithLookAndFeel::Colors::textPrimary);
    conversationDisplay->setColour(juce::TextEditor::outlineColourId, ZenithLookAndFeel::Colors::border);
    conversationDisplay->setFont(ZenithLookAndFeel::getFontMedium());
    addAndMakeVisible(conversationDisplay.get());
    
    // Welcome message
    appendToConversation("Wingman", "Hello! I'm Wingman, your AI assistant. I can control the DAW, generate presets, and help you create music. What would you like to do?");
    ...
    inputField->setColour(juce::TextEditor::backgroundColourId, ZenithLookAndFeel::Colors::backgroundPanel);
    inputField->setColour(juce::TextEditor::textColourId, ZenithLookAndFeel::Colors::textPrimary);
    inputField->setColour(juce::TextEditor::outlineColourId, ZenithLookAndFeel::Colors::accent);
    inputField->setFont(ZenithLookAndFeel::getFontMedium());
    inputField->setTextToShowWhenEmpty("Ask Wingman anything...", ZenithLookAndFeel::Colors::textSecondary);
    ...
    sendButton->setColour(juce::TextButton::buttonColourId, ZenithLookAndFeel::Colors::accent);
    sendButton->setColour(juce::TextButton::textColourOffId, ZenithLookAndFeel::Colors::textPrimary);
    ...
    modeLabel->setColour(juce::Label::textColourId, ZenithLookAndFeel::Colors::textPrimary);
    ...
    modeSelector->setColour(juce::ComboBox::backgroundColourId, ZenithLookAndFeel::Colors::backgroundPanel);
    modeSelector->setColour(juce::ComboBox::textColourId, ZenithLookAndFeel::Colors::textPrimary);
    modeSelector->setColour(juce::ComboBox::outlineColourId, ZenithLookAndFeel::Colors::accent);
    ...
    statusLabel->setColour(juce::Label::textColourId, juce::Colours::green); // Keep green for status
    ...
    clearButton->setColour(juce::TextButton::buttonColourId, ZenithLookAndFeel::Colors::panel);
    clearButton->setColour(juce::TextButton::textColourOffId, ZenithLookAndFeel::Colors::textSecondary);
    ...
    settingsButton->setColour(juce::TextButton::buttonColourId, ZenithLookAndFeel::Colors::panel);
    settingsButton->setColour(juce::TextButton::textColourOffId, ZenithLookAndFeel::Colors::textSecondary);
    settingsButton->addListener(this);
    addAndMakeVisible(settingsButton.get());
    
    // Update status based on Grok readiness
    if (isGrokReady())
    {
        setStatus("Grok Ready", juce::Colours::green);
    }
    else
    {
        setStatus("Grok API key not configured", juce::Colours::orange);
    }
}
...
void WingmanPanel::paint(juce::Graphics& g)
{
    ...
    g.drawText("Wingman AI Assistant", 10, 0, getWidth() - 20, 40, juce::Justification::centredLeft);
}
...
void WingmanPanel::buttonClicked(juce::Button* button)
{
    ...
    else if (button == clearButton.get())
    {
        ...
        appendToConversation("Wingman", "Conversation cleared. How can I help you?");
    }
...
void WingmanPanel::sendCommand()
{
    ...
    if (!isGrokReady())
    {
        appendToConversation("System", "Grok API key not configured. Click ⚙ to set it up.");
        return;
    }
    ...
    grokController->executeCommand(
        ...
        [this](juce::String status)
        {
            ...
            {
                setStatus(status, juce::Colour(0xffffff00));
            });
        }
    );
}

void WingmanPanel::appendToConversation(const juce::String& speaker, const juce::String& message)
{
    juce::String timestamp = juce::Time::getCurrentTime().toString(false, true, false, true);
    juce::String entry = "[" + timestamp + "] " + speaker + ": " + message + "\n\n";
