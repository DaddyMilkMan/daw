...
void ZenithEffects::process(float &left, float &right) {
    ...
    if (delayMix_ > 0.0f && !echoBufferL_.empty()) {
        float time = delaySync_ ? getDelaySecondsForSyncRate(delaySyncRate_, bpm_) : delayTime_;
        ...
        int rPos = (echoPos_ - static_cast<int>(delaySamples) + echoBufferL_.size()) % echoBufferL_.size();
    ...
    if (reverbAmount_ > 0.0f) {
        ...
        float combOut = 0.0f;
        for (auto& comb : combs_) combOut += comb.process(input);
        float allpassOut = combOut;
        for (auto& allpass : allpasses_) allpassOut = allpass.process(allpassOut);
