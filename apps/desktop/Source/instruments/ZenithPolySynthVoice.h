...
public:
  ...
  void setSubOscLevel(float level) { subOscLevel_ = juce::jlimit(0.0f, 1.0f, level); }
  void setSubOscOctave(int octave) { subOscOctave_ = juce::jlimit(-2, -1, octave); }
  void setNoiseLevel(float level) { noiseLevel_ = juce::jlimit(0.0f, 1.0f, level); }
  void setVelocityCurve(float curve) { velocityCurve_ = juce::jlimit(0.0f, 2.0f, curve); }
  void setMasterGain(float gain) { masterGain_.setTargetValue(juce::jlimit(0.0f, 2.0f, gain)); }
  ...
  void setLFO1Sync(bool sync, SyncRate rate, bool retr) { lfo1Sync_ = sync; lfo1SyncRate_ = rate; lfo1Retr_ = retr; }
  void setLFO2Sync(bool sync, SyncRate rate, bool retr) { lfo2Sync_ = sync; lfo2SyncRate_ = rate; lfo2Retr_ = retr; }
...
private:
  ...
  float filterEnvAmount_ = 0.5f;       // Filter envelope depth (-1 to +1 normalized to 0-1)
  ...
  float previousFrequency_ = 440.0f;   // For glide - preserves last note frequency
