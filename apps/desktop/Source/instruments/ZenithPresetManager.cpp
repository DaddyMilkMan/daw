...
juce::String Preset::generateId(const juce::String &name) {
  // Sanitize name: lowercase, replace spaces with underscores, keep
  // alphanumeric
  juce::String sanitized =
      name.toLowerCase().replaceCharacter(' ', '_').retainCharacters(
...
ZenithPresetManager::ZenithPresetManager() {
  ...
  factoryPresetsRoot_ =
      contentRoot.getChildFile("Instruments").getChildFile("Presets");
...
std::vector<Preset> ZenithPresetManager::loadAllPresetsForInstrument(
    const juce::String &instrumentId) {
  // THREAD SAFETY: This method performs file I/O and MUST be called from the
  // message thread
...
std::vector<PresetMetadata>
ZenithPresetManager::getPresetList(const juce::String &instrumentId,
  ...
  if (factoryDir.exists()) {
    ...
    metadataList.insert(metadataList.end(), factoryMeta.begin(),
                        factoryMeta.end());
  }
...
Preset ZenithPresetManager::loadPreset(const juce::String &instrumentId,
...
juce::StringArray
ZenithPresetManager::getCategories(const juce::String &instrumentId) {
  // THREAD SAFETY: This method performs file I/O and MUST be called from the
  // message thread
  jassert(juce::MessageManager::getInstance()->isThisTheMessageThread());
  ...
  if (factoryDir.exists()) {
    auto subdirs =
        factoryDir.findChildFiles(juce::File::findDirectories, false);
...
bool ZenithPresetManager::applyPresetToInstrument(const Preset &preset,
                                                  Instrument &instrument) {
  // THREAD SAFETY: This method modifies instrument parameters and MUST be
...
Preset ZenithPresetManager::capturePresetFromInstrument(
    Instrument &instrument, const juce::String &name,
    const juce::String &category, const juce::String &author) {
  // THREAD SAFETY: This method reads instrument parameters and MUST be called
...
bool ZenithPresetManager::savePreset(const Preset &preset, bool userPreset) {
  ...
  auto dir =
      getPresetsDirectory(preset.instrumentId, preset.category, userPreset);

  // Create directory if it doesn't exist
  if (!dir.exists()) {
    if (!dir.createDirectory()) {
      DBG("ZenithPresetManager: Failed to create directory: "
          << dir.getFullPathName());
      return false;
    }
  }
...
bool ZenithPresetManager::savePreset(const juce::String &instrumentId,
                                     const Preset &preset, bool userPreset) {
  // Create a copy with the instrumentId set
  Preset presetCopy = preset;
  if (presetCopy.instrumentId.isEmpty()) {
    presetCopy.instrumentId = instrumentId;
  }
  return savePreset(presetCopy, userPreset);
}
...
juce::File
ZenithPresetManager::getPresetsDirectory(const juce::String &instrumentId,
                                         const juce::String &category,
...
void ZenithPresetManager::setContentRoot(const juce::File &root) {
  ...
  factoryPresetsRoot_ =
      root.getChildFile("Instruments").getChildFile("Presets");
}
...
void ZenithPresetManager::registerSerializationHooks(
    const juce::String &instrumentId, ParameterSerializerFn serializer,
...
std::vector<Preset>
ZenithPresetManager::scanDirectory(const juce::File &directory,
                                   const juce::String &instrumentId) {
  std::vector<Preset> presets;
  ...
  auto files =
      directory.findChildFiles(juce::File::findFiles, true, "*.zpreset.json");
...
std::vector<PresetMetadata>
ZenithPresetManager::scanDirectoryMetadata(const juce::File &directory,
                                           const juce::String &instrumentId) {
  std::vector<PresetMetadata> metadataList;
...
PresetMetadata
ZenithPresetManager::loadMetadataFromFile(const juce::File &file) {
  PresetMetadata meta;
...
juce::File ZenithPresetManager::findPresetFile(const juce::String &instrumentId,
  ...
  auto files =
      baseDir.findChildFiles(juce::File::findFiles, true, "*.zpreset.json");
