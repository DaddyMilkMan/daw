...
struct Preset {
  ...
  juce::String
      category; ///< Category for browsing (e.g., "Pad", "Bass", "Lead")
  std::vector<juce::String>
      tags; ///< Search tags (e.g., ["warm", "lush", "ambient"])
  ...
  std::map<juce::String, float>
      parameters; ///< Parameter ID -> normalized value
   ...
   */
  Preset(const juce::String &name_, const juce::String &instrumentId_,
         const juce::String &category_ = "Uncategorized",
         const juce::String &author_ = "Factory")
      : name(name_), author(author_), category(category_),
        instrumentId(instrumentId_), version("1.0.0") {
...
//==============================================================================
/**
 * @brief Preset metadata (lightweight, for browsing without loading full
 * preset)
 */
...
public:
   ...
   */
  std::vector<Preset>
  loadAllPresetsForInstrument(const juce::String &instrumentId);
  ...
  getPresetList(const juce::String &instrumentId,
   ...
   * @note MESSAGE THREAD ONLY - performs file I/O
   */
  Preset loadPreset(const juce::String &instrumentId,
   ...
   * @param userPreset If true, save to user directory; if false, save to
   * factory directory
  ...
  /**
   * @brief Save preset to disk with explicit instrument ID
   * @param instrumentId Instrument identifier (used only to set
   * preset.instrumentId if not set)
   * @param preset Preset to save
   * @param userPreset If true, save to user directory; if false, save to
   * factory directory
   * @return true if successful
   * @note MESSAGE THREAD ONLY - performs file I/O
   */
  bool savePreset(const juce::String &instrumentId, const Preset &preset,
   ...
   * @param instrumentId Instrument identifier
   * @param presetId Preset unique ID
   * @param userPreset If true, delete from user directory; if false, from
   * factory
   * @return true if successful
   * @note MESSAGE THREAD ONLY - performs file I/O
   */
  bool deletePreset(const juce::String &instrumentId,
                    const juce::String &presetId, bool userPreset = true);
   ...
   */
  using ParameterSerializerFn =
      std::function<std::map<juce::String, float>(Instrument &)>;
   ...
   */
  using ParameterDeserializerFn =
      std::function<void(Instrument &, const std::map<juce::String, float> &)>;
...
private:
   ...
   */
  std::vector<PresetMetadata>
  scanDirectoryMetadata(const juce::File &directory,
                        const juce::String &instrumentId);
  ...
  juce::File findPresetFile(const juce::String &instrumentId,
                            const juce::String &presetId, bool userPreset);
