...
void ZenithPolySynthVoice::renderNextBlock(juce::AudioBuffer<float> &outputBuffer, 
    ...
    // (Optimization: call only if needed, but for safety call here or assume they are set)
    ...
    for (int i = 0; i < numSamples; ++i) {
        ...
        float pitchMod = std::exp2(pitchBend_ / 12.0f); // 12.0 for 1 octave range? Check range.
        // Pitchbend range usually +/- 2 semitones by default in many synths, but formula uses semitones.
        // If pitchBend_ is -1 to 1, then /12.0 is extremely small... 
        // Wait, MIDI pitch bend usually +/- 2 semitones *range*.
        // If pitchBend_ is normalized -1..1, then assuming range is 2 semitones:
        // pow(2, (bend * 2) / 12)
        // The original code was: std::pow(2.0, pitchBend_ / 12.0)
        // If pitchBend_ is -1..1, that's only +/- 1/12th of an octave, i.e. 1 semitone.
        // That seems acceptable as a default.
        ...
        // Oscillator 1
        float osc1Freq = freq * std::exp2(modulationState_.get(ModulationDestination::Osc1Pitch) / 12.0f);
        sample += osc1_.getNextSample(osc1Freq, oscShape_) * (osc1Mix_ + modulationState_.get(ModulationDestination::Osc1Mix));
        
        // Oscillator 2
        float osc2Freq = freq * std::exp2(modulationState_.get(ModulationDestination::Osc2Pitch) / 12.0f);
        sample += osc2_.getNextSample(osc2Freq, oscShape_) * (osc2Mix_ + modulationState_.get(ModulationDestination::Osc2Mix));
        
        // Oscillator 3
        float osc3Freq = freq * std::exp2(modulationState_.get(ModulationDestination::Osc3Pitch) / 12.0f);
        ...
        filter1_.setCutoff(filterCutoff_ * std::exp2(cutoffMod * 5.0f)); // 5 octaves range
        ...
        // Note: Effects are now per-voice in the original description OR per-patch in the processor?
        // Original code had effects_ inside Voice but commented "Moved to Processor" in my view?
        // Wait, in my previous reading of ZenithPolySynth.cpp lines 438: 
