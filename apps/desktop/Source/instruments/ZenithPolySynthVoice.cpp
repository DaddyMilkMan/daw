...
void ZenithPolySynthVoice::renderNextBlock(
  ...
  while (samplesProcessed < numSamples) {
    ...
    // Oversampling API requires using processSamplesUp to get the internal buffer
    // We use downBlock as dummy input
    auto internalBlock = oversampler_->processSamplesUp(validDownBlock);
    internalBlock.copyFrom(validUpBlock);
    ...
    if (oversamplingFactor_ > 1) {
      oversampler_ = std::make_unique<juce::dsp::Oversampling<float>>(
          ...
          juce::dsp::Oversampling<float>::FilterType::
              filterHalfBandPolyphaseIIR, // filter
          true                            // isBuffered
      );
    } else {
