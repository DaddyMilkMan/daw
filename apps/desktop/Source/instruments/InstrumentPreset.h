...
struct ZenithInstrumentPreset {
   ...
   * @param author_ Author name
   * @param id_ Optional existing ID. If empty, a new ID is generated.
   */
  ...
  static ZenithInstrumentPreset fromValueTree(const juce::ValueTree &tree) {
    ...
    // Optimization: Pass ID to constructor to avoid redundant generation
  ...
  static ZenithInstrumentPreset fromJson(const juce::var &json) {
    ...
    // Optimization: Pass ID to constructor
    ZenithInstrumentPreset preset(name, instrumentId, author, id);
