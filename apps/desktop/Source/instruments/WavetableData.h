  ...
  */
  void generateMipLevels() {
    ...
    for (int level = 1; level < WAVETABLE_MIP_LEVELS; ++level) {
      ...
      // FIX: Improved normalization logic
      if (maxVal > 1e-6f) {
...
inline int calculateMipLevel(float frequency, double sampleRate) {
  // FIX: Accurate MIP calculation using phase increment
  if (frequency <= 0.0f || sampleRate <= 0.0)
    return 0;

  // The phase increment determines how many table entries we step over per output sample.
  // A larger increment means higher frequency playback, which requires more anti-aliasing.
  const float phaseIncrement = frequency * (WAVETABLE_FRAME_SIZE / sampleRate);

  // The MIP level is the base-2 logarithm of the phase increment.
  // This effectively selects a pre-filtered table that matches the required bandwidth.
  const int level = static_cast<int>(std::log2(phaseIncrement));
