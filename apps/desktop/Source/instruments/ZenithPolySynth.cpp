...
const juce::String ZenithPolySynthProcessor::PitchBendRange = "pitch_bend_range";
...
void ZenithPolySynthProcessor::processBlock(juce::AudioBuffer<float> &buffer,
  ...
  if (auto* ph = getPlayHead()) {
     if (auto pos = ph->getPosition()) {
         if (pos->getBpm()) currentBpm_ = *pos->getBpm();
     }
  }
...
juce::AudioProcessorValueTreeState::ParameterLayout ZenithPolySynthProcessor::createParameterLayout() {
    ...
    juce::StringArray waves = {"Sine", "Saw", "Square", "Triangle", "Noise", "Supersaw", "Wavetable"};

    // =========================================================================
    // OSCILLATORS
    // =========================================================================
    params.push_back(std::make_unique<juce::AudioParameterChoice>(Osc1Wave, "Osc 1 Wave", waves, 1));
    params.push_back(std::make_unique<juce::AudioParameterFloat>(Osc1Detune, "Osc 1 Detune", -100.0f, 100.0f, 0.0f));
    params.push_back(std::make_unique<juce::AudioParameterFloat>(Osc1Mix, "Osc 1 Mix", 0.0f, 1.0f, 1.0f));
    params.push_back(std::make_unique<juce::AudioParameterFloat>(Osc1Shape, "Osc 1 Shape", 0.0f, 1.0f, 0.5f));
    
    params.push_back(std::make_unique<juce::AudioParameterChoice>(Osc2Wave, "Osc 2 Wave", waves, 1));
    params.push_back(std::make_unique<juce::AudioParameterFloat>(Osc2Detune, "Osc 2 Detune", -100.0f, 100.0f, 0.0f));
    params.push_back(std::make_unique<juce::AudioParameterFloat>(Osc2Mix, "Osc 2 Mix", 0.0f, 1.0f, 0.0f));
    params.push_back(std::make_unique<juce::AudioParameterFloat>(Osc2Shape, "Osc 2 Shape", 0.0f, 1.0f, 0.5f));

    params.push_back(std::make_unique<juce::AudioParameterChoice>(Osc3Wave, "Osc 3 Wave", waves, 0));
    params.push_back(std::make_unique<juce::AudioParameterFloat>(Osc3Detune, "Osc 3 Detune", -100.0f, 100.0f, 0.0f));
    params.push_back(std::make_unique<juce::AudioParameterFloat>(Osc3Mix, "Osc 3 Mix", 0.0f, 1.0f, 0.0f));
    params.push_back(std::make_unique<juce::AudioParameterFloat>(Osc3Shape, "Osc 3 Shape", 0.0f, 1.0f, 0.5f));

    // Sub-oscillator & Noise
    params.push_back(std::make_unique<juce::AudioParameterFloat>(SubOscLevel, "Sub Osc Level", 0.0f, 1.0f, 0.0f));
    params.push_back(std::make_unique<juce::AudioParameterChoice>(SubOscOctave, "Sub Osc Octave", juce::StringArray{"-2 Oct", "-1 Oct"}, 1));
    params.push_back(std::make_unique<juce::AudioParameterFloat>(NoiseLevel, "Noise Level", 0.0f, 1.0f, 0.0f));
    
    // Unison
    params.push_back(std::make_unique<juce::AudioParameterInt>(UnisonVoices, "Unison Voices", 1, 7, 1));
    params.push_back(std::make_unique<juce::AudioParameterFloat>(UnisonDetune, "Unison Detune", 0.0f, 100.0f, 10.0f));

    // Flagship Features
    params.push_back(std::make_unique<juce::AudioParameterBool>(Osc2Sync, "Sync 2->1", false));
    params.push_back(std::make_unique<juce::AudioParameterFloat>(Osc2FM, "FM 1->2", 0.0f, 1.0f, 0.0f));
    params.push_back(std::make_unique<juce::AudioParameterFloat>(RingMod, "Ring Mod", 0.0f, 1.0f, 0.0f));
    params.push_back(std::make_unique<juce::AudioParameterChoice>(FilterModel, "Filter Model", juce::StringArray{"State Variable", "Ladder"}, 0));
    ...
    params.push_back(std::make_unique<juce::AudioParameterFloat>(FilterCutoff, "Filter Cutoff", juce::NormalisableRange<float>(20.0f, 20000.0f, 1.0f, 0.3f), 20000.0f));
    params.push_back(std::make_unique<juce::AudioParameterFloat>(FilterResonance, "Filter Resonance", 0.0f, 1.0f, 0.0f));
    params.push_back(std::make_unique<juce::AudioParameterFloat>(FilterDrive, "Filter Drive", 1.0f, 10.0f, 1.0f));
    params.push_back(std::make_unique<juce::AudioParameterFloat>(FilterEnvAmount, "Filter Env Amount", 0.0f, 1.0f, 0.5f));  
    params.push_back(std::make_unique<juce::AudioParameterChoice>(FilterKeyTrack, "Filter Key Track", juce::StringArray{"Off", "50%", "100%"}, 0));

    // Envelopes
    params.push_back(std::make_unique<juce::AudioParameterFloat>(AmpAttack, "Amp Attack", 0.001f, 10.0f, 0.01f));
    params.push_back(std::make_unique<juce::AudioParameterFloat>(AmpDecay, "Amp Decay", 0.001f, 10.0f, 0.1f));
    params.push_back(std::make_unique<juce::AudioParameterFloat>(AmpSustain, "Amp Sustain", 0.0f, 1.0f, 1.0f));
    params.push_back(std::make_unique<juce::AudioParameterFloat>(AmpRelease, "Amp Release", 0.001f, 10.0f, 0.1f));

    params.push_back(std::make_unique<juce::AudioParameterFloat>(ModAttack, "Mod Attack", 0.001f, 10.0f, 0.01f));
    params.push_back(std::make_unique<juce::AudioParameterFloat>(ModDecay, "Mod Decay", 0.001f, 10.0f, 0.3f));
    params.push_back(std::make_unique<juce::AudioParameterFloat>(ModSustain, "Mod Sustain", 0.0f, 1.0f, 0.0f));
    params.push_back(std::make_unique<juce::AudioParameterFloat>(ModRelease, "Mod Release", 0.001f, 10.0f, 0.1f));

    // LFOs
    params.push_back(std::make_unique<juce::AudioParameterFloat>(LFO1Rate, "LFO 1 Rate", 0.01f, 50.0f, 1.0f));
    params.push_back(std::make_unique<juce::AudioParameterFloat>(LFO1Amount, "LFO 1 Amount", 0.0f, 1.0f, 0.0f));
    params.push_back(std::make_unique<juce::AudioParameterChoice>(LFO1Target, "LFO 1 Target", juce::StringArray{"Cutoff", "Osc1 Pitch", "Osc2 Pitch", "Osc1 Mix", "Osc2 Mix", "Amp", "Osc1 Shape"}, 0));
    params.push_back(std::make_unique<juce::AudioParameterChoice>(LFO1Waveform, "LFO 1 Wave", juce::StringArray{"Sine", "Triangle", "Saw", "Square", "S&H"}, 0));
    params.push_back(std::make_unique<juce::AudioParameterBool>(LFO1Sync, "LFO 1 Sync", false));
    params.push_back(std::make_unique<juce::AudioParameterChoice>(LFO1SyncRate, "LFO 1 Sync Rate", juce::StringArray{"1/64", "1/32", "1/16", "1/8", "1/4", "1/2", "1/1", "2/1", "4/1"}, 4));
    params.push_back(std::make_unique<juce::AudioParameterBool>(LFO1Retr, "LFO 1 Retr", true));

    params.push_back(std::make_unique<juce::AudioParameterFloat>(LFO2Rate, "LFO 2 Rate", 0.01f, 50.0f, 1.0f));
    params.push_back(std::make_unique<juce::AudioParameterFloat>(LFO2Amount, "LFO 2 Amount", 0.0f, 1.0f, 0.0f));
    params.push_back(std::make_unique<juce::AudioParameterChoice>(LFO2Target, "LFO 2 Target", juce::StringArray{"Cutoff", "Osc1 Pitch", "Osc2 Pitch", "Osc1 Mix", "Osc2 Mix", "Amp", "Osc1 Shape"}, 0));
    params.push_back(std::make_unique<juce::AudioParameterChoice>(LFO2Waveform, "LFO 2 Wave", juce::StringArray{"Sine", "Triangle", "Saw", "Square", "S&H"}, 0));
    params.push_back(std::make_unique<juce::AudioParameterBool>(LFO2Sync, "LFO 2 Sync", false));
    params.push_back(std::make_unique<juce::AudioParameterChoice>(LFO2SyncRate, "LFO 2 Sync Rate", juce::StringArray{"1/64", "1/32", "1/16", "1/8", "1/4", "1/2", "1/1", "2/1", "4/1"}, 4));
    params.push_back(std::make_unique<juce::AudioParameterBool>(LFO2Retr, "LFO 2 Retr", true));

    // Performance
    params.push_back(std::make_unique<juce::AudioParameterFloat>(GlideTime, "Glide Time", 0.0f, 2.0f, 0.0f));
    params.push_back(std::make_unique<juce::AudioParameterBool>(MonoMode, "Mono Mode", false));
    params.push_back(std::make_unique<juce::AudioParameterFloat>(MasterGain, "Master Gain", 0.0f, 2.0f, 0.8f));
    params.push_back(std::make_unique<juce::AudioParameterInt>(PitchBendRange, "Pitch Bend Range", 1, 24, 2));  
    params.push_back(std::make_unique<juce::AudioParameterFloat>(VelocityCurve, "Velocity Curve", 0.25f, 4.0f, 1.0f));  

    params.push_back(std::make_unique<juce::AudioParameterInt>(MaxVoices, "Max Voices", 1, 32, 16));
    params.push_back(std::make_unique<juce::AudioParameterChoice>(QualitySetting, "Quality", juce::StringArray{"Low", "Medium", "High"}, 1));

    params.push_back(std::make_unique<juce::AudioParameterFloat>(DistortionAmount, "Distortion", 0.0f, 1.0f, 0.0f));
    params.push_back(std::make_unique<juce::AudioParameterFloat>(ChorusAmount, "Chorus", 0.0f, 1.0f, 0.0f));
    params.push_back(std::make_unique<juce::AudioParameterFloat>(ReverbAmount, "Reverb", 0.0f, 1.0f, 0.0f));

    params.push_back(std::make_unique<juce::AudioParameterFloat>(DelayTime, "Delay Time", 0.01f, 2.0f, 0.5f));
    ...
    params.push_back(std::make_unique<juce::AudioParameterChoice>(DelaySyncRate, "Delay Rate", juce::StringArray{"1/64", "1/32", "1/16", "1/8", "1/4", "1/2", "1/1", "2/1", "4/1"}, 4));
...
void ZenithPolySynthProcessor::updateVoiceParameters() {
    ...
    int osc1Wave = static_cast<int>(parameters_.getRawParameterValue(Osc1Wave)->load());
    ...
    int osc2Wave = static_cast<int>(parameters_.getRawParameterValue(Osc2Wave)->load());
    ...
    int osc3Wave = static_cast<int>(parameters_.getRawParameterValue(Osc3Wave)->load());
    ...
    int subOscOctave = static_cast<int>(parameters_.getRawParameterValue(SubOscOctave)->load());
    
    int unisonVoices = static_cast<int>(parameters_.getRawParameterValue(UnisonVoices)->load());
    ...
    int filterModel = static_cast<int>(parameters_.getRawParameterValue(FilterModel)->load());
    
    int filterType = static_cast<int>(parameters_.getRawParameterValue(FilterType)->load());
    float filterCutoff = parameters_.getRawParameterValue(FilterCutoff)->load();
    float filterResonance = parameters_.getRawParameterValue(FilterResonance)->load();
    float filterDrive = parameters_.getRawParameterValue(FilterDrive)->load();
    float filterEnvAmount = parameters_.getRawParameterValue(FilterEnvAmount)->load();
    int filterKeyTrack = static_cast<int>(parameters_.getRawParameterValue(FilterKeyTrack)->load());
    ...
    int lfo1Target = static_cast<int>(parameters_.getRawParameterValue(LFO1Target)->load());
    int lfo1Waveform = static_cast<int>(parameters_.getRawParameterValue(LFO1Waveform)->load());
    bool lfo1Sync = parameters_.getRawParameterValue(LFO1Sync)->load() > 0.5f;
    int lfo1SyncIdx = static_cast<int>(parameters_.getRawParameterValue(LFO1SyncRate)->load());
    ...
    int lfo2Target = static_cast<int>(parameters_.getRawParameterValue(LFO2Target)->load());
    int lfo2Waveform = static_cast<int>(parameters_.getRawParameterValue(LFO2Waveform)->load());
    bool lfo2Sync = parameters_.getRawParameterValue(LFO2Sync)->load() > 0.5f;
    int lfo2SyncIdx = static_cast<int>(parameters_.getRawParameterValue(LFO2SyncRate)->load());
    ...
    int pitchBendRange = static_cast<int>(parameters_.getRawParameterValue(PitchBendRange)->load());
    float velocityCurve = parameters_.getRawParameterValue(VelocityCurve)->load();
    int qualitySetting = static_cast<int>(parameters_.getRawParameterValue(QualitySetting)->load());
    ...
    int delaySyncIdx = static_cast<int>(parameters_.getRawParameterValue(DelaySyncRate)->load());
    
    effects_.setDistortion(parameters_.getRawParameterValue(DistortionAmount)->load());
    ...
    for (int i = 0; i < synthesiser_.getNumVoices(); ++i) {
        if (auto* voice = dynamic_cast<ZenithPolySynthVoice*>(synthesiser_.getVoice(i))) {
            ...
            voice->setLFO1(lfo1Rate, lfo1Amount, static_cast<LFOTarget>(lfo1Target), static_cast<LFOWaveform>(lfo1Waveform));
            voice->setLFO1Sync(lfo1Sync, static_cast<SyncRate>(lfo1SyncIdx + 1), lfo1RetrVal);
            voice->setLFO2(lfo2Rate, lfo2Amount, static_cast<LFOTarget>(lfo2Target), static_cast<LFOWaveform>(lfo2Waveform));
            voice->setLFO2Sync(lfo2Sync, static_cast<SyncRate>(lfo2SyncIdx + 1), lfo2RetrVal);
...
float ZenithPolySynthProcessor::getModulationMatrix(ModulationSource src, ModulationDestination dst) const {
    for (const auto& slot : globalModMatrix_) {
        if (slot.source == src && slot.destination == dst) return slot.amount;
    }
    return 0.0f;
}
...
void ZenithPolySynthProcessor::setModulationMatrix(ModulationSource src, ModulationDestination dst, float amount) {
    for (auto& slot : globalModMatrix_) {
        if (slot.source == src && slot.destination == dst) { slot.amount = amount; return; }
    }
    for (auto& slot : globalModMatrix_) {
        if (slot.source == ModulationSource::None) { slot.source = src; slot.destination = dst; slot.amount = amount; return; }
    }
}
...
int ZenithPolySynthProcessor::readFromVisualizer(float* buffer, int numSamples) {
    ...
    if (numReadable > 0) {
        ...
        visualizerFifo_.prepareToRead(std::min(numReadable, numSamples), start1, size1, start2, size2);
        if (size1 > 0) memcpy(buffer, visualizerBuffer_.data() + start1, size1 * sizeof(float));
        if (size2 > 0) memcpy(buffer + size1, visualizerBuffer_.data() + start2, size2 * sizeof(float));
...
void ZenithPolySynthProcessor::pushToVisualizer(const float* buffer, int numSamples) {
    ...
    if (size1 > 0) memcpy(visualizerBuffer_.data() + start1, buffer, size1 * sizeof(float));
    if (size2 > 0) memcpy(visualizerBuffer_.data() + start2, buffer + size1, size2 * sizeof(float));
    visualizerFifo_.finishedWrite(size1 + size2);
}

void ZenithPolySynthProcessor::updateVoiceCount() {
    int targetVoices = static_cast<int>(parameters_.getRawParameterValue(MaxVoices)->load());
    if (targetVoices != currentMaxVoices_) {
        while (synthesiser_.getNumVoices() > targetVoices) synthesiser_.removeVoice(synthesiser_.getNumVoices() - 1);
        while (synthesiser_.getNumVoices() < targetVoices) synthesiser_.addVoice(new ZenithPolySynthVoice());
