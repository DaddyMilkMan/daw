...
ZenithSamplerEditor::ZenithSamplerEditor(ZenithSamplerProcessor &proc,
    ...
    : juce::AudioProcessorEditor(&proc), sampler(proc), instrument_(instrument),
  ...
  presetBrowser_->setLoadPresetCallback(
      [this](const Preset &preset) { onPresetLoaded(preset); });
  ...
  sampleMapTable_.setColour(juce::ListBox::backgroundColourId,
  ...
  // presetLabel.setJustification(SkiaLabel::Justification::Center); // Default
  // is left/center?
  ...
  auto setupSlider = [this](ZenithKnob &knob, SkiaLabel &label,
  ...
  auto addAttachment = [&](const juce::String &paramId,
...
ZenithSamplerEditor::~ZenithSamplerEditor() { stopTimer(); }
...
void ZenithSamplerEditor::paint(juce::Graphics &g) {
  ...
  g.drawText("Zenith Sampler", 0, 10, getWidth(), 30,
...
void ZenithSamplerEditor::resized() {
  ...
  attackSlider.setBounds(
      envContent.removeFromLeft(knobSize + 20).removeFromTop(knobSize + 30));
  ...
  filterCutoffSlider.setBounds(
      filterContent.removeFromLeft(knobSize + 20).removeFromTop(knobSize + 30));
  filterContent.removeFromLeft(5);
  filterResonanceSlider.setBounds(
      filterContent.removeFromLeft(knobSize + 20).removeFromTop(knobSize + 30));
  ...
  tuneSlider.setBounds(
      globalContent.removeFromLeft(knobSize + 20).removeFromTop(knobSize + 30));
...
void ZenithSamplerEditor::onPatchSelected() {
  ...
  if (selectedId > 0) {
    // With SkiaComboBox, we can just get the current text which corresponds to
    // the selection
...
void ZenithSamplerEditor::loadSampleMapData() {
  ...
  if (numSounds == 0) {
    ...
    for (int i = 0; i < numSounds; ++i) {
      if (auto *sound =
              dynamic_cast<ZenithSamplerSound *>(synth.getSound(i).get())) {
        SampleInfo info;
...
ZenithSamplerEditor::SampleMapTableModel::SampleMapTableModel(
    ZenithSamplerEditor &owner)
    : owner_(owner) {}
...
void ZenithSamplerEditor::SampleMapTableModel::paintRowBackground(
    juce::Graphics &g, int rowNumber, int width, int height,
    bool rowIsSelected) {
  if (rowIsSelected)
...
void ZenithSamplerEditor::SampleMapTableModel::paintCell(juce::Graphics &g,
                                                         int rowNumber,
                                                         int columnId,
  ...
  switch (columnId) {
  case 1: // Filename
    g.drawText(sample.fileName, 4, 0, width - 8, height,
               juce::Justification::centredLeft, true);
    break;
  case 2: // Low Key
    g.drawText(juce::String(sample.lowKey), 4, 0, width - 8, height,
               juce::Justification::centred, true);
    break;
  case 3: // High Key
    g.drawText(juce::String(sample.highKey), 4, 0, width - 8, height,
               juce::Justification::centred, true);
    break;
  case 4: // Root Note
    g.drawText(juce::String(sample.rootNote), 4, 0, width - 8, height,
               juce::Justification::centred, true);
    break;
  }
}
