...
WavetableLoader::loadBundledWavetable(const juce::String &name) {
  ...
  if (name.startsWith("Basic ")) {
    // FIX: Using std::map for cleaner dispatch
    static const std::map<juce::String, int> typeMap = {
        {"Sine", 0}, {"Saw", 1},       {"Square", 2},
        {"Triangle", 3}, {"PWM", 4}, {"Formant", 5}};
    ...
    if (it != typeMap.end()) {
      int type = it->second;
      auto wt =
          generateBasicWavetable(type, (type == 4 || type == 5) ? 64 : 1);
