...
ONNXStemSeparator::ONNXStemSeparator() : pImpl(std::make_unique<Impl>()) {
    ...
    try {
        ...
        pImpl->env = std::make_unique<Ort::Env>(ORT_LOGGING_LEVEL_WARNING, "ZenithStemSeparator");
        ...
        DBG("ONNXStemSeparator: Failed to initialize ONNX Runtime - " + juce::String(e.what()));
    }
#else
...
bool ONNXStemSeparator::initialize(const juce::File& modelPath) {
    if (!modelPath.existsAsFile()) {
        DBG("ONNXStemSeparator: Model file not found - " + modelPath.getFullPathName());
        return false;
    }
    ...
    try {
        ...
        pImpl->sessionOptions->SetGraphOptimizationLevel(GraphOptimizationLevel::ORT_ENABLE_ALL);
        
        // Set thread count for inference (audio processing typically uses 2-4 threads)
        ...
        pImpl->session = std::make_unique<Ort::Session>(*pImpl->env, wModelPath.c_str(), *pImpl->sessionOptions);
#else
        // Unix systems use regular strings
        std::string sModelPath = modelPath.getFullPathName().toStdString();
        pImpl->session = std::make_unique<Ort::Session>(*pImpl->env, sModelPath.c_str(), *pImpl->sessionOptions);
#endif
        ...
        if (numInputs > 0) {
            Ort::AllocatedStringPtr inputNameAllocated = pImpl->session->GetInputNameAllocated(0, allocator);
        ...
        for (size_t i = 0; i < numOutputs; ++i) {
            Ort::AllocatedStringPtr outputNameAllocated = pImpl->session->GetOutputNameAllocated(i, allocator);
            pImpl->outputNames.push_back(outputNameAllocated.get());
        }
        
        pImpl->isLoaded = true;
        DBG("ONNXStemSeparator: Model loaded successfully - " + modelPath.getFileName());
        DBG("ONNXStemSeparator: Inputs: " + juce::String((int)numInputs) + ", Outputs: " + juce::String((int)numOutputs));
    ...
    pImpl->isLoaded = false;
    DBG("ONNXStemSeparator: Model path stored, but ONNX Runtime not available - using DSP fallback");
...
ONNXStemSeparator::SeparationResult ONNXStemSeparator::separate(const juce::AudioBuffer<float>& input, double sampleRate) {
    ...
    int numChannels = juce::jmin(2, input.getNumChannels()); // Force stereo for stem separation
    ...
    if (isAvailable() && pImpl->isLoaded && pImpl->session) {
        try {
            ...
            std::vector<int64_t> inputShape = {1, static_cast<int64_t>(numChannels), static_cast<int64_t>(numSamples)};
            ...
            for (int ch = 0; ch < numChannels; ++ch) {
                ...
                float* destData = inputTensorValues.data() + (ch * numSamples);
            ...
            auto outputTensors = pImpl->session->Run(
                ...
                pImpl->inputNames.data(),
                &inputTensor,
                1,
                pImpl->outputNames.data(),
                pImpl->outputNames.size()
            );
            ...
            DBG("ONNXStemSeparator: Inference failed - " + juce::String(e.what()) + " - falling back to DSP");
            // Fall through to DSP fallback
        }
    }
#endif
...
#ifdef ZENITH_USE_ONNX_RUNTIME
void ONNXStemSeparator::copyTensorToBuffer(const float* tensorData, juce::AudioBuffer<float>& buffer, int numChannels, int numSamples) {
    ...
    if (pImpl->session) {
        ...
        if (!pImpl->inputShape.empty()) {
            ...
            for (size_t i = 0; i < pImpl->inputShape.size(); ++i) {
                ...
                if (i < pImpl->inputShape.size() - 1) info << ", ";
            }
            info << "]\n";
        }
    }
