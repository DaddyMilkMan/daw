...
public:
    ...
    enum class ChatStyle { Obedient, Teacher, Creative, Customized };
    enum class ReasoningPreview { Hidden, Compact, Detailed };
    enum class WingmanApprovalMode { AskBeforeApplying, AutoApproveSafeChanges };
    enum class WingmanResponseLength { Short, Balanced, Long };
    enum class WingmanSuggestionFrequency { Low, Balanced, High };
    ...
    void load() {
        ...
        if (auto* userSettings = props.getUserSettings()) {
            ...
            chatStyle_ = (ChatStyle)userSettings->getIntValue("wingmanChatStyle", (int)ChatStyle::Obedient);
            customPersonaPrompt_ = userSettings->getValue("wingmanCustomPersonaPrompt", "");
            reasoningPreview_ = (ReasoningPreview)userSettings->getIntValue("wingmanReasoningPreview", (int)ReasoningPreview::Compact);
            approvalMode_ = (WingmanApprovalMode)userSettings->getIntValue("wingmanApprovalMode", (int)WingmanApprovalMode::AskBeforeApplying);
            responseLength_ = (WingmanResponseLength)userSettings->getIntValue("wingmanResponseLength", (int)WingmanResponseLength::Balanced);
            suggestionFrequency_ = (WingmanSuggestionFrequency)userSettings->getIntValue("wingmanSuggestionFrequency", (int)WingmanSuggestionFrequency::Balanced);
            autoApproveSafeChanges_ = userSettings->getBoolValue("wingmanAutoApproveSafeChanges", false);

            // Features
            stayAwakeDuringProject_ = userSettings->getBoolValue("stayAwakeDuringProject", true);
            appLaunchCount_ = userSettings->getIntValue("appLaunchCount", 0);
        }
    }
    ...
    void save() {
        ...
        if (auto* userSettings = props.getUserSettings()) {
            ...
            userSettings->setValue("wingmanCustomPersonaPrompt", customPersonaPrompt_);
            userSettings->setValue("wingmanReasoningPreview", (int)reasoningPreview_);
            userSettings->setValue("wingmanApprovalMode", (int)approvalMode_);
            userSettings->setValue("wingmanResponseLength", (int)responseLength_);
            userSettings->setValue("wingmanSuggestionFrequency", (int)suggestionFrequency_);
            userSettings->setValue("wingmanAutoApproveSafeChanges", autoApproveSafeChanges_);
            
            // Features
            userSettings->setValue("stayAwakeDuringProject", stayAwakeDuringProject_);
            userSettings->setValue("appLaunchCount", appLaunchCount_);
    ...
    //==============================================================================
    // Wingman Chat Settings
    //==============================================================================
    void setChatStyle(ChatStyle style) { if (chatStyle_ != style) { chatStyle_ = style; save(); sendChangeMessage(); } }
    ChatStyle getChatStyle() const { return chatStyle_; }
    void setCustomPersonaPrompt(const juce::String& prompt) { if (customPersonaPrompt_ != prompt) { customPersonaPrompt_ = prompt; save(); sendChangeMessage(); } }
    juce::String getCustomPersonaPrompt() const { return customPersonaPrompt_; }
    void setReasoningPreview(ReasoningPreview mode) { if (reasoningPreview_ != mode) { reasoningPreview_ = mode; save(); sendChangeMessage(); } }
    ReasoningPreview getReasoningPreview() const { return reasoningPreview_; }
    void setApprovalMode(WingmanApprovalMode mode) { if (approvalMode_ != mode) { approvalMode_ = mode; save(); sendChangeMessage(); } }
    WingmanApprovalMode getApprovalMode() const { return approvalMode_; }
    void setResponseLength(WingmanResponseLength mode) { if (responseLength_ != mode) { responseLength_ = mode; save(); sendChangeMessage(); } }
    WingmanResponseLength getResponseLength() const { return responseLength_; }
    void setSuggestionFrequency(WingmanSuggestionFrequency mode) { if (suggestionFrequency_ != mode) { suggestionFrequency_ = mode; save(); sendChangeMessage(); } }
    WingmanSuggestionFrequency getSuggestionFrequency() const { return suggestionFrequency_; }
    void setAutoApproveSafeChanges(bool enabled) { if (autoApproveSafeChanges_ != enabled) { autoApproveSafeChanges_ = enabled; save(); sendChangeMessage(); } }
    bool getAutoApproveSafeChanges() const { return autoApproveSafeChanges_; }
    ...
    // App Lifecycle
    //==============================================================================
    int getAppLaunchCount() const { return appLaunchCount_; }
    void incrementAppLaunchCount() { ++appLaunchCount_; save(); }
...
private:
    ...
    ChatStyle chatStyle_ = ChatStyle::Obedient;
    juce::String customPersonaPrompt_;
    ReasoningPreview reasoningPreview_ = ReasoningPreview::Compact;
    WingmanApprovalMode approvalMode_ = WingmanApprovalMode::AskBeforeApplying;
    WingmanResponseLength responseLength_ = WingmanResponseLength::Balanced;
    WingmanSuggestionFrequency suggestionFrequency_ = WingmanSuggestionFrequency::Balanced;
    bool autoApproveSafeChanges_ = false;

    // Features
    bool stayAwakeDuringProject_ = true;
    int appLaunchCount_ = 0;
};
