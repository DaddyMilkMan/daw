...
int PluginHost::scanInternal(
  ...
  for (int i = 0; i < searchPaths.getNumPaths(); ++i) {
    ...
    for (const auto& file : filesToScan) {
        ...
        if (safeScanner.scanPluginOutOfProcess(file, desc)) {
...
void PluginHost::clearBlacklist() {
  safeScanner.getBlacklistManager().getEntries().clear();
  juce::File blacklistFile = juce::File::getSpecialLocation(juce::File::userApplicationDataDirectory)
        .getChildFile("ZenithDAW")
        .getChildFile("PluginBlacklist.xml");
  if (blacklistFile.existsAsFile()) {
      blacklistFile.deleteFile();
  }
  DBG("PluginHost: Cleared plugin blacklist");
}
