...
public:
    ...
    // Message thread only
    void addLane(const juce::String& paramId, std::shared_ptr<AutomationLane> lane);
    void clearLanes();

    // Audio thread safe
    const std::shared_ptr<AutomationLane> getLane(const juce::String& paramId) const;
...
private:
    ...
    std::vector<std::shared_ptr<AutomationSnapshot>> snapshotTrash_;
