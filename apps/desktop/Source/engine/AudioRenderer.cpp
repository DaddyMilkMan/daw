...
//==============================================================================
void AudioRenderer::prepare(double sampleRate, int blockSize, 
    ...
    DBG("AudioRenderer: Prepared with " + juce::String(numTracks) + 
        " tracks, " + juce::String(numAuxBuses) + " aux buses");
}
...
//==============================================================================
int AudioRenderer::calculatePDC(const std::vector<std::shared_ptr<Track>>& tracks) {
    int maxLatency = 0;
...
void AudioRenderer::applyPDCDelay(juce::AudioBuffer<float>& buffer, 
    ...
    for (int ch = 0; ch < buffer.getNumChannels(); ++ch) {
        ...
        for (int i = 0; i < numSamples; ++i) {
            // Read delayed sample
            int readPos = (writePos - delayNeeded + constants::kMaxPDCLatencySamples) 
