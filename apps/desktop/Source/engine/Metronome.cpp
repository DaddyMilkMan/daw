...
float Metronome::getLevel() const
{
    return level_.load();
}
...
void Metronome::triggerClick(float frequency)
{
    ...
    phaseIncrement_ = (frequency * juce::MathConstants<float>::twoPi) / static_cast<float>(sampleRate_);
    
    // Reset or smooth phase? Hard reset for click consistency
    currentPhase_ = 0.0f;
    
    currentNoteSamplesRemaining_ = static_cast<int>(kClickDurationSec * sampleRate_);
}
                                  ...
                                  const TempoMap& tempoMap)
{
    ...
    float* channelData1 = (numChannels > 1) ? bufferToFill.getWritePointer(1) : nullptr;
    ...
    for (int i = 0; i < numSamples; ++i)
    {
        ...
        // Because of floating point, "crossing" is better detected by observing the change in integral beat
        // But samplesToBeats might jump. 
        // Better approach: Calculate the NEXT beat's sample position and see if it falls in this buffer?
        // Or check simply if the integer part of beat changed.
        ...
        // A robust way used in DAWs is tracking the "last beat index" and firing if "current beat index" > "last".
        // However, we process sample by sample here (or small blocks). 
        // Since we are iterating i, let's just check equality with a epsilon relative to sample rate?
        // No, `samplesToBeats` is precise. The beat starts exactly when samplePos corresponds to beat X.0.
        // 
        ...
        if (currentNoteSamplesRemaining_ > 0)
        {
            ...
            float envelope = static_cast<float>(currentNoteSamplesRemaining_) / (kClickDurationSec * sampleRate_);
