...
//==============================================================================
TrackAutomationSynchronizer::TrackAutomationSynchronizer(ProjectState &ps,
                                                         Engine &eng)
...
void TrackAutomationSynchronizer::start(int updateRateHz) {
  ...
  DBG("TrackAutomationSynchronizer: Started at " + juce::String(updateRateHz) +
      " Hz");
}
...
void TrackAutomationSynchronizer::timerCallback() {
  ...
  // Phase 15: Use TempoMap for samples → beats conversion (handles variable
  // tempo)
  double playbackBeats =
      engine.getTempoMap().samplesToBeats(playheadSamples, sampleRate);
...
void TrackAutomationSynchronizer::valueTreePropertyChanged(
    juce::ValueTree &tree, const juce::Identifier &property) {
  // If a track property changed (mute, solo, armed, volume, pan), trigger
  // immediate sync
...
void TrackAutomationSynchronizer::valueTreeChildAdded(juce::ValueTree &parent,
...
void TrackAutomationSynchronizer::valueTreeChildRemoved(juce::ValueTree &parent,
                                                        juce::ValueTree &child,
                                                        int index) {
...
void TrackAutomationSynchronizer::valueTreeChildOrderChanged(
    juce::ValueTree &parent, int oldIndex, int newIndex) {
  juce::ignoreUnused(parent, oldIndex, newIndex);
}

void TrackAutomationSynchronizer::valueTreeParentChanged(
    juce::ValueTree &tree) {
...
double
TrackAutomationSynchronizer::sampleEnvelope(const juce::ValueTree &envelope,
...
void TrackAutomationSynchronizer::updateTrackAutomation(
    const juce::String &trackId, zenith::Track *track, double playbackBeats) {
  if (track == nullptr)
    return;
...
void TrackAutomationSynchronizer::rebuildListeners() {
  ...
  for (const auto &trackNode : tracksNode) {
    if (trackNode.hasType(zenith::ProjectState::ID_TRACK)) {
      juce::String trackId =
          trackNode[zenith::ProjectState::PROP_ID].toString();
      trackIdToIndex[trackId] = index;
      ++index;
    }
  }

  DBG("TrackAutomationSynchronizer: Rebuilt listeners for " +
