                        ...
                        const juce::MidiBuffer* incomingMidi)
{
    ...
    // Modifications to the plugin chain should ensure thread safety (e.g. suspend processing).
    if (!masterPlugins_.empty())
    {
        juce::MidiBuffer midi; // Master bus usually doesn't handle MIDI, but plugins might expect it
    ...
    int numSamples)
{
    ...
    for (auto& session : audioRecordingSessions_)
    {
        ...
        else if (session.numChannels == 2)
        {
            ...
            // For now, let's just use what we have, passing nullptr for missing channels (writer handles it?)
            // Actually ThreadedWriter::write expects valid pointers.
    ...
    double sampleRate)
{
    ...
    if (projectState_ != nullptr)
    {
        ...
        if (trackId.isNotEmpty())
        {
            ...
            if (tempoMap_)
            {
                startBeats = tempoMap_->samplesToBeats(recordingStartSamples, sampleRate);
                lengthBeats = tempoMap_->samplesToBeats(fileHandle->lengthInSamples, sampleRate);
            }
            else
            {
                // Fallback
                ...
                lengthBeats = (fileHandle->lengthInSamples / sampleRate) / secondsPerBeat;
            }
