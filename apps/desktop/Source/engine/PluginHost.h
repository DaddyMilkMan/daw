...
public:
  ...
  createInstance(const juce::PluginDescription &description, double sampleRate,
   ...
   */
  std::unique_ptr<juce::AudioPluginInstance>
  createInstance(const juce::String &identifier, double sampleRate,
                 int blockSize, juce::String &errorMessage);
  ...
  // Convenience wrapper
  std::unique_ptr<juce::AudioPluginInstance>
  createPlugin(const juce::PluginDescription &description);
  ...
  void blacklistPlugin(const juce::String &filePath);
  void clearBlacklist();
...
private:
  ...
  /**
   * @brief Performs out-of-process scanning for a single plugin file.
   * @return true if successful and description added.
   */
  bool scanOutOfProcess(const juce::File &file,
