...
void AudioTrack::getNextAudioBlock(const juce::AudioSourceChannelInfo& bufferToFill, 
    ...
    // Actually, TrackBase should handle automation. I'll need to move that back to TrackBase.
        ...
        for (auto* clip : currentSnapshot->clips) {
            if (clip != nullptr && clip->isPlaying() && clip->isActiveAt(playheadSamples)) {
                if (clip->getType() == Clip::Type::Audio) {
                    ...
                    const int channelsToMix = juce::jmin(bufferToFill.buffer->getNumChannels(), clipBuffer_.getNumChannels());
                    for (int ch = 0; ch < channelsToMix; ++ch) {
                        bufferToFill.buffer->addFrom(ch, bufferToFill.startSample, clipBuffer_, ch, 0, bufferToFill.numSamples);
                    }
                }
            }
        }
