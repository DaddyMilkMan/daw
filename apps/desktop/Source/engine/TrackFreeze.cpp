...
//==============================================================================
bool TrackFreezeManager::freezeTrack(Track &track, Engine &engine,
  ...
  juce::String filename =
      track.getName().replaceCharacter(' ', '_') + "_freeze_" +
...
//==============================================================================
void TrackFreezeManager::cancelFreeze() { shouldCancel_.store(true); }
...
//==============================================================================
void TrackFreezeManager::restorePluginStates(Track &track,
                                             const juce::ValueTree &states) {
  for (auto pluginState : states) {
    if (!pluginState.hasType("Plugin"))
      continue;

    int index = pluginState.getProperty("index", -1);
    if (index < 0 || index >= track.getNumPlugins())
      continue;
...
//==============================================================================
TrackFreezeManager::FreezeState *
TrackFreezeManager::getFreezeState(const juce::String &trackId) {
  auto it = freezeStates_.find(trackId);
...
void FreezeRenderThread::run() {
  ...
  totalSamples +=
      static_cast<juce::int64>(constants::kExportTailSeconds * sampleRate);
  ...
  auto fos = std::make_unique<juce::FileOutputStream>(outputFile_);
  ...
  std::unique_ptr<juce::OutputStream> outputStream = std::move(fos);
  ...
  while (samplesRendered < totalSamples && !shouldCancel_.load()) {
    ...
    int samplesToRender = static_cast<int>(juce::jmin(
        static_cast<juce::int64>(blockSize), totalSamples - samplesRendered));
    ...
    if (progress_) {
      ...
      juce::String status =
          "Freezing: " + juce::String(progressValue * 100.0f, 1) + "%";

      juce::MessageManager::callAsync(
          [=, progress = progress_]() { progress(progressValue, status); });
    }
  }
  ...
  // LIFETIME SAFETY FIX: Capture by value, not by reference to avoid dangling
  // refs We capture trackId to look up the track later, rather than holding a
  // reference
  ...
  juce::MessageManager::callAsync(
      [trackId, trackName, progressCopy, &track = track_]() {
