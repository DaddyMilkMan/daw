...
juce::File ProjectState::saveCrashDump()
{
    ...
    auto crashDir = documentsDir.getChildFile("ZenithDAW").getChildFile("CrashDumps");
...
juce::String ProjectState::addTrack(const juce::String& name, const juce::String& type)
{
    ...
    // Assign random color
    auto randomColor = juce::Colour::fromHSV(juce::Random::getSystemRandom().nextFloat(), 0.7f, 0.9f, 1.0f);
                              ...
                              const juce::String& actionName)
{
    ...
    undoManager.beginNewTransaction(actionName);
    clip.setProperty(PROP_LENGTH, newLengthSamples, &undoManager); // Note: PROP_LENGTH can store beats or samples depending on clip type/phase. 
    // For Phase 11, we assume samples for audio clips.

    DBG("ProjectState: Resized clip " + clipId + " to length=" + juce::String(newLengthSamples));
}
...
std::pair<juce::ValueTree, juce::ValueTree> ProjectState::findClip(const juce::String& clipId) const
{
    auto tracksNode = state.getChildWithName(ID_TRACKS);
...
juce::ValueTree ProjectState::findTrackInternal(const juce::String& trackId) const
{
    // O(1) Lookup
                            ...
                            const juce::String& actionName)
{
    ...
    {
        addNote(clipId, noteSpec.startBeats, noteSpec.lengthBeats, 
                noteSpec.pitch, noteSpec.velocity, ""); // Empty action since we already started transaction
    }
}
...
juce::String ProjectState::createTrack(const juce::String &type, const juce::String &name, const juce::String &actionName)
{
    undoManager.beginNewTransaction(actionName);
...
void ProjectState::moveMarker(const juce::String& markerId, double newBeats, const juce::String& actionName)
{
    ...
    for (int i = 0; i < markers.getNumChildren(); ++i)
    {
        ...
        if (marker[PROP_ID].toString() == markerId)
        {
            ...
            DBG("ProjectState: Moved marker " + markerId + " to " + juce::String(newBeats));
            return;
        }
    }
}
