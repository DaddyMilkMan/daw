    ...
    double splitBeats, const juce::String& actionName)
{
    ...
    if (audioFile.isNotEmpty())
    {
        ...
        if (rightClip.isValid())
        {
            ...
            // Fix: Offset is in seconds for audio clips, beats for MIDI?
            // Assuming offset is beat-based for consistency in this context,
            // but if it's sample/time based, we need conversion.
            // For now, simple addition is preserved but we must ensure type safety.
            double currentOffset = (double)clip.getProperty(ProjectState::PROP_OFFSET);
