...
static constexpr int kNumAutoColors = sizeof(kTrackColors) / sizeof(kTrackColors[0]);
...
juce::String TrackStateManager::addTrack(const juce::String& name, const juce::String& type)
{
...
juce::String TrackStateManager::getTrackName(const juce::String& trackId) const
{
    auto track = findTrack(trackId);
    return track.isValid() ? track[ProjectState::PROP_NAME].toString() : juce::String();
}

juce::String TrackStateManager::getTrackType(const juce::String& trackId) const
{
    auto track = findTrack(trackId);
    return track.isValid() ? track[ProjectState::PROP_TYPE].toString() : juce::String();
}

float TrackStateManager::getTrackVolume(const juce::String& trackId) const
{
    auto track = findTrack(trackId);
    return track.isValid() ? static_cast<float>(track[ProjectState::PROP_VOLUME]) : 1.0f;
}

float TrackStateManager::getTrackPan(const juce::String& trackId) const
{
    auto track = findTrack(trackId);
    return track.isValid() ? static_cast<float>(track[ProjectState::PROP_PAN]) : 0.0f;
}

bool TrackStateManager::isTrackMuted(const juce::String& trackId) const
{
    auto track = findTrack(trackId);
    return track.isValid() ? static_cast<bool>(track[ProjectState::PROP_MUTE]) : false;
}

bool TrackStateManager::isTrackSolo(const juce::String& trackId) const
{
    auto track = findTrack(trackId);
    return track.isValid() ? static_cast<bool>(track[ProjectState::PROP_SOLO]) : false;
}

bool TrackStateManager::isTrackArmed(const juce::String& trackId) const
{
    auto track = findTrack(trackId);
    return track.isValid() ? static_cast<bool>(track[ProjectState::PROP_ARMED]) : false;
}

juce::Colour TrackStateManager::getTrackColor(const juce::String& trackId) const
{
    auto track = findTrack(trackId);
    if (!track.isValid())
...
void TrackStateManager::setTrackName(const juce::String& trackId, const juce::String& name,
                                      const juce::String& actionName)
{
    ...
    if (track.isValid())
    {
        ...
        // FIX: Pass address
        track.setProperty(ProjectState::PROP_NAME, name, &projectState_.getUndoManager());
    }
}

void TrackStateManager::setTrackVolume(const juce::String& trackId, float volume,
                                        const juce::String& actionName)
{
    auto track = findTrack(trackId);
    if (track.isValid())
    {
        projectState_.getUndoManager().beginNewTransaction(actionName);
        track.setProperty(ProjectState::PROP_VOLUME, volume, &projectState_.getUndoManager());
    }
}
                                     ...
                                     const juce::String& actionName)
{
    ...
    {
        projectState_.getUndoManager().beginNewTransaction(actionName);
        track.setProperty(ProjectState::PROP_PAN, pan, &projectState_.getUndoManager());
    }
}
                                      ...
                                      const juce::String& actionName)
{
    ...
    {
        projectState_.getUndoManager().beginNewTransaction(actionName);
        track.setProperty(ProjectState::PROP_MUTE, muted, &projectState_.getUndoManager());
    }
}
                                      ...
                                      const juce::String& actionName)
{
    ...
    {
        projectState_.getUndoManager().beginNewTransaction(actionName);
        track.setProperty(ProjectState::PROP_SOLO, solo, &projectState_.getUndoManager());
    }
}
                                       ...
                                       const juce::String& actionName)
{
    ...
    {
        projectState_.getUndoManager().beginNewTransaction(actionName);
        track.setProperty(ProjectState::PROP_ARMED, armed, &projectState_.getUndoManager());
    }
}

void TrackStateManager::setTrackColor(const juce::String& trackId, const juce::Colour& color,
                                       bool manuallySet, const juce::String& actionName)
{
    ...
    {
        projectState_.getUndoManager().beginNewTransaction(actionName);
        track.setProperty(ProjectState::PROP_COLOR, color.toString(), &projectState_.getUndoManager());
        track.setProperty(ProjectState::PROP_MANUALLY_COLORED, manuallySet, &projectState_.getUndoManager());
    }
}
...
juce::ValueTree TrackStateManager::findTrack(const juce::String& trackId) const
{
    auto tracksNode = getTracksContainer();
    if (!tracksNode.isValid())
        return {};
