...
std::vector<float> GenreDetector::computeMFCC(const juce::AudioBuffer<float>& audio, double sampleRate) {
    ...
    auto spectrum = visualAnalyzer->computeSpectrum(audio.getReadPointer(0), audio.getNumSamples());
    if (spectrum.empty()) return std::vector<float>(numCoeffs, 0.0f);
    
    // 2. Mel Filterbank Setup
    ...
    float maxMel = hzToMel(static_cast<float>(sampleRate) / 2.0f);
    ...
    for (int i = 0; i < numFilters + 2; ++i) {
        binPoints[i] = static_cast<int>(std::floor((fftSize + 1) * melPoints[i] / sampleRate));
    }
    
    // 3. Apply Filterbank to Power Spectrum
    std::vector<float> filterEnergies(numFilters, 0.0f);
    for (int m = 1; m <= numFilters; ++m) {
        for (int k = binPoints[m-1]; k < binPoints[m]; ++k) {
            float weight = (float)(k - binPoints[m-1]) / (binPoints[m] - binPoints[m-1]);
            filterEnergies[m-1] += weight * spectrum[k] * spectrum[k];
        }
        for (int k = binPoints[m]; k < binPoints[m+1]; ++k) {
            float weight = (float)(binPoints[m+1] - k) / (binPoints[m+1] - binPoints[m]);
            filterEnergies[m-1] += weight * spectrum[k] * spectrum[k];
        }
    }
    ...
    // 4. Log and DCT (Type II)
    std::vector<float> mfccs(numCoeffs, 0.0f);
    for (int i = 0; i < numCoeffs; ++i) {
        for (int j = 0; j < numFilters; ++j) {
            float val = std::log(std::max(1e-6f, filterEnergies[j]));
            mfccs[i] += val * std::cos(pi * i * (j + 0.5f) / numFilters);
        }
    }
