...
void PredictiveMastering::updateModelWeights(std::unique_ptr<NeuralNetwork>& model,
                                             ...
                                             float learningRate) {
    if (model) {
        // Call actual training method from RealNeuralNetwork
        float loss = model->trainSingle(input, target, learningRate);
        
        if (std::isnan(loss)) {
            DBG("PredictiveMastering: WARNING - Training produced NaN loss. Resetting layer weights.");
            // In a real system, we might reload from last checkpoint or reduce LR
        }
    }
}
