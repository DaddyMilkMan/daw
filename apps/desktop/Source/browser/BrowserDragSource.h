...
public:
  ...
  explicit BrowserDragData(std::shared_ptr<BrowserItem> item) : item_(item) {}

  std::shared_ptr<BrowserItem> getItem() const { return item_; }
  BrowserItemType getType() const {
    return item_ ? item_->type : BrowserItemType::Unknown;
  }
...
public:
   ...
   */
  static BrowserItemType
  getTypeFromDescription(const juce::String &description) {
    if (!isBrowserDrag(description))
      return BrowserItemType::Unknown;

    juce::String typeName =
        description.fromFirstOccurrenceOf(dragDescriptionPrefix, false, false);
    ...
    if (typeName == "Plugin")
      return BrowserItemType::Plugin;
    if (typeName == "Instrument")
      return BrowserItemType::Instrument;
    if (typeName == "Preset")
      return BrowserItemType::Preset;
    if (typeName == "Folder")
      return BrowserItemType::Folder;
  ...
  static void startDrag(juce::Component *sourceComponent,
    ...
    // We encode item info in the description since JUCE 8 doesn't support
    // custom drag data
    juce::String description =
        createDragDescription(item->type) + "|" + item->id + "|" + item->name;
  ...
  static bool parseDragDescription(const juce::String &description,
                                   ...
                                   juce::String &outId, juce::String &outName) {
    if (!isBrowserDrag(description))
      return false;
...
private:
  ...
  static juce::Image createDragImage(std::shared_ptr<BrowserItem> item) {
    ...
    case BrowserItemType::AudioFile:
      iconColour = juce::Colour(0xff66dd66);
      break;
    case BrowserItemType::MidiFile:
      iconColour = juce::Colour(0xffdd66dd);
      break;
    case BrowserItemType::Plugin:
      iconColour = juce::Colour(0xff6699ff);
      break;
    case BrowserItemType::Instrument:
      iconColour = juce::Colour(0xff66ccff);
      break;
    case BrowserItemType::Preset:
      iconColour = juce::Colour(0xffcc66dd);
      break;
    default:
      iconColour = juce::Colours::grey;
      break;
    }
    ...
    g.drawText(displayName, 30, 0, width - 35, height,
