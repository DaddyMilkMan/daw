...
BrowserModel::BrowserModel(InstrumentRegistry& registry, PluginHost& host)
    : instrumentRegistry_(registry),
      pluginHost_(host)
{
    // Default paths
    userLibraryPaths_.add(juce::File::getSpecialLocation(juce::File::userMusicDirectory).getFullPathName());
...
{
    rootItem = std::make_shared<BrowserItem>("root", "Root", BrowserItemType::Folder);
    allIndexableItems_.clear();

    // 0. Favorites (always at top)
    favoritesNode_ = std::make_shared<BrowserItem>("favorites_root", "★ Favorites", BrowserItemType::Folder);
    rootItem->addChild(favoritesNode_);

    // 1. Internal Instruments Group
    instrumentsNode = std::make_shared<BrowserItem>("instruments_root", "Instruments", BrowserItemType::Folder);
    rootItem->addChild(instrumentsNode);
    
    // 2. Plugins Group
    pluginsNode = std::make_shared<BrowserItem>("plugins_root", "Plugins", BrowserItemType::Folder);
    pluginsNode->metadata.description = "VST3 and AU Plugins";
    rootItem->addChild(pluginsNode);

    // 3. User Library
    userLibraryNode = std::make_shared<BrowserItem>("library_root", "User Library", BrowserItemType::Folder);
    rootItem->addChild(userLibraryNode);

    // 4. Presets
    presetsNode = std::make_shared<BrowserItem>("presets_root", "Presets", BrowserItemType::Folder);
...
std::vector<std::shared_ptr<BrowserItem>> BrowserModel::search(const juce::String& queryText)
{
...
std::vector<std::shared_ptr<BrowserItem>> BrowserModel::getItemsByType(BrowserItemType type)
{
    std::vector<std::shared_ptr<BrowserItem>> results;
...
std::vector<juce::String> BrowserModel::getItemTags(std::shared_ptr<BrowserItem> item) const
{
    if (!item) return {};
...
std::vector<std::shared_ptr<BrowserItem>> BrowserModel::getItemsByTag(const juce::String& tag)
{
    std::vector<std::shared_ptr<BrowserItem>> results;
    juce::String normalizedTag = tag.toLowerCase().trim();
