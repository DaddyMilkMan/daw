...
# Optional SVG module (SkSVGDOM)
find_library(SKIA_SVG_LIBRARY NAMES svg skia_svg PATHS ${SKIA_LIB_DIR} NO_DEFAULT_PATH)
if(SKIA_SVG_LIBRARY)
    message(STATUS "  Skia SVG module: ${SKIA_SVG_LIBRARY}")
    target_link_libraries(ZenithDAW PRIVATE ${SKIA_SVG_LIBRARY})
    if(TARGET ZenithDAWTests)
        target_link_libraries(ZenithDAWTests PRIVATE ${SKIA_SVG_LIBRARY})
    endif()
    # Ensure core Skia symbols resolve after svg (static link order)
    target_link_libraries(ZenithDAW PRIVATE ${SKIA_LIBRARY})
    if(TARGET ZenithDAWTests)
        target_link_libraries(ZenithDAWTests PRIVATE ${SKIA_LIBRARY})
    endif()
...
# Optional Skia module libs (for SVG/text shaping support)
set(_skia_optional_libs skshaper skparagraph skunicode_core skunicode_icu sksg skottie)
foreach(_lib ${_skia_optional_libs})
    find_library(SKIA_OPTIONAL_${_lib} NAMES ${_lib} PATHS ${SKIA_LIB_DIR} NO_DEFAULT_PATH)
    if(SKIA_OPTIONAL_${_lib})
        message(STATUS "  Skia optional module: ${SKIA_OPTIONAL_${_lib}}")
        target_link_libraries(ZenithDAW PRIVATE ${SKIA_OPTIONAL_${_lib}})
        if(TARGET ZenithDAWTests)
            target_link_libraries(ZenithDAWTests PRIVATE ${SKIA_OPTIONAL_${_lib}})
        endif()
    endif()
endforeach()

# ICU (needed by skunicode_icu)
set(_icu_libs icuuc icui18n icudata)
foreach(_iculib ${_icu_libs})
    find_library(ICU_${_iculib} NAMES ${_iculib} PATHS ${SKIA_LIB_DIR} NO_DEFAULT_PATH)
    if(ICU_${_iculib})
        message(STATUS "  ICU dependency: ${ICU_${_iculib}}")
        target_link_libraries(ZenithDAW PRIVATE ${ICU_${_iculib}})
        if(TARGET ZenithDAWTests)
            target_link_libraries(ZenithDAWTests PRIVATE ${ICU_${_iculib}})
        endif()
    endif()
endforeach()

# Expat (required by Skia XML)
find_library(EXPAT_LIBRARY NAMES expat PATHS ${SKIA_LIB_DIR} NO_DEFAULT_PATH)
if(EXPAT_LIBRARY)
    message(STATUS "  Expat dependency: ${EXPAT_LIBRARY}")
    target_link_libraries(ZenithDAW PRIVATE ${EXPAT_LIBRARY})
    if(TARGET ZenithDAWTests)
        target_link_libraries(ZenithDAWTests PRIVATE ${EXPAT_LIBRARY})
    endif()
endif()
