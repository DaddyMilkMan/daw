        ...
        switch (destFormat)
        {
            case BitDepth::Int16:
                for (int i = 0; i < numSamples; ++i) {
                    float s = std::max(-1.0f, std::min(1.0f, srcChannel[i]));
                    int16_t v = (s >= 0) ? static_cast<int16_t>(s * 32767.0f) 
                                         : static_cast<int16_t>(s * 32768.0f);
                    char* d = rawDest + (i * numChannels + ch) * 2;
                    *reinterpret_cast<int16_t*>(d) = static_cast<int16_t>(juce::ByteOrder::swapIfBigEndian((uint16_t)v));
                }
                break;
            ...
            case BitDepth::Int24:
                for (int i = 0; i < numSamples; ++i) {
                    float s = std::max(-1.0f, std::min(1.0f, srcChannel[i]));
                    int32_t v = (s >= 0) ? static_cast<int32_t>(s * 8388607.0f) 
                                         : static_cast<int32_t>(s * 8388608.0f);
                    char* d = rawDest + (i * numChannels + ch) * 3;
            ...
            case BitDepth::Int32:
                for (int i = 0; i < numSamples; ++i) {
                    float s = std::max(-1.0f, std::min(1.0f, srcChannel[i]));
                    int32_t v = (s >= 0) ? static_cast<int32_t>(s * 2147483647.0f) 
                                         : static_cast<int32_t>(s * 2147483648.0f);
                    char* d = rawDest + (i * numChannels + ch) * 4;
                    *reinterpret_cast<int32_t*>(d) = static_cast<int32_t>(juce::ByteOrder::swapIfBigEndian((uint32_t)v));
                }
                break;
            ...
            case BitDepth::Float32:
                for (int i = 0; i < numSamples; ++i) {
                    char* d = rawDest + (i * numChannels + ch) * 4;
                    // Float binary layout is generally consistent, handling endianness if needed
                    // For now assuming LE target as per previous function name
                    float v = srcChannel[i];
                    uint32_t iv;
                    memcpy(&iv, &v, 4);
                    iv = juce::ByteOrder::swapIfBigEndian(iv);
                    memcpy(d, &iv, 4);
                }
                break;
        }
