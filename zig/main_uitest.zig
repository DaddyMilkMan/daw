...
pub fn main() !void {
    ...
    step(&u, &p, bar, &state, 170, 29, true);
    step(&u, &p, bar, &state, 170, 29, false);
    const play1 = state.playing;

    // grab track-0 (Drums) mixer fader near the top, drag to the bottom, release
    step(&u, &p, bar, &state, 250, 466, true);
    step(&u, &p, bar, &state, 250, 514, true);
    step(&u, &p, bar, &state, 250, 514, false);
