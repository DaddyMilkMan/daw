...
| `widgets.zig` | **GPU immediate-mode widgets** — faders, sliders (+bipolar), rotary knobs, toggles, checkboxes, segmented tabs, progress, **data tables**, icon slots; per-id hover/press animation. Drive with `flex` rects or raw coords. | `gpu2d` (→ `color`) |
