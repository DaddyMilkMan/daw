...
- **The layout engine:** the above + `flex.zig`.
- **A windowed Linux app:** add `window_glx.zig` (links `X11`, `GL`, `libc`).
- **A CPU-only renderer (no GPU):** copy `color.zig`, `font.zig`, `render2d.zig`.

DAW-specific code (not part of the reusable toolkit): `ui.zig`, `ui_gpu.zig`,
`uikit.zig`, `project.zig`, `arrangement.zig`, the audio/DSP modules, and the
`main_*.zig` entry points.
